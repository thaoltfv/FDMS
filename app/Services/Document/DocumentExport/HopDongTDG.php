...
class HopDongTDG
{
    ...
    public function generateDocx($company, $certificate, $format, $appraises): array
    {
        ...
        => false));
        $row2->addCell(2700, $cellVCentered)->addText('-   Địa chỉ', null, $cellHCentered);
        $row2->addCell(7200, $cellVCentered)->addText(': ' . $certificate->petitioner_address, null, $cellHCentered);

        $row3 = $table->addRow(100, array(
            'tblHeader' => false,
            'cantSplit' => false
        ));
        $row3->addCell(2700, $cellVCentered)->addText('-   Số CCCD', null, $cellHCentered);
        $row3->addCell(7200, $cellVCentered)->addText(': ' . $certificate->petitioner_identity_card, null, $cellHCentered);

        $row4 = $table->addRow(100, array('tblHeader' => false, 'cantSplit' => false));
        $row4->addCell(2700, $cellVCentered)->addText('-   Số điện thoại', null, $cellHCentered);
        $row4->addCell(7200, $cellVCentered)->addText(': ' . $certificate->petitioner_phone, null, $cellHCentered);
