...
class GiayYeuCau
{
    ...
    public function generateDocx($company, $certificate, $format, $documentConfig): array
    {
        ...
        $listItem = $section->addListItem();
        // Create the first paragraph with its own style
        $paragraph1 = $listItem->addTextRun()->addText("Các hồ sơ, dữ liệu cá nhân, cung cấp gồm: ");


        // Create the second paragraph with its own style
        $paragraph2 = $listItem->addTextRun()->addText("01 Bản Giấy chứng nhận quyền sử dụng đất quyền sở hữu nhà ở và tài sản khác gắn liền với đất số CK 096662 số vào sổ cấp GCN:CS23305/DA ngày 30/05/2018 do Sở Tài Nguyên và Môi Trường thành phố Hồ Chí Minh cấp.");
        $paragraph2->getFontStyle()->setItalic(true);
