...
class ReportAppraisal extends Report
{
    ...
    public function printTitle(Section $section, $data)
    {
        ...
        $section->addText('(Kèm theo Chứng thư Thẩm định giá số ' . htmlspecialchars($this->certificateCode) . ', ngày ' . $this->certificateShortDateText . ')', ['italic' => true], ['align' => 'center', 'spaceAfter' => 300]);
    }
    protected function getAssetName($certificate)
    {
        ...
        return htmlspecialchars($name_assets);
    }
    ...
    protected function step1Sub2($section, $certificate)
    {
        ...
        $section->addListItem('Họ và tên Tổng Giám đốc: ' . ((isset($certificate->appraiserManager) && isset($certificate->appraiserManager->name)) ? htmlspecialchars($certificate->appraiserManager->name) : ''), 0, null, 'bullets');
        $section->addListItem('Họ và tên Thẩm định viên về giá: ' . ((isset($certificate->appraiser) && isset($certificate->appraiser->name)) ? htmlspecialchars($certificate->appraiser->name) : ''), 0, null, 'bullets');
        // $section->addListItem('Người lập báo cáo: ' . (isset($certificate->createdBy->name) ? $certificate->createdBy->name : ''), 0, null, 'bullets');
        $section->addListItem('Trợ lý thẩm định viên: ' . (isset($certificate->appraiserPerform)  && isset($certificate->appraiserPerform->name) ? htmlspecialchars($certificate->appraiserPerform->name) : ''), 0, null, 'bullets');
    }
    ...
    protected function step1Sub4($section, $certificate)
    {
        ...
        $listTmp->addText(htmlspecialchars($this->contractCode) . ' ' . $this->documentLongDateText . " giữa " . htmlspecialchars($this->companyName) . " và " . htmlspecialchars($certificate->petitioner_name) . '.');
        ...
        $listTmp->addText(htmlspecialchars($appraisePurpose) . '.');
    }
    ...
    protected function step2(Section $section, $certificate)
    {
        ...
        if (isset($certificate->legalDocumentsOnValuation))
            foreach ($certificate->legalDocumentsOnValuation as $doc) {
                ...
                $table->addCell(2000, $this->cellVCentered)->addText(isset($doc['document_type']) ? htmlspecialchars($doc['document_type']) : '', [], ['align' => 'left']);
                $table->addCell(3300, $this->cellVCentered)->addText(isset($doc['date']) ? $doc['date'] : '', [], ['align' => 'left']);
                $table->addCell(4000, $this->cellVCentered)->addText(isset($doc['content']) ? CommonService::nl2br(htmlspecialchars($doc['content'])) : '');
            }
        ...
        foreach ($realEstates as $realEstate) {
            if ($realEstate->assetType->acronym === 'CC') {
                ...
                foreach ($laws as $law) {
                    ...
                    $result['title'] = CommonService::nl2br(htmlspecialchars($result['title']));
                    $result['document_num'] = $law->document_num ? htmlspecialchars($law->document_num) : '';
                    $result['document_date'] = !empty($law->document_date) ?  ' ngày ' . date_format(date_create($law->document_date), "d/m/Y") : '';
                    $result['content'] = !empty($law->content) ? CommonService::nl2br(htmlspecialchars($law->content)) : '';
                    $result['certifying_agency'] = !empty($law->certifying_agency) ? CommonService::nl2br(htmlspecialchars($law->certifying_agency)) : '';
                ...
                foreach ($laws as $law) {
                    ...
                    $result['title'] = CommonService::nl2br(htmlspecialchars($result['title']));
                    $result['document_num'] = $law->date ? htmlspecialchars($law->date) : '';
    ...
    protected function assetCharacteristicsApartment(Section $section, $realEstate)
    {
        ...
        $table->addCell($this->rowFourthWidth, ['borderRightSize' => 'none'])->addText(htmlspecialchars($assetName));
        //3
        $table->addRow(400, $this->cantSplit);
        ...
        $table->addCell($this->rowFourthWidth, ['borderLeftSize' => 'none'])->addText('Tầng ' . htmlspecialchars($apartment->apartmentAssetProperties->floor->name));
        //4
        $table->addRow(400, $this->cantSplit);
        ...
        $table->addCell($this->rowThirdWidth, ['borderRightSize' => 'none'])->addText('');
        $table->addCell($this->rowFourthWidth, ['borderLeftSize' => 'none'])->addText(CommonService::mbCaseTitle(htmlspecialchars($apartment->apartmentAssetProperties->direction->description)));
        //5
        $table->addRow(400, $this->cantSplit);
    ...
    protected function getToThua($appraise)
    {
        ...
        return htmlspecialchars($strToThua);
    }
            ...
            foreach ($property->propertyDetail as $item) {
                ...
                if (isset($item->is_zoning) && $item->is_zoning) {
                    if ($countZoning == 0) {
                        ...
                        $isZoning = true;
                        $desciptionZoning = 'Thửa đất có ' . number_format(floatval($totalViolateArea), 2, ',', '.') . $this->m2  . ' thuộc ' . htmlspecialchars($item->type_zoning);
                    } else {
                        if ($isZoning == true) {
                            $typeZoning = str_replace(' ', '', mb_strtolower(htmlspecialchars($item->type_zoning)));
                            if (strcmp($typeZoningDefault, $typeZoning) == 0) {
                                $totalViolateArea = $totalViolateArea + $item->planning_area;
                                $desciptionZoning = 'Thửa đất có ' .  number_format(floatval($totalViolateArea), 2, ',', '.') . $this->m2  . ' thuộc ' . htmlspecialchars($item->type_zoning);
                            } else {
                                $totalViolateArea = $item->planning_area;
                                $desciptionZoning = $desciptionZoning . ', ' . number_format(floatval($totalViolateArea), 2, ',', '.') . $this->m2  . ' thuộc ' . htmlspecialchars($item->type_zoning);
                            }
                        } else {
                            $totalViolateArea = $item->planning_area;
                            $desciptionZoning = 'Thửa đất có ' . number_format(floatval($totalViolateArea), 2, ',', '.') . $this->m2  . ' thuộc ' . htmlspecialchars($item->type_zoning);
                ...
                if (isset($item->landTypePurpose->description) && !isset($existLandTypePurpose[$item->landTypePurpose->acronym])) {
                    ...
                    $mdsd .= ($sttTmp) ? ', ' : '';
                    $mdsd .= CommonService::mbUcfirst(htmlspecialchars($item->landTypePurpose->description)) . ' (' . htmlspecialchars($item->landTypePurpose->acronym) . ')';
                    $sttTmp++;
                }
                $countZoning += 1;
            }
    ...
    protected function assetCharacteristicsAppraiseLegal($table, $appraise)
    {
        ...
        $table->addCell($this->rowFourthWidth, ['borderLeftSize' => 'none'])
            ->addText(htmlspecialchars($strToThua . ', ' . $appraise->ward->name . ', ' . $appraise->district->name . ', ' . $appraise->province->name . '.'), null, ['align' => 'left']);
        $table->addRow(400, $this->cantSplit);
        $table->addCell(null, ['valign' => 'center', 'vMerge' => 'continue']);
        $table->addCell(null, ['valign' => 'center', 'vMerge' => 'continue']);
        ...
        $table->addCell($this->rowFourthWidth, ['borderLeftSize' => 'none'])
            ->addText(htmlspecialchars($thsd), null, ['align' => 'left']);
        $table->addRow(400, $this->cantSplit);
        $table->addCell(null, ['valign' => 'center', 'vMerge' => 'continue']);
        $table->addCell(null, ['valign' => 'center', 'vMerge' => 'continue']);
        ...
        if (isset($appraise->appraiseLaw[0])) {
            ...
            $textRun->addText(htmlspecialchars($appraiseLaw->origin_of_use), null, ['align' => 'left']);
        }
        $table->addRow(400, $this->cantSplit);
        $table->addCell(null, ['valign' => 'center', 'vMerge' => 'continue']);
        $table->addCell(null, ['valign' => 'center', 'vMerge' => 'continue']);
    ...
    protected function assetCharacteristicsAppraiseLocation($table, $appraise)
    {
        ...
        foreach ($appraise->properties as $index => $property) {
            $positionType .= ($index) ? ', ' : '';
            $positionType .= (isset($property->description) ? htmlspecialchars($property->description) : '');
        }
    ...
    protected function assetCharacteristicsAppraiseTraffic($table, $appraise)
    {
        ...
        foreach ($appraise->properties as $index => $property) {
            if (isset($property->propertyTurningTime) && count($property->propertyTurningTime)) {
                ...
                $mainRoad = (isset($itemTmp->material->description) ? CommonService::mbUcfirst(htmlspecialchars($itemTmp->material->description)) : '') . ' rộng khoảng ' . number_format($itemTmp->main_road_length, 2, ',', '.') . ' m ';
                $mainRoad = ($index) ? $mainRoad : '' . $mainRoad;
                $roadCellTmp->addText($mainRoad, ['bold' => false], ['align' => 'left']);
            } else {
                $mainRoad = "";
                $mainRoad .= (isset($property->material->description) ? CommonService::mbUcfirst(htmlspecialchars($property->material->description)) : '') . ' rộng khoảng ' . (isset($property->main_road_length) ? number_format($property->main_road_length, 2, ',', '.') . 'm' : '');
                if (!empty($mainRoad)) {
                    $roadCellTmp->addText('' . CommonService::mbUcfirst(htmlspecialchars($mainRoad)), ['bold' => false], ['align' => 'left']);
                }
            }
        }
    }
    ...
    protected function assetCharacteristicsAppraiseBusiness($table, $appraise)
    {
        ...
        foreach ($appraise->properties as $index => $property) {
            $business .= ($index) ? ', ' : '';
            $business .= (isset($property->business) && isset($property->business->description)) ? htmlspecialchars($property->business->description) : '';
        }
        ...
        foreach ($appraise->properties as $index => $property) {
            $conditions .= ($index) ? ', ' : '';
            $conditions .= (isset($property->conditions) && isset($property->conditions->description)) ? htmlspecialchars($property->conditions->description) : '';
        }
    ...
    protected function assetCharacteristicsAppraiseShape($table, $appraise)
    {
        ...
        foreach ($appraise->properties as $index => $property) {
            $landShape .= ($index) ? ', ' : '';
            $landShape .= (isset($property->landShape) && isset($property->landShape->description)) ? htmlspecialchars($property->landShape->description) : '';
        }
    ...
    protected function assetCharacteristicsAppraiseStatus($section, $table, $appraise)
    {
        ...
        if (isset($appraise->tangibleAssets) && count($appraise->tangibleAssets)) {
            ...
            foreach ($appraise->tangibleAssets as $index => $tangibleAsset) {
                ...
                $buildingType =  isset($tangibleAsset->tangible_name) ? CommonService::mbUcfirst(htmlspecialchars($tangibleAsset->tangible_name)) : '';
                ...
                $cellTmp->addText('   ' . str_replace("\n", '<w:br/>   ', htmlspecialchars($tangibleAsset->contruction_description)), null, ['valign' => 'center', 'align' => 'left']);
    ...
    protected function step5(Section $section, $certificate)
    {
        ...
        $section->addListItem('Căn cứ vào mục đích, tính chất đặc điểm của tài sản thẩm định giá, Công ty TNHH Thẩm định giá ' . htmlspecialchars($this->acronym) . '  chọn ' . htmlspecialchars($basicProperty) . ' làm cơ sở thẩm định giá.', 0, null, 'bullets', $this->indentFistLine);
        $section->addListItem(htmlspecialchars($basicPropertyDescription), 0, null, 'bullets', $this->indentFistLine);
    ...
    protected function getPrincipleOfValuationDescription($certificatePrinciple, $section)
    {
        foreach ($certificatePrinciple as $index => $item) {
            $section->addListItem(htmlspecialchars($item->name), 0, null, 'bullets', $this->indentFistLine);
        }
    ...
    protected function step7(Section $section, $certificate)
    {
        ...
        $section->addListItem(htmlspecialchars($certificate->document_description), 0, null, 'bullets');
    }
    //VIII
    ...
    protected function step8sub3Appraise(Section $section)
    {
        ...
        foreach ($appraiseApproaches as $item) {
            $section->addListItem("Cách tiếp cận: " . htmlspecialchars($item->appraiseApproach->name), 0, ['bold' => true, 'italic' => true], 'bullets');
            $section->addText(htmlspecialchars($item->appraiseApproach->description), null, $this->indentFistLine);
            $section->addListItem("Phương pháp sử dụng: " . htmlspecialchars($item->appraiseMethodUsed->name), 0, ['bold' => true, 'italic' => true], 'bullets');
            $section->addText(htmlspecialchars($item->appraiseMethodUsed->description), null, $this->indentFistLine);
        }
        if ($this->isTangibleAsset) {
    ...
    protected function step8sub3Apartment(Section $section)
    {
        ...
        foreach ($approaches as $item) {
            $section->addListItem("Cách tiếp cận: " . htmlspecialchars($item->approach->name), 0, ['bold' => true, 'italic' => true], 'bullets');
            $section->addText(htmlspecialchars($item->approach->description), null, $this->indentFistLine);
            $section->addListItem("Phương pháp sử dụng: " . htmlspecialchars($item->methodUsed->name), 0, ['bold' => true, 'italic' => true], 'bullets');
            $section->addText(htmlspecialchars($item->methodUsed->description), null, $this->indentFistLine);
        }
    }
    ...
    protected function tangibleAsset(Section $section)
    {
        ...
        $section->addText('+ ' . htmlspecialchars($this->acronym) . ' căn cứ theo Quyết định số 09-11/2019/QĐ-UBND ngày 11/3/2019 về việc Ban hành Quy định về giá nhà, giá vật kiến trúc trên địa bàn tỉnh Đồng Nai kết hợp khảo sát đơn giá xây dựng thực tế tại thì trường tỉnh Đồng Nai và tìm hiểu thông tin đơn giá xây dựng của các công ty xây dựng trên địa bàn tỉnh Đồng Nai. ', null, ['indentation' => ['firstLine' => Converter::inchToTwip(0)]]);
        ...
        foreach ($constructionCompanies as $index => $item) {
            $stt++;
            $section->addText(($stt) . '. ' . htmlspecialchars($item->constructionCompany->name) . '.', null, $this->indentFistLine);
            $section->addText('- Địa chỉ: ' .  htmlspecialchars($item->constructionCompany->address) . '.', null, $this->indentFistLine);
            $section->addText('- Điện thoại: ' . htmlspecialchars($item->constructionCompany->phone_number), null, $this->indentFistLine);
            $section->addText('- Giám đốc: ' . htmlspecialchars($item->constructionCompany->manager_name) . '.', null, $this->indentFistLine);
        }
        $section->addText(($stt + 1) . '. Các tài liệu lưu trữ của ' . htmlspecialchars($this->acronym) . '.', null, $this->indentFistLine);
        $section->addText('b) Về đánh giá chất lượng còn lại', ['bold' => true], $this->indentFistLine);
        $section->addText('- Căn cứ theo biên bản kiểm kê và kết quả khảo sát hiện trạng của Tổ thẩm định giá. ' . htmlspecialchars($this->acronym) . ' đánh giá nguyên giá, chất lượng còn lại của công trình xây dựng:', null, $this->indentFistLine);
        $section->addText('❖ Phương pháp đánh giá chất lượng còn lại nhà cửa, vật kiến trúc: ' . htmlspecialchars($this->acronym) . ' lựa chọn 2 phương pháp:', ['bold' => true], $this->indentFistLine);
    ...
    protected function step9(Section $section, $certificate)
    {
        ...
        $section->addText('Trên cơ sở các tài liệu do ' . htmlspecialchars($certificate->petitioner_name) . ' cung cấp, với phương pháp thẩm định giá như trên được áp dụng trong tính toán, ' . htmlspecialchars($this->companyName) . ' thông báo kết quả thẩm định giá như sau:', null, $this->indentFistLine);
        if ($this->isApartment) {
            ...
            if ($onlyLandAsset) {
                ...
                if (isset($appraise->tangibleAssets) && count($appraise->tangibleAssets)) {
                    ...
                    */
                    ...
                    foreach ($appraise->tangibleAssets as $index => $tangibleAsset) {
                        ...
                        $table->addCell(1000, $this->cellVCentered)->addText(CommonService::mbUcfirst(htmlspecialchars($building_type)), null, ['align' => 'left']);
                ...
                if (isset($appraise->otherAssets) && count($appraise->otherAssets)) {
                    ...
                    foreach ($appraise->otherAssets as $index => $otherAsset) {
                        $table->addRow(400, $this->cantSplit);
                        $table->addCell(1000, $this->cellVCentered)->addText(htmlspecialchars($otherAsset->name), null, ['align' => 'left']);
    ...
    protected function step9Apartment(Section $section)
    {
        ...
        foreach ($this->realEstates as $realEstate) {
            ...
            $totalAmnt += $total;
            $table->addRow(400, $this->rowHeader);
            $table->addCell(5000, $this->cellVCentered)->addText(htmlspecialchars($name), null, ['align' => 'center', 'keepNext' => true]);
    ...
    protected function step10(Section $section, $certificate)
    {
        ...
        $section->addTitle('NHỮNG HẠN CHẾ VÀ LOẠI TRỪ CỦA THẨM ĐỊNH GIÁ:', 1);
        $section->addTitle('Khuyến nghị về việc sử dụng hồ sơ thẩm định giá: ', 2);
        $section->addListItem('Không có.', 0, null, 'bullets', $this->indentFistLine);
        $section->addTitle('Những hạn chế của hồ sơ thẩm định giá: ', 2);
        $section->addListItem('Bên yêu cầu thẩm định giá chịu trách nhiệm về tính chính xác, hợp pháp của các hồ sơ pháp lý, thông tin cung cấp cho Công ty TNHH Thẩm định giá Nova. Bên yêu cầu thẩm định giá trực tiếp hoặc ủy quyền hợp pháp cho người có đủ hiểu biết về tài sản thẩm định giá hướng dẫn chuyên viên của công ty TNHH Thẩm định giá Nova thực hiện khảo sát, thu thập thông tin về tài sản thẩm định giá. Công ty TNHH Thẩm định giá Nova không đối chiếu hồ sơ pháp lý khách hàng cung cấp với bản chính và không chịu trách nhiệm về chứng thư thẩm định giá trong trường hợp khách hàng cung cấp thông tin sai lệch dẫn tới sai lệch trong kết quả thẩm định giá.', 0, null, 'bullets', $this->indentFistLine);
        ...
        $section->addListItem('Tại thời điểm thẩm định giá, tổ thẩm định giá Công ty TNHH Thẩm định giá Nova đã nỗ lực thu thập thông tin giao dịch thành công tại khu vực thẩm định giá. Tuy nhiên, do đặc điểm thị trường, chất lượng thông tin của các giao dịch thành công có độ chính xác thấp, giá giao dịch ghi trong hợp đồng cách biệt tương đối lớn so với giá giao dịch thực tế. Nên tổ thẩm định giá không lựa chọn được tài sản đã giao dịch thành công làm tài sản so sánh.', 0, ['color' => 'FF0000', 'bgColor' => 'FFFF00' ], 'bullets', $this->indentFistLine);
        $section->addListItem('Chứng thư thẩm định giá, báo cáo thẩm định giá chỉ có giá trị sử dụng trong thời gian hiệu lực theo đúng mục đích thẩm định giá gắn với đúng thông tin tài sản, số lượng tài sản tại Hợp đồng thẩm định giá. Bên yêu cầu hoặc bên thứ ba sử dụng kết quả thẩm định giá có trách nhiệm đối chiếu số lượng, đặc điểm tài sản đã mô tả trong báo cáo thẩm định giá với tài sản yêu cầu thẩm định giá trước khi sử dụng chứng thư thẩm định giá. Trường hợp có sự sai khác về số lượng, đặc điểm tài sản thì khách hàng hoặc bên thứ ba sử dụng kết quả thẩm định giá không được sử dụng chứng thư thẩm định giá và thông báo cho Công ty TNHH Thẩm định giá Nova phối hợp giải quyết.', 0, null, 'bullets', $this->indentFistLine);
        $section->addListItem('Chứng thư thẩm định giá, báo cáo thẩm định giá và các phụ lục kèm theo là những phần không thể tách rời trong quá trình sử dụng kết quả tư vấn thẩm định giá. Chứng thư thẩm định giá chỉ có giá trị sử dụng với bản chính, theo số lượng phát hành ghi trong chứng thư.', 0, null, 'bullets', $this->indentFistLine);
        $section->addListItem('Kết quả thẩm định giá là mức giá Công ty TNHH Thẩm định giá Nova tư vấn giá trị tài sản, được sử dụng làm một trong những căn cứ để Bên yêu cầu hoặc tổ chức, cá nhân có liên quan được ghi tại Hợp đồng thẩm định giá tham khảo, xem xét, phê duyệt giá của tài sản. Chứng thư thẩm định giá, báo cáo thẩm định giá không phải là văn bản bắt buộc bất cứ bên nào tham gia giao dịch phải thực hiện theo giá trị tư vấn.', 0, null, 'bullets', $this->indentFistLine);
        $section->addTitle('Những điều khoản loại trừ trách nhiệm đối với hồ sơ thẩm định giá ', 2);
        $section->addListItem('Công ty TNHH Thẩm định giá Nova sẽ không chịu bất kỳ trách nhiệm nào đối với kết quả thẩm định giá, chứng thư thẩm định giá, báo cáo thẩm định giá nếu Bên yêu cầu thẩm định giá hoặc bên thứ ba sử dụng kết quả thẩm định giá có tên trong Hợp đồng thẩm định giá vi phạm một trong các hành vi sau đây: ', 0, null, 'bullets', $this->indentFistLine);
        $section->addText('         + Cố ý cung cấp thông tin sai lệch về tài sản thẩm định giá;');
        $section->addText('         + Sử dụng chứng thư thẩm định giá đã hết hiệu lực; sử dụng chứng thư thẩm định giá không theo đúng mục đích thẩm định giá gắn với đặc điểm, số lượng tài sản thẩm định giá ghi tại hợp đồng thẩm định giá;');
        $section->addText('         + Mua chuộc, hối lộ; câu kết, thỏa thuận để làm sai lệch giá trị tài sản thẩm định giá nhằm vụ lợi, trục lợi; thông đồng về giá, thẩm định giá.');
        $section->addListItem('Kết quả thẩm định giá được giới hạn trong điều kiện đơn vị thẩm định giá là đơn vị tư vấn, không phải là chủ tài sản, không trực tiếp là đơn vị tổ chức bán đấu giá. Chủ tài sản và đơn vị tổ chức cuộc bán đấu giá cần có kế hoạch truyền thông thu hút khách hàng tham gia cuộc bán đấu giá tài sản, đảm bảo tính hiệu quả cao nhất có thể khi đưa tài sản ra bán đấu giá. Trong mọi trường hợp, đơn vị tư vấn không có trách nhiệm đối với những vi phạm pháp luật của khách hàng cũng như những thiệt hại khác xảy ra trong quá trình tổ chức bán đấu giá tài sản (nếu có).', 0, null, 'bullets', $this->indentFistLine);
        $section->addListItem('Kết quả thẩm định giá trên chỉ có giá trị khi các bên tham gia tuân thủ và hoàn thành các điều khoản trong hợp đồng cung cấp dịch vụ thẩm định giá. Trong trường hợp khách hàng không thực hiện đầy đủ các nghĩa vụ được ghi trong Hợp đồng cung cấp dịch vụ thẩm định giá đã ký kết với  Công ty TNHH Thẩm định giá Nova thì Hợp đồng trên mặc nhiên vô hiệu; Chứng thư thẩm định giá, Báo cáo thẩm định giá kèm theo Hợp đồng trên sẽ không có giá trị pháp lý.', 0, null, 'bullets', $this->indentFistLine);
        $section->addListItem('Người sử dụng chứng thư thẩm định giá hợp pháp chỉ là Bên yêu cầu thẩm định giá hoặc bên thứ ba sử dụng kết quả thẩm định giá được thể hiện trong Văn bản yêu cầu thẩm định giá hoặc Hợp đồng thẩm định giá. Các chủ thể khác sử dụng chứng thư thẩm định giá khi chưa có sự đồng ý bằng văn bản của Công ty TNHH Thẩm định giá Nova là hành vi không hợp pháp, kết quả thẩm định giá, chứng thư thẩm định giá, báo cáo thẩm định giá sẽ không có hiệu lực.', 0, null, 'bullets', $this->indentFistLine);
    ...
    protected function step10HN(Section $section, $certificate)
    {
        // $section->addTitle('NHỮNG ĐIỀU KHOẢN LOẠI TRỪ VÀ HẠN CHẾ CỦA KẾT QUẢ THẨM ĐỊNH GIÁ:', 1);
        $section->addTitle('NHỮNG HẠN CHẾ VÀ LOẠI TRỪ CỦA THẨM ĐỊNH GIÁ:', 1);
        $section->addTitle('Khuyến nghị về việc sử dụng hồ sơ thẩm định giá: ', 2);
        $section->addListItem('Không có.', 0, null, 'bullets', $this->indentFistLine);
        $section->addTitle('Những hạn chế của hồ sơ thẩm định giá: ', 2);
        $section->addListItem('Bên yêu cầu thẩm định giá chịu trách nhiệm về tính chính xác, hợp pháp của các hồ sơ pháp lý, thông tin cung cấp cho Công ty TNHH Thẩm định giá Nova - Chi nhánh Hà Nội. Bên yêu cầu thẩm định giá trực tiếp hoặc ủy quyền hợp pháp cho người có đủ hiểu biết về tài sản thẩm định giá hướng dẫn chuyên viên của công ty TNHH Thẩm định giá Nova - Chi nhánh Hà Nội thực hiện khảo sát, thu thập thông tin về tài sản thẩm định giá. Công ty TNHH Thẩm định giá Nova - Chi nhánh Hà Nội không đối chiếu hồ sơ pháp lý khách hàng cung cấp với bản chính và không chịu trách nhiệm về chứng thư thẩm định giá trong trường hợp khách hàng cung cấp thông tin sai lệch dẫn tới sai lệch trong kết quả thẩm định giá.', 0, null, 'bullets', $this->indentFistLine);
        ...
        $section->addListItem('Tại thời điểm thẩm định giá, tổ thẩm định giá Công ty TNHH Thẩm định giá Nova - Chi nhánh Hà Nội đã nỗ lực thu thập thông tin giao dịch thành công tại khu vực thẩm định giá. Tuy nhiên, do đặc điểm thị trường, chất lượng thông tin của các giao dịch thành công có độ chính xác thấp, giá giao dịch ghi trong hợp đồng cách biệt tương đối lớn so với giá giao dịch thực tế. Nên tổ thẩm định giá không lựa chọn được tài sản đã giao dịch thành công làm tài sản so sánh.', 0, ['color' => 'FF0000', 'bgColor' => 'FFFF00' ], 'bullets', $this->indentFistLine);
        $section->addListItem('Chứng thư thẩm định giá, báo cáo thẩm định giá chỉ có giá trị sử dụng trong thời gian hiệu lực theo đúng mục đích thẩm định giá gắn với đúng thông tin tài sản, số lượng tài sản tại Hợp đồng thẩm định giá. Bên yêu cầu hoặc bên thứ ba sử dụng kết quả thẩm định giá có trách nhiệm đối chiếu số lượng, đặc điểm tài sản đã mô tả trong báo cáo thẩm định giá với tài sản yêu cầu thẩm định giá trước khi sử dụng chứng thư thẩm định giá. Trường hợp có sự sai khác về số lượng, đặc điểm tài sản thì khách hàng hoặc bên thứ ba sử dụng kết quả thẩm định giá không được sử dụng chứng thư thẩm định giá và thông báo cho Công ty TNHH Thẩm định giá Nova - Chi nhánh Hà Nội phối hợp giải quyết.', 0, null, 'bullets', $this->indentFistLine);
        $section->addListItem('Chứng thư thẩm định giá, báo cáo thẩm định giá và các phụ lục kèm theo là những phần không thể tách rời trong quá trình sử dụng kết quả tư vấn thẩm định giá. Chứng thư thẩm định giá chỉ có giá trị sử dụng với bản chính, theo số lượng phát hành ghi trong chứng thư.', 0, null, 'bullets', $this->indentFistLine);
        $section->addListItem('Kết quả thẩm định giá là mức giá Công ty TNHH Thẩm định giá Nova - Chi nhánh Hà Nội tư vấn giá trị tài sản, được sử dụng làm một trong những căn cứ để Bên yêu cầu hoặc tổ chức, cá nhân có liên quan được ghi tại Hợp đồng thẩm định giá tham khảo, xem xét, phê duyệt giá của tài sản. Chứng thư thẩm định giá, báo cáo thẩm định giá không phải là văn bản bắt buộc bất cứ bên nào tham gia giao dịch phải thực hiện theo giá trị tư vấn.', 0, null, 'bullets', $this->indentFistLine);
        $section->addTitle('Những điều khoản loại trừ trách nhiệm đối với hồ sơ thẩm định giá ', 2);
        $section->addListItem('Công ty TNHH Thẩm định giá Nova - Chi nhánh Hà Nội sẽ không chịu bất kỳ trách nhiệm nào đối với kết quả thẩm định giá, chứng thư thẩm định giá, báo cáo thẩm định giá nếu Bên yêu cầu thẩm định giá hoặc bên thứ ba sử dụng kết quả thẩm định giá có tên trong Hợp đồng thẩm định giá vi phạm một trong các hành vi sau đây: ', 0, null, 'bullets', $this->indentFistLine);
        $section->addText('         + Cố ý cung cấp thông tin sai lệch về tài sản thẩm định giá;');
        $section->addText('         + Sử dụng chứng thư thẩm định giá đã hết hiệu lực; sử dụng chứng thư thẩm định giá không theo đúng mục đích thẩm định giá gắn với đặc điểm, số lượng tài sản thẩm định giá ghi tại hợp đồng thẩm định giá;');
        $section->addText('         + Mua chuộc, hối lộ; câu kết, thỏa thuận để làm sai lệch giá trị tài sản thẩm định giá nhằm vụ lợi, trục lợi; thông đồng về giá, thẩm định giá.');
        $section->addListItem('Kết quả thẩm định giá được giới hạn trong điều kiện đơn vị thẩm định giá là đơn vị tư vấn, không phải là chủ tài sản, không trực tiếp là đơn vị tổ chức bán đấu giá. Chủ tài sản và đơn vị tổ chức cuộc bán đấu giá cần có kế hoạch truyền thông thu hút khách hàng tham gia cuộc bán đấu giá tài sản, đảm bảo tính hiệu quả cao nhất có thể khi đưa tài sản ra bán đấu giá. Trong mọi trường hợp, đơn vị tư vấn không có trách nhiệm đối với những vi phạm pháp luật của khách hàng cũng như những thiệt hại khác xảy ra trong quá trình tổ chức bán đấu giá tài sản (nếu có).', 0, null, 'bullets', $this->indentFistLine);
        $section->addListItem('Kết quả thẩm định giá trên chỉ có giá trị khi các bên tham gia tuân thủ và hoàn thành các điều khoản trong hợp đồng cung cấp dịch vụ thẩm định giá. Trong trường hợp khách hàng không thực hiện đầy đủ các nghĩa vụ được ghi trong Hợp đồng cung cấp dịch vụ thẩm định giá đã ký kết với  Công ty TNHH Thẩm định giá Nova - Chi nhánh Hà Nội thì Hợp đồng trên mặc nhiên vô hiệu; Chứng thư thẩm định giá, Báo cáo thẩm định giá kèm theo Hợp đồng trên sẽ không có giá trị pháp lý.', 0, null, 'bullets', $this->indentFistLine);
        $section->addListItem('Người sử dụng chứng thư thẩm định giá hợp pháp chỉ là Bên yêu cầu thẩm định giá hoặc bên thứ ba sử dụng kết quả thẩm định giá được thể hiện trong Văn bản yêu cầu thẩm định giá hoặc Hợp đồng thẩm định giá. Các chủ thể khác sử dụng chứng thư thẩm định giá khi chưa có sự đồng ý bằng văn bản của Công ty TNHH Thẩm định giá Nova - Chi nhánh Hà Nội là hành vi không hợp pháp, kết quả thẩm định giá, chứng thư thẩm định giá, báo cáo thẩm định giá sẽ không có hiệu lực.', 0, null, 'bullets', $this->indentFistLine);
    }
    //X
