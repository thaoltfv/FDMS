...
export default class Certificate extends Model {
	...
	static async getAppraiseLaws() {
		...
		if (isEmpty(resp)) {
			resp = await (new this()).request({ method: 'GET', url: `appraise-laws`, isStatic: true })
	...
	static async getAppraiseOthers() {
		...
		if (isEmpty(resp)) {
			resp = await (new this()).request({ method: 'GET', url: `appraise-others`, isStatic: true })
	...
	static async getAppraiseConstructions() {
		...
		if (isEmpty(resp)) {
			resp = await (new this()).request({ method: 'GET', url: `appraisal-constructions`, isStatic: true })
			localStorage.setItem('appraisal-constructions', JSON.stringify(resp))
		} else resp = JSON.parse(resp)
		return resp
	}
	static async getCertificate(id) {
		return (new this()).request({ method: 'GET', url: `certificate/${id}`, isStatic: true })
	}
	static async getPrint(id) {
		return (new this()).request({ method: 'GET', url: `certificate-asset/print/phu-luc-1/${id}`, isStatic: true })
	}
	static async getPrintAppendix(id) {
		return (new this()).request({ method: 'GET', url: `certificate-asset/print/phu-luc-2/${id}`, isStatic: true })
	}
	static async getPrintImage(id) {
		return (new this()).request({ method: 'GET', url: `certificate-asset/print/phu-luc-hinh-anh/${id}`, isStatic: true })
	}
	static async getPrintReport(id) {
		return (new this()).request({ method: 'GET', url: `certificate-asset/print/bao-cao/${id}`, isStatic: true })
	}
	static async getPrintProof(id) {
		return (new this()).request({ method: 'GET', url: `certificate-asset/print/chung-thu/${id}`, isStatic: true })
	}
	static async getPrintGYC(id) {
		return (new this()).request({ method: 'GET', url: `certificate-asset/print/giay-yeu-cau/${id}`, isStatic: true })
	}
	static async postData(payloadData) {
		return (new this()).request({ method: 'POST', url: `appraises/comparison-factor`, data: payloadData, isStatic: true })
	}
	static async postDataSummarizationReport(id, round_appraise_total) {
		const dataSend = {
			round_appraise_total: +round_appraise_total
		}
		return (new this()).request({ method: 'POST', url: `appraises/bao-cao/edit/${id}`, data: dataSend, isStatic: true })
	}
	static async postDataSummarize(id, round_appraise_total) {
		const dataSend = {
			round_appraise_total: +round_appraise_total
		}
		return (new this()).request({ method: 'POST', url: `certification_asset/step7-round-appraise-total/${id}`, data: dataSend, isStatic: true })
	}
	static async postDataCertificate(data, other_data, delete_data, price_UBND) {
		...
		return (new this()).request({ method: 'POST', url: `certificate-asset/comparison-factor`, data: comparison_factor, isStatic: true })
	}
	// TSTD
	static async postDataComparison(data, dataConstruction) {
		const comparison_tangible_factor = { comparison_tangible_factor: data, construction_company: dataConstruction }
	...
	static async postDataComparisonCert(data, dataConstruction) {
		const comparison_tangible_factor = { comparison_tangible_factor: data, construction_company: dataConstruction }
		return (new this()).request({ method: 'POST', url: `certificate-asset/tangible-comparison`, data: comparison_tangible_factor, isStatic: true })
	}
	static async getDataComparison(id) {
		return (new this()).request({ method: 'GET', url: `appraises/comparison-factor/${id}`, isStatic: true })
	}
	static async getDataComparisonCertificateAsset(id) {
		return (new this()).request({ method: 'GET', url: `certificate-asset/comparison-factor/${id}`, isStatic: true })
	}
	static async downloadDocument(id) {
		return (new this()).request({ method: 'GET', url: `certification_brief/download-document/${id}`, isStatic: true })
	}
	static async deleteDocument(id) {
		return (new this()).request({ method: 'DELETE', url: `certification_brief/delete-document/${id}`, isStatic: true })
	}
}
