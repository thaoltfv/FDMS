<template>
	<div
		v-if="!isMobile()"
		class="navbar"
		:class="toggleItem ? 'sidebar-mini' : 'sidebar'"
	>
		<aside
			:class="[toggleItem ? navbar_mini : '']"
			class="navbar navbar-vertical d-flex flex-column navbar-expand-lg h-100"
		>
			<button
			...
			<div
				class="d-flex w-100 navbar-brand align-items-center justify-content-center"
			>
			...
			<div class="collapse navbar-collapse" id="navbar-menu">
				<ul class="navbar-nav">
					...
					<li
						...
						class="nav-item nav-user dropdown"
					>
						<svg
							width="11"
							height="6"
						...
						<div class="nav-link">
							<div class="nav-contain-wrapper">
								...
								<span class="name mb-0 d-flex flex-column nav-link-title"
									>{{ currentUser ? currentUser.name : ""
									}}<span class="mt-1">{{
										currentUser ? currentUser.roles[0].role_name : ""
									}}</span></span
								>
							</div>
						</div>
						<ul class="dropdown-item">
							...
							<li class="nav-link-title nav-dropdown" @click="logout()">
								<div class="nav-link">Đăng xuất</div>
							</li>
						</ul>
					</li>
				</ul>
			...
			<div
				class="toggle d-flex justify-content-end"
				:class="[toggleItem ? toggle_mini : '']"
			>
...
export default {
	...
	methods: {
		...
		dropdownClick(e) {
			...
			const isSubMenu = e.target.parentElement.classList.contains(
				"nav-dropdown"
			);

			[].forEach.call(
				document.querySelectorAll("#navbar-menu li.nav-item"),
				function(el) {
					const countActiveSubMenu = el.querySelectorAll(
