				...
				<div class="card">
					<div class="contain-detail table-wrapper">
						<a-table
							...
							:data-source="listPriceEstimates"
							:loading="isLoading"
							:row-selection="{
						...
						>
							...
							<template slot="id" slot-scope="id">
								<p class="link-detail mb-0">{{ "TSSB_" + id }}</p>
							</template>
						</a-table>
					</div>
...
import PriceEstimatesAsset from "@/models/PriceEstimatesAsset";
...
export default {
	...
	async mounted() {
		await this.getListPriceEstimate();
		if (this.data.price_estimates.length > 0) {
			this.data.price_estimates.forEach(item => {
				this.selectedRowKeys.push(item.price_estimate_id);
	...
	methods: {
		async getSelectRows() {
			...
			//   }
			//   const res = await PreCertificate.getAppraisals(ids)
		...
		async getListPriceEstimate(params = {}) {
			this.isLoading = true;
			const filter = {};
			try {
				const resp = await PriceEstimatesAsset.paginate({
					query: {
						page: 1,
						limit: 15,
						status: 2,
						popup: true,
						pre_certificate_id: this.idData,
						...params,
						...filter
					}
				});
				if (resp) {
					this.listPriceEstimates = [...resp.data.data];
					for (let i = 0; i < this.listPriceEstimates.length; i++) {
						const element = this.listPriceEstimates[i];
		...
		async onPageChange(pagination) {
			...
			await this.getListPriceEstimate(params);
		},
		...
		async handleAction(event) {
			...
			await this.selectedRows.forEach(item => {
				dataSend.price_estimates.push(item.id);
			});
			this.isSubmit = true;
			const res = await PreCertificate.updatePriceEstimatesList(
				this.idData,
				dataSend
			);
			if (res.data) {
				this.$toast.open({
					message: "Cập nhật tài sản sơ bộ thành công",
					type: "success",
					position: "top-right"
				});
				this.$emit("updatePriceEstimateList", res.data);
			} else {
				this.$toast.open({
