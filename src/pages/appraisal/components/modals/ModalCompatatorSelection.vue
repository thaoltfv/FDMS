        ...
        <div class="card">
          ...
          <div class="contain-detail">
            <vue-tabs @tab-change="handleChangeTab">
              <v-tab v-for="(dataChild,index) in data" :key="dataChild.id" :title="`TSTD_${dataChild.oldID ? dataChild.oldID : dataChild.id}`">
                <div class="property-detail">
                  <div class="container__table container__table--detail">
                  <a-table bordered
                            :columns="columnDocuments"
                            :data-source="documentData && documentData.length > 0 && documentData[index] ? documentData[index] : documents"
                            :loading="isLoading"
...
export default {
	...
	created () {
		...
		this.data.forEach((data, index) => {
			if (data.table && data.table.length > 0) {
				...
				if (data.oldID) {
					this.dataIdOld.push(data.oldID)
				}
	...
	methods: {
		disableCheckBox (record) {
			return {
				props: {
					disabled: record.name === 'Pháp lý', // Column configuration not to be checked
					name: record.name
				}
			}
		},
		...
		async sortDocument (index) {
			let arrayTemple = []
			await this.documents.forEach(document => {
				const select = this.selectedRowKeys[index].find(selectedKey => selectedKey === document.name)
				if (select) {
					arrayTemple.push(document)
				}
			})
			// await this.documents.forEach(data => {
			//   const select1 = arrayTemple.find(document => document.id === data.id)
			//   if (!select1) {
			//     arrayTemple.push(data)
			//   }
			// })
			await this.documentData.push(arrayTemple)
		},
		format (value) {
		...
		async handleAction (event) {
			...
			let message = ''
			let count = 0
			await this.selectedRowKeys.forEach((data, index) => {
				if (data.length < 1) {
					check = false
					message = 'Vui lòng chọn yếu tố so sánh'
				}
				if (data.length > 0) {
					for (let i = 0; i < data.length; i++) {
						...
						if (count < this.selectedRowKeys.length) {
							check = false
							message = 'Yếu tố pháp lý là bắt buộc'
						} else {
							check = true
						}
					}
			...
			if (check) {
				this.$emit('cancel', event)
				this.$emit('action', this.selectedRowKeys, this.dataID, this.dataIdOld)
			} else {
				this.$toast.open({
					message: message,
					type: 'error',
