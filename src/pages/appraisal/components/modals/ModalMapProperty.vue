    ...
    <ValidationObserver tag="form"
      ...
      <div class="card">
        <div class="d-block d-sm-flex justify-content-between my-3">
          <button class="btn btn-search w-100" type="button" @click="handleSubmit">Cập nhật</button>
...
export default {
	name: 'ModalMap',
	props: ['data', 'location', 'address', 'center_map', 'radius', 'frontSide', 'landType', 'propertyTypes'],
	components: {
		LMap,
		LControlZoom,
		LTileLayer,
	...
	methods: {
		...
		updateAsset () {
			...
			this.locationArray.forEach((location, index) => {
		...
		async handleSubmit () {
			...
			if (this.locationArrayPopup.length > 3) {
				this.$toast.open({
		...
		async getAssetGenerals () {
			...
			const resp = await AppraiseData.getSearchAll(distance, location, frontSide, transaction, assetType)
