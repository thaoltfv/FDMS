  ...
  <div class="card">
    ...
    <div class="card-body card-info" v-show="showCard">
      <div class="container-content__detail row align-items-center mb-0">
        <div class="container-content__title col-3 mb-0">Chọn yếu tố so sánh</div>
        <div class="col d-flex align-items-center">
          <button type="button" class="btn btn-orange" @click="handleOpenModalExpertise">Xem</button>
          <div class="text-danger">
            Đã chọn {{comparison_factor.length}} tài sản
          </div>
        </div>
      </div>
    </div>
    <ModalCompatatorSelection
...
import ModalCompatatorSelection from '@/pages/appraisal/components/modals/ModalCompatatorSelection'
export default {
	name: 'ConstructionUnit',
	props: ['comparison', 'comparison_factor'],
	data () {
		return {
			showCard: false,
			title: '',
	...
	methods: {
		saveData (event, id, oldId) {
			this.$emit('saveComparison', event, id, oldId)
		...
		handleOpenModalExpertise () {
			this.title = 'Lựa chọn yếu tố'
			this.documents = []
			this.data = this.comparison_factor
