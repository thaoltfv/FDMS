<template>
  <div class="card">
    ...
    <div class="card-body card-info" v-show="showCard">
      <div class="row">
        <!-- <div class="col-12 col-lg-3 mt-2" v-for="(asset, index) in data" :key="index">
          <div class="container__property">
            ...
            <div v-for="(landTypePrice, index) in asset.land_type_purpose_price" :key="'landTypePrice' + index">
              <div class="property__detail d-flex justify-content-between" v-for="(land_type, index) in landTypePurposes" :key="'land_type' + index" v-if="landTypePrice.id === land_type.id">
        ...
        <div class="col-4 d-flex" v-for="data in data" :key="data.id" >
          <button class="btn btn-orange" style="width: auto" type="button" @click="handleMap(data)">Sơ đồ vị trí tài sản thẩm định TSTD_{{data.appraise_id ? data.appraise_id : data.id}} </button>
        </div>
    </div>
...
export default {
	...
	props: ['data', 'landTypePurposes', 'radius', 'location', 'appraisers', 'formData', 'propertyTypes', 'assets'],
	computed: {
		optionsAppraiser () {
			return {
	...
	methods: {
		...
		saveData (id) {
			this.$emit('savePropertyId', id, this.index)
		},
