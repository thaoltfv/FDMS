            ...
            <div class="property-detail">
              <div class="container__table container__table--detail">
                <a-table bordered
                         ...
                         :loading="isLoading"
                         :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange, getCheckboxProps: disableCheckBox }"
...
export default {
	...
	mounted () {
		...
		this.data.forEach(id => {
			this.selectedRowKeys.push(id.comparison_factor)
			this.getDataRow(id)
		})
		// let i = 0
		// this.data.forEach(id => {
		//   i++
		//   this.selectedRowKeys.push({id:i,name:id.comparison_factor})
		//   this.selectedRows.push({id:i,name:id.comparison_factor})
		//   // this.getDataRow(id)
		// })
		this.sortDocument()
	},
	...
	methods: {
		...
		getDataRow (id) {
			this.documents.forEach(row => {
				if (id && id.comparison_factor === row.name) {
					const select = this.documents.find(document => document.name === row.name)
		...
		sortDocument () {
			...
			//   const select = this.documents.find(document => document.name === selectedKey)
			//   if (select) {
			//     this.documentData.push(select)
			//   }
			// })
			this.documents.forEach(item => {
				const select = this.selectedRowKeys.find(select => select === item.name)
				if (select) {
