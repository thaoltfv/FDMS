  ...
  <div>
      <div
        ...
        <div class="card">
          ...
          <div class="card-table">
            ...
            <div class="card-body card-info card-land" v-if="show_description">
                ...
                <div class="property-content row" v-if="property.asset_type_id === 39">
                  ...
                  <p class="property-detail col" v-if="property.apartment !== undefined && property.apartment !== null" >{{property.apartment.name}}</p>
                </div>
                <div class="property-content row" v-if="property.asset_type_id !== 39">
          ...
          <div class="card-table" v-if="property.asset_type_id === 39">
            ...
            <div class="card-body card-info card-land" v-if="show_info_apartment">
              <div class="property-content row">
                ...
                <p class="property-detail col"> {{property.block_specification[0].block_lists !== undefined && property.block_specification[0].block_lists !== null ? property.block_specification[0].block_lists.name : 'Chưa có thông tin'}}</p>
              </div>
              <div class="property-content row">
                <p class="property-title col-4">Mã căn hộ:</p>
                <p class="property-detail col"> {{property.room_details[0].room_num}}</p>
              </div>
              <div class="property-content row">
                <p class="property-title col-4">Tầng:</p>
                <p class="property-detail col"> {{property.block_specification[0].total_floor}}</p>
              </div>
              <div class="property-content row">
          ...
          <div class="card-table" v-if="property.asset_type_id !== 39">
            ...
            <div class="card-body card-info card-land" v-if="show_info_land">
              <div v-for="properties in property.properties" :key="properties.id">
                <div>
                  <div class="property-content row align-items-center">
                    <label class="property-title col-4">Chọn thửa đất</label>
                    <label class="container-radio ml-2">
                      <input class="property-detail" type="radio" name="property" :disabled="$route.name === 'certificate.detail'" :checked="properties.id === propertyIndex" :value="properties.id" @change="changeInfo($event)">
          ...
          <div class="container__print" v-if="$route.name !== 'certificate.detail'">
            <button class="btn btn-orange btn-orange--print" type="button" @click="handleAction">Lưu</button>
          </div>
      </div>
    </div>
  </div>
...
export default {
	name: 'ModalMapDetail',
	props: ['property', 'pic', 'propertyIndex'],
	data () {
		return {
			printDetail: '',
			openPrint: false,
	...
	mounted () {
		this.findFrontSide()
		this.propertyId = this.propertyIndex
	},
	components: {
		InputRadio
	},
	methods: {
		...
		handleAction () {
			this.$emit('save', this.propertyId, this.property)
			this.$emit('cancel')
		},
