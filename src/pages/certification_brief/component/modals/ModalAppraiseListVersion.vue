	...
	<div>
		<ValidationObserver tag="form" ref="observer">
			<div
				...
				<div class="card">
					<div class="">
						<h3>Danh sách TSTĐ cần cập nhật</h3>
					</div>
					<div class="contain-detail table-wrapper">
						...
						>
							<template slot="asset" slot-scope="asset">
								<p :id="asset.id" class="full-address mb-0"> {{asset.name}}</p>
							...
							<template slot="last_version" slot-scope="last_version">
								<p class="text-none mb-0">{{last_version}}</p>
							</template>
							<template slot="area" slot-scope="area">
								<p class="text-none mb-0">{{ area ? formatNumber(formatArea(area)) : 0 }} m <sup>2</sup> </p>
							</template>
					...
					<div class="container-title container-title__footer">
						<div class="d-flex justify-content-between justify-content-lg-end">
							<button class="btn btn-white btn-action-modal" type="button" @click="handleCancel"><img src="../../../../assets/icons/ic_cancel.svg"  class="mr-1" alt="save">Trở lại</button>
							<button class="btn btn-orange btn-action-modal" type="button" @click="handleAction" :class="{'btn-loading disabled': isSubmit}"> <img src="../../../../assets/icons/ic_save.svg" class="mr-1" alt="save">Lưu</button>
						</div>
					</div>
				</div>
			</div>
		...
		columnAssets () {
			let dataColumn = [
				...
				},
				{
					title: 'Version',
					align: 'center',
					scopedSlots: {customRender: 'last_version'},
	...
	methods: {
		...
		async handleAction (event) {
			...
			await this.selectedRows.forEach(item => {
				...
				dataSend.general_asset.push({general_asset_id: itemId, asset_type_id: item.asset_type_id, real_estate_id: item.real_estate_id})
			})
			this.isSubmit = true
			const res = await CertificationBrief.updateAppraiseCertificateVersion(this.idData, dataSend)
			if (res.data) {
				this.$toast.open({
				...
				this.$emit('updateAppraiseVersion', res.data)
			} else {
				this.$toast.open({
