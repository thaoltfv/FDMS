...
export default {
	...
	methods: {
		...
		async getListAppraise (params = {}) {
			...
			const filter = {}
			try {
				const resp = await AppraiseAsset.paginate({ query: {page: 1, limit: 15, status: 2, popup: true, certificate_id: this.idData, ...params, ...filter } })
				if (resp) {
		...
		async handleAction (event) {
			...
			await this.selectedRows.forEach(item => {
				...
				dataSend.general_asset.push({general_asset_id: itemId, asset_type_id: item.asset_type_id})
			})
			this.isSubmit = true
			const res = await CertificationBrief.updateAppraiseCertificate(this.idData, dataSend)
			if (res.data) {
				this.$toast.open({
