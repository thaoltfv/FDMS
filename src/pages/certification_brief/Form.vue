...
export default {
	...
	methods: {
		...
		async getAppraisers() {
			...
			this.form.step_1.appraiser_manager_id = await this.appraisersManager[0]
				.id;
			...
			if (this.form && this.form.step_1.appraiser_id) {
				const filterData = appraiser.filter(
					item =>
						item.id !== this.form.step_1.appraiser_id &&
						item.id !== this.form.step_1.appraiser_manager_id
				);
		...
		async handleSubmitStep_1(dataStep1, id = "") {
			this.isSubmit = true;
			const res = await CertificationBrief.submitStep1CertificationBrief(
				dataStep1,
				id
			);
			if (res.data) {
