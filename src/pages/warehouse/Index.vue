<template>
	...
	>
		...
		<div class="pannel">
			<div
				class="pannel__content row mx-0 justify-content-between justify-content-lg-end align-items-center"
			>
				...
				<div class="justify-content-between align-items-center btn-container">
					<button
						class="btn btn-white text-nowrap index-screen-button  d-none d-lg-inline-flex"
					...
					<button
						class="btn btn-white text-nowrap index-screen-button ml-2"
						@click.prevent="handleCancelSearch()"
						v-if="cancelSearch"
					>
						<img
							src="@/assets/icons/ic_search.svg"
            ...
            src="@/assets/icons/ic_add.svg" style="margin-right: 8px" alt="search">Tạo mới
					...
					<button
						class="btn mr-0 btn-white text-nowrap index-screen-button ml-2"
		...
		>
			...
			>
						...
						<b-dropdown class="dropdown-container" no-caret>
							...
							<b-dropdown-item @click="onShowImages(id, property.full_address)">
								<div class="dropdown-item-container">
									<img src="@/assets/icons/ic_img.svg" alt="img" />Xem hình ảnh
								</div>
							</b-dropdown-item>
							<b-dropdown-item @click.prevent="openPrint(id)">
								<div class="dropdown-item-container">
									<img src="@/assets/icons/ic_download.svg" alt="print" />Tải
									xuống
								</div>
							</b-dropdown-item>
							<b-dropdown-item @click.prevent="openPrintPdf(id)">
								<div class="dropdown-item-container">
									<img src="@/assets/icons/ic_printer.svg" alt="print" />In
								</div>
							</b-dropdown-item>
							<b-dropdown-item @click.prevent="disableClick(id)">
								<div class="dropdown-item-container">
									<img src="@/assets/icons/ic_lock.svg" alt="" />Vô hiệu hóa
								</div>
							</b-dropdown-item>
							<b-dropdown-item @click.prevent="enableClick(id)">
								<div class="dropdown-item-container" v-if="activeStatus">
									<img src="@/assets/icons/ic_unlock.svg" alt="" />Hủy vô hiệu
									hóa
								</div>
							</b-dropdown-item>
						</b-dropdown>
				...
				<template slot="status" slot-scope="status">
					<p
						class="status"
						:class="status === 2 ? 'red' : status === 3 ? 'orange' : ''"
					>
						{{
							status === 1
								? "Đang kích hoạt"
				...
				>
					<p class="total_construction_area mb-0">
						{{ formatNumber(total_construction_area) }} m<sup>2</sup>
					</p>
				</template>
				...
				<template
					slot="average_land_unit_price"
					slot-scope="average_land_unit_price"
				>
					<p class="average_land_unit_price mb-0">
						{{ formatPrice(average_land_unit_price) }}
					</p>
				</template>
				<template slot="transaction_type" slot-scope="transaction_type">
					<p class="transaction_type mb-0">
	...
	<div v-else id="wareHouse" class="container-fluid position-relative">
		...
		<div class="pannel">
			...
			>
				<div
					class="search-block col-6 col-md-6 col-xl-4 d-flex justify-content-end align-items-center"
					style="padding: 0;"
				>
					<Search @filter-changed="onFilterChange($event)" class="search-box" />
				</div>
    ...
    margin-top: -10px;"
		...
		>
			<b-card
				...
				v-for="element in listWarehouses"
			...
			>
						...
						<div class="d-flex">
							<span style="font-weight: 500"
								...
								><span :id="element.id + 'all'" class="text-left">{{
									element.full_address.substring(30, 0) + "..."
								}}</span></span
							>
				...
				<div class="property-content mb-2 d-flex color_content">
					<div class="label_container d-flex">
						<div class="d-flex">
							<span style="font-weight: 500"
								><strong class="d_inline mr-1">Loại giao dịch:</strong
								><span class="text-capitalize">{{
									element.transaction_type.description.toLowerCase()
								}}</span></span
							>
						</div>
					</div>
				...
				<div class="property-content mb-2 d-flex color_content">
					<div class="label_container d-flex">
						<div class="d-flex">
							<span style="font-weight: 500"
								><strong class="d_inline mr-1">Đơn giá bình quân(VNĐ):</strong
								><span class="text-none">{{
									element.average_land_unit_price
										? formatPrice(element.average_land_unit_price)
										: "-"
								}}</span></span
							>
						</div>
					...
					<div class="label_container d-flex">
						<div class="d-flex">
							<span style="font-weight: 500"
								...
								><span class="text-none">{{
									element.total_amount ? formatPrice(element.total_amount) : "-"
								}}</span></span
							>
						</div>
					</div>
				...
				<div class="property-content mb-2 d-flex color_content">
					<div class="label_container d-flex">
						<div class="d-flex">
							<span style="font-weight: 500"
								><strong class="d_inline mr-1">Thời điểm đăng:</strong
								><span class="public_date">{{
									formatDate(element.public_date)
								}}</span></span
							>
						</div>
					</div>
				...
				<div class="property-content mb-2 d-flex color_content">
					<div class="label_container d-flex">
						<div class="d-flex">
							<span style="font-weight: 500"
								><strong class="d_inline mr-1">Ngày cập nhật:</strong
								><span class="public_date">{{
...
export default {
	...
	methods: {
		...
		async disableClick(index) {
			...
			this.messageNotificationProperty =
		...
		async enableClick(index) {
			this.indexProperty = index;
			this.messageNotificationProperty =
				"Bạn có chắc muốn hủy vô hiệu hóa  " +
				"<b>" +
				"TSS_" +
				index +
				"</b>" +
		...
		async handleDisable() {
			...
			this.messageNotification =
				"Bạn có chắc muốn vô hiệu hóa với " +
				"<b>" +
				this.arr.length +
				"</b>" +
				" TSSS đã chọn không?";
		...
		async handleActive() {
			...
			this.active = true;
			this.messageNotification =
				"Bạn có chắc muốn kích hoạt lại với " +
				"<b>" +
				this.arr.length +
				"</b>" +
				" TSSS đã chọn không?";
		...
		formatNumber(value) {
			if (value) {
				let formatedNum = parseFloat(value)
					.toString()
					.replace(".", ",");
	...
	beforeRouteEnter: async (to, from, next) => {
		...
		const category = await WareHouse.find(
			to.query[
				("search",
				"asset_type_id",
				"created_at",
