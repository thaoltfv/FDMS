		...
		>
			<div class="position-relative info-container">
					...
					<div class="card-body card-info" v-if="showInfo">
						<div class="container-fluid">
							<div class="row">
								...
								<div
									v-if="form.asset_type_id && form.asset_type_id === 39"
							...
							<div
								...
								class="card-info"
							>
								<h4 class="title" v-if="form.asset_type_id !== 39">
									Vị trí tài sản:
								</h4>
								<div
								...
								>
									<InputCategory
										...
										:disabled="
											'id' in $route.query && $route.name === 'warehouse.edit'
										"
										vid="province_id"
										label="Tỉnh/Thành"
										rules="required"
								...
								>
									...
									/>
									<div
										class="col-12 col-md-12 col-lg-4 form-group-container position-relative"
									>
										<InputText
				...
				<div
					class="card"
					v-if="
						this.form.asset_type_id === 37 || this.form.asset_type_id === 38
					"
				>
					<div class="card-title">
						<div class="d-flex justify-content-between align-items-center">
							...
							<img
								class="img-dropdown"
								:class="!showTable ? 'img-dropdown__hide' : ''"
							...
							<table class="table-property">
								...
								<tbody>
									...
									>
										<td>
											<div
												class="d-flex justify-content-center align-items-center contain-total contain-total__table"
												v-if="$route.name === 'warehouse.edit'"
											>
												<div
													class="num num-id d-flex justify-content-center text-center"
													@click="handleProperty(property, index)"
												>
													<a>TSD_{{ property.id }}</a>
												</div>
											</div>
											<div
												class="d-flex align-items-center  contain-total contain-total__table"
												v-if="$route.name === 'warehouse.create'"
											>
												<div
													class="num num-id d-flex justify-content-center text-center"
													@click="handleProperty(property, index)"
										...
										<td>
											...
											>
												<div class="num">
													<p>
														{{
															property.compare_property_doc[0] !== undefined &&
															property.compare_property_doc[0] !== null
																? property.compare_property_doc[0].doc_num
																: ""
														}}
													</p>
												</div>
												...
												<div class="num">
													<p>
														{{
															property.compare_property_doc[0] !== undefined &&
															property.compare_property_doc[0] !== null
																? property.compare_property_doc[0].plot_num
																: ""
														}}
													</p>
												</div>
										...
										<td>
											...
											>
												<div class="num">
													<p>{{ formatNumber(property.front_side_width) }}</p>
												</div>
											</div>
										</td>
										<td>
											...
											>
												<div class="num">
													<p>{{ formatNumber(property.insight_width) }}</p>
												</div>
											</div>
										</td>
										<td>
				...
				<div class="card" v-if="this.form.asset_type_id === 38">
					<div class="card-title">
						<div class="d-flex justify-content-between align-items-center">
							...
							<img
								class="img-dropdown"
								:class="!showLand ? 'img-dropdown__hide' : ''"
								src="@/assets/images/icon-btn-down.svg"
								alt="dropdown"
								@click="showLand = !showLand"
							/>
						</div>
					</div>
					<div class="card-body card-info card-land" v-if="showLand">
						<div class="contain-table contain-table__tangible">
							<table class="table-property">
								...
								<tbody>
									<tr
										v-for="(tangible, index) in form.tangible_assets"
										:key="'tangible' + index"
									>
										<td>
											<div
											...
											>
												<div
													class="num num-id d-flex justify-content-center text-center"
													@click="openModalTangible(tangible, index)"
												>
													<a>TSN_{{ tangible.id }}</a>
												</div>
											</div>
											<div
												class="d-flex justify-content-center align-items-center contain-total contain-total__table"
											...
											>
												<div
													class="num num-id text-nowrap d-flex justify-content-center text-center"
													@click="openModalTangible(tangible, index)"
												>
													<a>Chỉnh sửa</a>
												</div>
										...
										<td>
											...
											>
												<div class="num">
													<p>
														{{ formatNumber(tangible.total_construction_base) }}
													</p>
												</div>
											</div>
										</td>
										<td>
											...
											>
												<div class="num">
													<p>{{ formatNumber(tangible.unit_price_m2) }}</p>
												</div>
											</div>
										</td>
										<td>
											<div
												class="d-flex align-items-center contain-total contain-total__table"
											>
												<div class="num" @change="changeAmount($event, index)">
													<p>{{ formatNumber(tangible.estimation_value) }}</p>
												</div>
											</div>
										</td>
										<td>
											<div class="contain-file">
												...
												>
													...
													<div
														class="img-contain img-contain__table"
														v-for="images in tangible.pic"
														:key="images.id"
													>
														<div
															class="contain-img contain-img__table"
				...
				<div
					class="card card__order"
					v-if="
						this.form.asset_type_id === 38 || this.form.asset_type_id === 37
					"
				>
					<div class="card-title">
						<div class="d-flex justify-content-between align-items-center">
							...
							<img
								class="img-dropdown"
								:class="!showOther ? 'img-dropdown__hide' : ''"
								src="@/assets/images/icon-btn-down.svg"
								alt="dropdown"
					...
					<div class="card-body card-info card-land" v-if="showOther">
						<div class="contain-table">
							<table class="table-property table-property__order">
								...
								<tbody>
									<tr
										v-for="(other, index) in form.other_assets"
										:key="other.id"
									>
										<td>
											<InputText
										...
										<td>
											<div class="contain-file justify-content-center">
												...
												>
													<div
														class="img-upload ml-0"
														v-if="other.pic.length === 0"
													>
														Tải ảnh lên
														<input
															type="file"
															accept="image/png, image/jpeg, image/gif, image/jpg"
															@change="onImageOtherChange($event, index)"
														/>
													</div>
													<div
														class="img-contain img-contain__table"
														v-for="images in other.pic"
														:key="images.id"
													>
														<div
															class="contain-img contain-img__table"
					...
					<div class="card-title card-title__img">
						...
						>
							...
							<div class="img-upload">
								<img
									src="@/assets/icons/ic_image-white.svg"
								...
								<input
									...
									@change="onImagePropertyChange($event)"
								/>
							</div>
						</form>
					</div>
				...
				<div class="card" v-if="this.form.asset_type_id !== ''">
					<div class="card-title">
						<div class="d-flex justify-content-between align-items-center">
							<h3 class="title">
								Ước tính đơn giá
								{{ this.form.asset_type_id === 39 ? "căn hộ" : "đất" }}
							</h3>
							<img
								class="img-dropdown"
								:class="!showInfoTransaction ? 'img-dropdown__hide' : ''"
								src="@/assets/images/icon-btn-down.svg"
								alt="dropdown"
								@click="showInfoTransaction = !showInfoTransaction"
							/>
						</div>
					</div>
							...
							<div class="card-info">
								<div class="row justify-content-between">
									<div class="col-12 col-md-4 col-lg-3 form-group-container">
										<label class="color-black font-weight-bold"
											>Tổng giá trị (VND)</label
										>
										<div
											class="form-control disabled d-flex justify-content-end"
										>
											<p class="mb-0">
									...
									>
										...
										>
											<p class="mb-0">
												{{
													form.room_details && form.room_details[0].area
														? formatNumber(form.room_details[0].area)
														: 0
												}}
											</p>
											<p class="mb-0 ml-2">m<sup>2</sup></p>
										</div>
									</div>
								...
								<div class="row justify-content-between">
									<div class="col-12 col-md-4 col-lg-3 form-group-container">
										<label class="color-black font-weight-bold"
											>Tổng giá trị
											{{
												this.form.asset_type_id === 39 ? "căn hộ" : "tài sản"
											}}
											sau điều chỉnh (VND)</label
										>
										<div
											class="form-control disabled d-flex justify-content-end"
										>
											<p class="mb-0">
												{{ formatCurrency(form.total_estimate_amount) }}
											</p>
										</div>
									</div>
								...
								>
									<div class="col-12 col-md-4 col-lg-3 form-group-container">
										<label class="color-black font-weight-bold"
											>Đơn giá bình quân căn hộ (đ/m<sup>2</sup>)</label
										>
										<div
											class="form-control disabled d-flex justify-content-end"
										>
											<p class="mb-0">
												{{
													formatCurrency(this.form.room_details[0].unit_price)
												}}
											</p>
										</div>
									</div>
								...
								>
									<div class="col-12 col-md-4 col-lg-3 form-group-container">
										<label class="color-black font-weight-bold"
											>Tổng giá trị đất thuần còn lại (VND)</label
										>
										<div
											class="form-control disabled d-flex justify-content-end"
										>
											<p class="mb-0">
												{{ formatCurrency(form.total_raw_amount) }}
											</p>
										</div>
										<span class="text-error" v-if="form.total_raw_amount < 0"
											>Tổng giá trị đất thuần còn lại không được nhỏ hơn 0</span
										>
									</div>
									<div class="col-12 col-md-4 col-lg-3 form-group-container">
										<label class="color-black font-weight-bold"
											>Tổng giá trị công trình xây dựng (VND)</label
										>
										<div
											class="form-control disabled d-flex justify-content-end"
										>
											<p class="mb-0">
												{{ formatCurrency(form.total_construction_amount) }}
											</p>
										</div>
									</div>
									<div class="col-12 col-md-4 col-lg-4 form-group-container">
										<label class="color-black font-weight-bold"
											>Tổng giá trị tài sản khác (VND)</label
										>
										<div
											class="form-control disabled d-flex justify-content-end"
										>
											<p class="mb-0">
												{{ formatCurrency(form.total_order_amount) }}
											</p>
										</div>
									</div>
								</div>
								<div v-if="this.form.asset_type_id !== 39">
									<div v-if="this.form.convert_fee_total > 0">
										...
										<div class="row justify-content-between">
											<div class="col-12 form-group-container">
												<label class="color-black font-weight-bold"
													>Tổng giá trị chuyển mục đích sử dụng đất (VND)</label
												>
												<div
													class="form-control disabled d-flex justify-content-end"
												>
													<p class="mb-0">
														{{ formatCurrency(form.convert_fee_total) }}
													</p>
												</div>
											</div>
										</div>
										...
										>
											<div
												...
												v-for="property_detail in property.property_detail"
												:key="property_detail.id"
												v-if="property_detail.convert_fee > 0"
											>
												<label class="color-black font-weight-bold"
													>Chi phí chuyển mục đích sử dụng từ
													{{
														property_detail.land_type_purpose_data !==
															undefined &&
														property_detail.land_type_purpose_data !== null
															? property_detail.land_type_purpose_data
																	.description
															: ""
													}}
													sang {{ form.max_value_description }} (VND)</label
												>
												<div
													class="form-control disabled d-flex justify-content-end"
												>
													<p class="mb-0">
														{{ formatCurrency(property_detail.convert_fee) }}
													</p>
												</div>
											</div>
										</div>
									...
									<div v-if="this.form.total_amount > 0">
										...
										<div class="row justify-content-between">
											<div class="col-12 form-group-container">
												<label class="color-black font-weight-bold"
													>Giá trị QSDĐ ước tính (VND)</label
												>
												<div
													class="form-control disabled d-flex justify-content-end"
												>
													<p class="mb-0">
														{{ formatCurrency(form.total_land_unit_price) }}
													</p>
												</div>
												<span
													class="text-error"
													v-if="
														form.total_land_unit_price &&
															form.total_land_unit_price < 0
													"
													>Giá trị QSDĐ ước tính không được nhỏ hơn 0</span
												>
											</div>
										</div>
										<div v-if="sortedArray.length > 0">
											...
											<ul class="row justify-content-between">
												<li
													class="col-12 col-md-6 form-group-container"
													v-for="purpose_use_land in sortedArray"
													:key="purpose_use_land.id"
												>
													<label class="color-black font-weight-bold"
														>Đơn giá đất
														{{
															purpose_use_land.land_type_purpose_data !==
																undefined &&
															purpose_use_land.land_type_purpose_data !== null
																? purpose_use_land.land_type_purpose_data
																		.description
																: ""
														}}
													...
													>
														<p class="mb-0">
															{{ formatCurrency(purpose_use_land.price_land) }}
														</p>
													</div>
													<span
														class="text-error"
														v-if="purpose_use_land.price_land <= 0"
														>Loại đất
														{{
															purpose_use_land.land_type_purpose_data !==
																undefined &&
														...
														hiện có đơn giá bằng 0. Vui lòng kiểm tra lại giá
														trị đầu vào.</span
													>
												</li>
											</ul>
				...
				<div class="card">
					<div class="card-title">
						<div class="d-flex justify-content-between align-items-center">
							...
							<img
								class="img-dropdown"
								:class="!showNote ? 'img-dropdown__hide' : ''"
								src="@/assets/images/icon-btn-down.svg"
								alt="dropdown"
...
export default {
	...
	methods: {
		...
		removeTableRow(index) {
			...
			this.form.properties.forEach(item => {
				...
				total_area_amount =
					total_area_amount + item.asset_general_value_sum_area;
			...
			this.form.properties.forEach(property => {
				property.property_detail.forEach(property_detail => {
					property_detail.convert_fee = parseInt(
						(max_value - property_detail.circular_unit_price) *
							property_detail.total_area
					);
					convert_fee = convert_fee + property_detail.convert_fee;
					type_purposes.forEach(purposes => {
						if (property_detail.land_type_purpose === purposes.id) {
							property_detail.land_type_purpose_data.description =
			...
			this.form.properties.forEach(property => {
				property.property_detail.forEach(property_detail => {
					if (
						...
						property_detail.price_land = parseInt(
							this.form.total_land_unit_price / total_area -
								(max_value - property_detail.circular_unit_price)
						);
					} else {
		...
		removeTangible(index) {
			...
			total.forEach(item => {
				totalConstructionAmount =
					totalConstructionAmount + parseInt(item.estimation_value);
				totalConstructionArea =
					totalConstructionArea + parseInt(item.total_construction_area);
			});
		...
		changeStreetDistance() {
			...
			if (wardName === "") {
				this.form.full_address =
					streetTemp + ", " + districtName + ", " + provinceName;
			} else if (streetName === "") {
				this.form.full_address =
		...
		changeAmount(event, index) {
			...
			if (
				this.form.total_area_amount !== 0 ||
				this.form.total_other_amount !== 0
			) {
				this.form.total_amount =
					this.amount() +
					this.form.total_area_amount +
					this.form.total_other_amount;
			} else {
		...
		async validateBeforeSubmit() {
			...
			if (!isValid) {
				...
				(this.form.total_land_unit_price < 0 ||
					this.form.total_raw_amount < 0) &&
				(this.form.asset_type_id === 37 || this.form.asset_type_id === 38)
			) {
				this.$toast.open({
				...
				this.sortedArray.find(array => array.price_land <= 0)
			) {
				this.$toast.open({
					message:
						"Hiện có đơn giá đất bằng 0. Vui lòng kiểm tra lại giá trị đầu vào",
					type: "error",
					position: "top-right"
				});
			} else {
		...
		async getDictionary() {
			try {
				...
				propertyTypesBDS = await propertyTypesTem.filter(
					item => item.dictionary_acronym === "BDS"
				);
		...
		handleChangeBlock(blockId) {
			...
			if (blockId) {
				...
				this.form.apartment_specification.handover_year =
		...
		changeAreaApartment(event) {
			...
			if (this.form.total_amount) {
				this.form.total_estimate_amount =
					parseInt(this.form.total_amount) + parseInt(this.form.adjust_amount);
			} else {
				this.form.total_estimate_amount = 0;
			}
			if (
				this.form.room_details.length > 0 &&
				this.form.room_details[0].area > 0
			) {
				this.form.room_details[0].unit_price = parseFloat(
					this.form.total_estimate_amount / this.form.room_details[0].area
				).toFixed(0);
			...
			total.forEach(item => {
				total_area_amount =
					total_area_amount + parseFloat(item.asset_general_value_sum_area);
				total_area = total_area + parseFloat(item.asset_general_land_sum_area);
				item.property_detail.forEach(property_detail => {
					land_use.push(property_detail);
				});
		...
		async handleSave(property, property_index) {
			...
			properties.forEach(property => {
				total_area_amount =
					total_area_amount + parseFloat(property.asset_general_value_sum_area);
				total_area =
					total_area + parseFloat(property.asset_general_land_sum_area);
				property.property_detail.forEach(property_detail => {
			...
			this.form.total_land_unit_price =
				this.form.total_raw_amount + convert_fee;
			properties.forEach(property => {
				property.property_detail.forEach(property_detail => {
					if (
						this.form.total_land_unit_price / total_area -
		...
		async handleSaveTangible(tangible, tangible_index) {
			...
			total.forEach(item => {
				total_tangible_area =
					total_tangible_area + parseFloat(item.total_construction_area);
		...
		async handleCategory(index) {
			const resp = await WareHouse.getBuildingPrices(
				this.form.tangible_assets[index].building_category_id
			);
		...
		changeTotalAmount(event) {
			...
			this.form.adjust_amount = parseFloat(
				this.form.total_amount * (this.form.adjust_percent / 100)
			).toFixed(0);
			this.render_percent++;
		...
		changeAdjustPercent(event) {
			...
			this.form.adjust_amount = parseFloat(
				this.form.total_amount * (event / 100)
			).toFixed(0);
			this.render_percent++;
			if (
				this.form.total_amount !== null &&
				this.form.total_amount !== undefined &&
				this.form.total_amount !== ""
			) {
				this.form.total_estimate_amount =
					parseInt(this.form.total_amount) + parseInt(this.form.adjust_amount);
			...
			total.forEach(item => {
				item.property_detail.forEach(unit => {
					unit.convert_fee = parseInt(
						(max_value - unit.circular_unit_price) * unit.total_area
					);
					if (
						this.form.total_land_unit_price / total_area -
							(max_value - unit.circular_unit_price) >
	...
	async beforeMount() {
		...
		this.isSubmit = false;
		this.form.source_id =
			this.infoSources && this.infoSources.length > 0
				? this.infoSources[0].id
				: "";
