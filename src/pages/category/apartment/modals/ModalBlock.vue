	...
	<div class="card">
		<div class="container-title">
			<div class="d-flex justify-content-between">
				<h2 class="title">{{isEdit ? 'Chỉnh sửa thông tin block' : 'Thêm mới thông tin block'}}</h2>
				<img height="35px" @click="handleCancel" class="cancel" src="@/assets/icons/ic_cancel_2.svg" alt="">
			</div>
		</div>
		<div class="card-body">
			<ValidationObserver tag="form" ref="formBlock" @submit.prevent="validateBlock">
				<InputText
				v-model="form.name"
				rules="required"
				:max-length="200"
				...
				:min="0"
				rules="required"
				@change="changeTotalFloor($event)"
				/>
				<InputNumberNoneFormat
				v-model="form.first_floor"
				:vid="'first_floor'"
				class="form-group-container col-12"
				label="Tầng đầu tiên"
				:max="99999999999999"
				...
				:vid="'apartments_per_floor'"
				class="form-group-container col-12"
				label="Số căn hộ mỗi tầng"
				:max="99999999999999"
				:min="0"
				rules="required"
				@change="changApartmentPerFloor($event)"
				/>
			</ValidationObserver>
			<div class="btn__group">
				...
				<button class="btn btn-white btn-orange text-nowrap" type="button" @click.prevent="validateBlock"><img src="@/assets/icons/ic_save.svg" style="margin-right: 5px" alt="save"> Lưu</button>
			</div>
		</div>
	</div>
</div>
...
export default {
	...
	methods: {
		...
		async validateBlock () {
			const valid = await this.$refs.formBlock.validate()
			if (valid) {
