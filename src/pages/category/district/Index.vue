<template>
 <div class="container-fluid">
   <div class="pannel">
     <div class="pannel__content d-lg-flex d-block justify-content-end align-items-end">
       <Search @filter-changed="onFilterChange($event)"/>
       <router-link :to="{name: 'district.create'}" class="btn btn-create btn-white text-nowrap index-screen-button m-0 px-3 mb-0" tag="button" v-if="add"><img
...
export default {
	...
	methods: {
		async onFilterChange ($event) {
			...
			this.filter = {...$event}
			await this.listDistrict(params)
		},
		...
		async handleDelete () {
			try {
				await District.deleteDistrict(this.id)
		...
		async listDistrict (params = {}) {
			...
			try {
				const resp = await District.paginate({
					query: {
						page: 1,
						limit: 20,
						...params,
