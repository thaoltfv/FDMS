 ...
 <div class="container-fluid">
   <div class="pannel">
     <div class="pannel__content d-md-flex d-block justify-content-end align-items-end">
       <Search :filter_search="filter_search"  @filter-changed="onFilterChange($event)" />
       <button @click.prevent="handleCreate(type)" :class="type !== '' && type !== undefined && type !== null ? '' : 'disabled'" class="btn btn-create btn-white text-nowrap index-screen-button mb-0 px-3" tag="button" v-if="add"><img
...
export default {
	...
	methods: {
		async onFilterChange ($event) {
			...
			if ($event.search !== '' && $event.search !== undefined && $event.search !== null) {
				this.filter = {...$event}
				const params = {
					query: $event.search,
					status: 'ALL'
