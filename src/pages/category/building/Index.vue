...
Vue.filter('formatedNumber', function (value) {
	if (value) {
		let formatedNum = value.toString().replace('.', ',')
		return formatedNum.toString().replace(/^[+-]?\d+/, function (int) {
			return int.replace(/(\d)(?=(\d{3})+$)/g, '$1.') + ' đ'
		})
	}
})
...
export default {
		...
		columns () {
			return [
				...
				},
				{
					...
					align: 'left',
					dataIndex: 'category_building.description'
				},
				{
					title: 'Cấp nhà',
					align: 'center',
					dataIndex: 'level.description'
				},
				{
					title: 'Hạng nhà',
					align: 'center',
					dataIndex: 'rate.description'
				},
				{
					title: 'Cẩu trục',
					align: 'center',
					dataIndex: 'crane.description'
				},
				{
					title: 'Khẩu độ',
					align: 'center',
					dataIndex: 'aperture.description'
				},
				{
					title: 'Đơn giá',
					align: 'center',
	...
	methods: {
		...
		async handleDelete () {
			const resp = await Building.deleteBuilding(this.id)
			if (resp.data) {
		...
		async getBuildings (params = {}) {
			...
			try {
				const resp = await Building.paginate({
					query: {
						page: 1,
						limit: 20,
						...params,
	...
	async handleDelete () {
		try {
			await Building.deleteBuilding(this.id)
		} catch (err) {
