<template>
	<div class="container__filter">
		...
		<div
			class="filter__detail"
			:class="
				search.estimate_type === 'CHUNG_CU' ? 'filter__detail--apartment' : ''
			"
		>
			<div class="filter__detail--container">
				<div class="container__btn--search">
					<div class="d-block d-md-flex justify-content-end btn-container">
						<button
							class="btn btn-orange btn-search"
							type="button"
							@click="handleSearchAdvanced"
				...
				<div class="container-filter">
					<div class="container-input d-flex">
						<div
							class="container-input__detail w-100 align-items-center justify-content-between"
						>
							<InputCategory
							...
							/>
							<button
								class="btn btn-orange btn-orange--search"
								id="coordinateFilter"
								@click="handleSearch"
							>
								Tìm kiếm
...
export default {
	...
	methods: {
		...
		async initMap() {
			...
			if (address) {
				if (
					...
					parseFloat(address.split(",")[0]) &&
					parseFloat(address.split(",")[1])
				) {
					await this.geocodeAddress(geocoder, address, "location");
				} else {
					await this.geocodeAddress(geocoder, address, "address");
				}
			}
		},
		...
		async getAddressDetail(address_components) {
			let address_province = address_components.find(
				address_component_province =>
					address_component_province.types[0] === "administrative_area_level_1"
			);
			let address_district = address_components.find(
				address_component_district =>
					address_component_district.types[0] === "locality" ||
					address_component_district.types[0] === "administrative_area_level_2"
			);
			let address_street = address_components.find(
				address_component => address_component.types[0] === "route"
			);
			if (address_province) {
				this.address_components.province = address_province.long_name.normalize(
					"NFC"
				);
			} else {
			...
			if (address_district) {
				this.address_components.district = address_district.long_name.normalize(
					"NFC"
				);
			} else {
			...
			if (address_street) {
				this.address_components.street = address_street.long_name.normalize(
					"NFC"
				);
			} else {
		...
		async getAddressLocation() {
			...
			const resp = await PriceEstimate.getAddress({
				province,
				district,
				wards
			});
			...
			if (this.address_ids.district && this.address_ids.district.id) {
				...
				if (resp.data.street) {
					...
					streetJson.data.forEach(streetDistrict => {
						if (
							address_street &&
							streetDistrict.district.toLowerCase() ===
								this.address_ids.district.name.toLowerCase() &&
							streetDistrict.street ===
								address_street.long_name.normalize("NFC")
						) {
							street = streetDistrict.street_compare;
						}
					});
					const respAgain = await PriceEstimate.getStreet({
						district_id,
						street
					});
		...
		getAddress() {
			if (
				this.address_ids.province !== undefined &&
				this.address_ids.province !== null
			) {
			...
			if (
				this.address_ids.district !== undefined &&
				this.address_ids.district !== null
			) {
			...
			if (
				this.address_ids_street &&
				this.address_ids_street.street !== undefined &&
				this.address_ids_street.street !== null
			) {
			...
			if (
				this.address_ids.ward !== undefined &&
				this.address_ids.ward !== null
			) {
		...
		changeAssetType() {
			...
			this.assetTypes.forEach(assetType => {
				if (
					(this.search.estimate_type === "DAT" &&
						assetType.description === "ĐẤT CÓ NHÀ") ||
					(this.search.estimate_type === "DAT" &&
						assetType.description === "ĐẤT TRỐNG")
				) {
					this.search.asset_type_ids.push(assetType.id);
				} else if (
					this.search.estimate_type === "CHUNG_CU" &&
					assetType.description === "CHUNG CƯ"
				) {
					this.search.asset_type_ids.push(assetType.id);
				}
			});
		...
		async getApartmentAddress() {
			this.apartments.forEach(apartment => {
				if (apartment.id === this.search.apartment_id) {
					if (apartment.province !== undefined && apartment.province !== null) {
						if (
							this.apartment.province === "" ||
							this.apartment.province === undefined ||
							this.apartment.province === null
						) {
							this.apartment.province = apartment.province.name;
						}
						if (
							this.apartment.province_id === "" ||
							this.apartment.province_id === undefined ||
							this.apartment.province_id === null
						) {
					...
					if (apartment.district !== undefined && apartment.district !== null) {
						if (
							this.apartment.district === "" ||
							this.apartment.district === undefined ||
							this.apartment.district === null
						) {
							this.apartment.district = apartment.district.name;
						}
						if (
							this.apartment.district_id === "" ||
							this.apartment.district_id === undefined ||
							this.apartment.district_id === null
						) {
					...
					if (apartment.ward !== undefined && apartment.ward !== null) {
						if (
							this.apartment.ward === "" ||
							this.apartment.ward === undefined ||
							this.apartment.ward === null
						) {
							this.apartment.ward = apartment.ward.name;
						}
						if (
							this.apartment.ward_id === "" ||
							this.apartment.ward_id === undefined ||
							this.apartment.ward_id === null
						) {
					...
					if (apartment.street !== undefined && apartment.street !== null) {
						if (
							this.apartment.street === "" ||
							this.apartment.street === undefined ||
							this.apartment.street === null
						) {
							this.apartment.street = apartment.street.name;
						}
						if (
							this.apartment.street_id === "" ||
							this.apartment.street_id === undefined ||
							this.apartment.street_id === null
						) {
			...
			if (this.name !== "" && this.name !== null && this.name !== undefined) {
				this.search.full_address =
					`${this.name ? this.name : ""}` +
					`${this.apartment.street ? ", " + this.apartment.street : ""}` +
					`${this.apartment.ward ? ", " + this.apartment.ward : ""}` +
					`${this.apartment.district ? ", " + this.apartment.district : ""}` +
					`${this.apartment.province ? ", " + this.apartment.province : ""}`;
			} else if (
				this.apartment.street !== "" &&
				this.apartment.street !== undefined &&
				this.apartment.street !== null
		...
		getRadius() {
			let radius = "";
			if (
				this.apartment.district !== "Thành phố Biên Hòa" &&
				this.apartment.district !== "Thành phố biên hòa" &&
				this.apartment.dibstrict !== "Thành phố Long Khánh"
			) {
				radius = 2000;
			} else {
				radius = 1000;
		...
		handleApartment() {
			...
			if (
				this.search.coordinates === "" ||
				this.search.coordinates === undefined ||
				this.search.coordinates === null
			) {
				this.search.full_address = name + ", " + apartmentAddress;
			} else {
				this.search.full_address = apartmentAddress;
		...
		getTransactionType() {
			this.transactionTypes.forEach(transaction => {
				if (
					transaction.description === "ĐÃ BÁN" ||
					transaction.description === "ĐANG RAO BÁN"
				) {
