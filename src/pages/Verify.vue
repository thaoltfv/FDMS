<template>
	<div class="login" v-if="layout === 'auth'"></div>
</template>
...
export default {
	...
	methods: {
		getToken() {
			if (
				store.getters.profile === undefined ||
				store.getters.profile === null
			) {
				firebase.auth().onAuthStateChanged(user => this.getUser(user));
			}
		},
		async getUser(user) {
			...
			try {
				...
				if (response.data && response.data.user_id) {
					...
					if (profile) {
						...
						let nameRoute = "certification_brief.index";
						if (
							profile &&
							profile.data &&
							profile.data.user &&
							profile.data.user.roles &&
							(profile.data.user.roles[0].role_name.toUpperCase() === "KHÁCH" ||
