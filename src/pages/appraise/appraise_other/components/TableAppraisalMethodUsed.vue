							...
							>
								<div class="row table-footer" v-if="isAddNewItem">
									<div class="input-table col-3">
										<label>Phương pháp thẩm định sử dụng</label>
										<InputText
											class="label-none input-error"
											v-model="form.name"
											vid="addName"
											label="phương pháp thẩm định sử dụng"
											rules="required"
										/>
									</div>
									...
									v-decorator="[
										'name',
										{
											rules: [
												{
													required: true,
													message: 'phương pháp thẩm định sử dụng là bắt buộc'
												}
											]
										}
									]"
...
export default {
	...
	computed: {
		columns() {
			return [
				{
					title: "Phương pháp thẩm định sử dụng",
					align: "left",
					width: "25%",
	...
	methods: {
			...
			try {
				const resp = await AppraiseOther.paginate({
					query: {
						page: 1,
						limit: 15,
						search: "PHUONG_PHAP_THAM_DINH_SU_DUNG",
						...query
					}
				});
		...
		async handleDelete() {
			...
			this.$toast.open({
				message: "Xóa phương pháp thẩm định sử dụng thành công",
				type: "success",
				position: "top-right"
			});
		},
		...
		async createAppraiser(data) {
			try {
				...
				if (resp) {
					if (resp.data) {
						this.isSubmit = false;
						this.message = "Tạo mới phương pháp thẩm định sử dụng thành công.";
		...
		async handleUpdateTDV(data) {
			...
			if (resp.data) {
				this.message = "Cập nhật phương pháp thẩm định sử dụng thành công.";
				this.openNotification = true;
				this.editingKey = "";
				this.isAddNewItem = false;
				this.onPageChange(1);
