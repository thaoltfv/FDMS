			...
			<div class="table-detail">
				<a-form :form="formANTD">
					...
					>
									...
									v-decorator="[
										'name',
										{
											rules: [
												{
													required: true,
													message: 'Tên cơ sở thẩm định là bắt buộc'
												}
											]
										}
									]"
						...
						<template
							slot="dictionary_acronym"
							slot-scope="dictionary_acronym, record"
						>
							<a-form-item class="mb-0" v-if="record.editable">
								<a-select
									mode="multiple"
									...
									v-decorator="[
										'dictionary_acronym',
										{
											rules: [
												{
													required: true,
													message: 'Loại thẩm định là bắt buộc'
												}
											]
										}
									]"
								>
									<a-select-option
										v-for="item in typeAppraiseProperty"
										:key="item.acronym"
										:value="item.acronym"
							...
							<p v-else class="mb-0 description">
								{{ showAcronymTable(dictionary_acronym) }}
							</p>
						</template>
						...
						<template slot="status" slot-scope="status, record">
							...
							<p v-else class="mb-0">
								{{ status ? "Kích hoạt" : "Chưa kích hoạt" }}
							</p>
						</template>
						...
						<template slot="action" slot-scope="action_delete, record">
							...
							<div v-else class="d-flex justify-content-end">
								<button
									...
									class="btn btn-white"
									type="button"
									@click.prevent="() => editTable(record.id)"
								>
									Sửa
								</button>
