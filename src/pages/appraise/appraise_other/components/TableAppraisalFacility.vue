                    ...
                    <a-table bordered :columns="columns" :data-source="appraiserList" :loading="isLoading" class="table__import"
                            ...
                            <ValidationObserver tag="form" ref="observer" @submit.prevent="validateBeforeSubmit">
                                <div class="row table-footer" v-if="isAddNewItem">
                                    <div class="input-table col-3">
																			<label>Tên cơ sở thẩm định</label>
                                      <InputText class="label-none input-error" v-model="form.name" vid="addName" label="Tên cơ sở thẩm định" rules="required"/>
                                    </div>
                        ...
                        <template slot="name" slot-scope="name,record">
                            <a-form-item class="mb-0" v-if="record.editable">
                                <a-input class="text-left" v-decorator="['name', { rules: [{ required: true, message: 'Tên cơ sở thẩm định là bắt buộc' }] }]"/>
                            </a-form-item>
                        ...
                        <template slot="description" slot-scope="description,record">
                            ...
                            <p v-else class="mb-0 description">{{showAcronymTable(dictionary_acronym)}}</p>
                        </template>
                        ...
                        <template slot="action" slot-scope="action_delete,record">
													...
													<div v-else class="d-flex justify-content-end">
														<button v-if="edit" :disabled="editingKey !== ''" class="btn btn-white" type="button" @click.prevent="() => editTable(record.id)">Sửa</button>
														<!-- <button v-if="deleted" :disabled="editingKey !== ''" class="btn btn-white" type="button" @click.prevent="handleOpenModal(record.id)">Xóa</button> -->
													</div>
                        </template>
                    </a-table>
                </a-form>
...
export default {
	...
	methods: {
		...
		async handleDelete () {
			...
			this.$toast.open({
				message: 'Xóa tên cơ sở thẩm định thành công',
				type: 'success',
				position: 'top-right'
			})
		},
		...
		async createAppraiser (data) {
			try {
				...
				if (resp) {
					if (resp.data) {
						this.isSubmit = false
						this.message = 'Tạo mới tên cơ sở thẩm định thành công.'
		...
		async  handleUpdateTDV (data) {
			...
			if (resp.data) {
				this.message = 'Cập nhật tên cơ sở thẩm định thành công.'
				this.openNotification = true
				this.editingKey = ''
				this.isAddNewItem = false
				this.onPageChange(1)
