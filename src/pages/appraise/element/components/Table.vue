<template>
  <div id="wareHouse" class="w-100 position-relative">
    ...
    <div class="table-detail container__table--detail position-relative" :class="listAppraise.length === 0 ? 'empty-data' : ''">
      <a-form :form="formYTSS">
        <a-table bordered
                ...
                >
          <!-- thêm mới footer -->
          <template slot="footer" slot-scope="currentPageData" v-if="add" class="footer">
                  <ValidationObserver tag="form"
                                      ref="observer"
                                      @submit.prevent="validateBeforeSubmit">
                    <div class="row table-footer align-items-center" v-if="isAddNewItem">
                      ...
                      <div class="input-table col-3">
                        <InputText class="label-none input-error" v-model="form.appraise_title" vid="addName" label="TSTD" rules="required"/>
                      </div>
                      <div class="input-table col-1">
                        <InputText class="label-none input-error" :max-length="3" type="number" v-model="form.appraise_point" label="Điểm" rules="required"/>
                      </div>
                      <div class="input-table col-2">
                        <InputText class="label-none input-error" v-model="form.asset_title" vid="addId" label="TSSS" rules="required"/>
                      </div>

                      <div class="input-table col-1">
                        <InputText class="label-none input-error" :max-length="3" type="number" v-model="form.asset_point" label="Điểm" rules="required"/>
                      </div>
                      <div class="input-table col-2">
                        <InputText class="label-none input-error" v-model="form.description" vid="addId" label="Đánh giá" rules="required"/>
                      </div>
                      <div class="input-table col-1">
                        <label><strong>TLĐC</strong></label>
                          <InputNumberFormat
                            ...
                            :min="-100" class="label-none input-error"
                    ...
                    <button v-if="!isAddNewItem && add" class="btn btn-white" type="button" @click="addItem">Thêm mới</button>
                  </ValidationObserver>
          </template>
          ...
          <template slot="appraise_title" slot-scope="appraise_title, record">
            <a-form-item class="mb-0" v-if="record.editable">
                <a-input v-decorator="['appraise_title', { rules: [{ required: true, message: 'TSTĐ là bắt buộc' }] }]" />
            </a-form-item>
          ...
          <template slot="asset_title" slot-scope="asset_title, record">
            <a-form-item class="mb-0" v-if="record.editable">
                <a-input v-decorator="['asset_title', { rules: [{ required: true, message: 'TSSS là bắt buộc' }] }]" />
            </a-form-item>
          ...
          <template slot="description" slot-scope="description, record">
            <a-form-item class="mb-0" v-if="record.editable">
                <a-input v-decorator="['description', { rules: [{ required: true, message: 'Đánh giá là bắt buộc' }] }]" />
            </a-form-item>
          ...
          <template slot="adjust_percent" slot-scope="adjust_percent, record">
            <a-form-item class="mb-0" v-if="record.editable">
                <a-input v-decorator="['adjust_percent', { rules: [{ required: true, message: 'TLĐC là bắt buộc' }] }]" />
            </a-form-item>
            <p v-else class="mb-0">{{adjust_percent ? adjust_percent + '%' : '0%'}}</p>
          </template>

          <!-- action -->
          <template slot="action" slot-scope="action_delete, record">
                <div class="d-flex justify-content-end" v-if="record.editable">
                  <button class="btn btn-orange mr-1" @click.prevent="handleSubmitForm">Lưu</button>
                  <button @click.prevent="() => openModalCancel=true" class="btn btn-white">Hủy</button>
                </div>
                <div class="d-flex justify-content-end" v-else>
                  <button v-if="edit" :disabled="editingKey !== ''" class="btn btn-white" type="button" @click.prevent="() => editTable(action_delete.id)">Sửa</button>
                  <button v-if="deleted" class="btn btn-white" type="button" @click.prevent="handleOpenModal(action_delete.id)">Xóa</button>
                </div>
          </template>
