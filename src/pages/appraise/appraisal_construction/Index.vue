            ...
            >
                ...
                <ValidationObserver tag="form"
                  ...
                  <div class="row table-footer align-items-center" v-if="isAddNewItem">
                    <div class="input-table col-2">
                      <InputText class="label-none input-error" v-model="form.name" vid="addName" label="Tên" rules="required"/>
                    </div>
                    <div class="input-table col-2">
                      <InputText class="label-none input-error" v-model="form.address" vid="addId" label="Địa chỉ" rules="required"/>
                    </div>
                    <div class="input-table col-1">
                      <InputText class="label-none input-error" :max-length="11" type="number" v-model="form.phone_number" label="Điện thoại" rules="required"/>
                    </div>
                    <div class="input-table col-2">
                      <InputText class="label-none input-error" v-model="form.manager_name" label="Giám đốc" rules="required"/>
                    </div>
                    <div class="input-table col-2">
                      <label>Đơn giá xây dựng</label>
                      <InputNumberFormat @change="changeUnitPrice($event)" :formatter="valueFormat => `${valueFormat}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')" :max="99999999999999" :min="-99999999999999" class="label-none input-error" v-model="form.unit_price_m2" label="Đơn giá xây dựng" rules="required"/>
                    </div>
              ...
              <template slot="address" slot-scope="address, record">
                <a-form-item class="mb-0" v-if="record.editable">
                  <a-input v-decorator="['address', { rules: [{ required: true, message: 'Địa chỉ là bắt buộc' }] }]" />
                </a-form-item>
                <p v-else class="mb-0 address">{{address}}</p>
              </template>
              <template slot="name" slot-scope="name, record">
                <a-form-item class="mb-0" v-if="record.editable">
                  <a-input v-decorator="['name', { rules: [{ required: true, message: 'Tên là bắt buộc' }] }]" />
                </a-form-item>
                <p v-else class="mb-0 address">{{name}}</p>
              </template>
              <template slot="manager_name" slot-scope="manager_name, record">
                <a-form-item class="mb-0" v-if="record.editable">
                  <a-input v-decorator="['manager_name', { rules: [{ required: true, message: 'Giám đốc là bắt buộc' }] }]" />
                </a-form-item>
              ...
              <template slot="unit_price_m2" slot-scope="unit_price_m2, record">
                <a-form-item class="mb-0" v-if="record.editable">
                  <InputNumberFormat
                    ...
                    :max="99999999999999" :min="-99999999999999" class="label-none input-error"
                    label="Đơn giá xây dựng"
                    rules="required"/>
                </a-form-item>
                <p v-else class="mb-0">{{unit_price_m2 ? formatNumber(unit_price_m2) : 0}} đ</p>
              </template>
              ...
              <template slot="is_defaults" slot-scope="is_defaults, record">
                ...
                <p v-else class="mb-0">{{is_defaults === true ? 'Có' : 'Không'}}</p>
              </template>
              <template slot="action" slot-scope="action_delete, record">
                  ...
                  <div class="d-flex justify-content-end" v-else>
                    ...
                    <button class="btn btn-white" v-if="deleted" type="button" @click.prevent="handleOpenModal(action_delete.id)">Xóa</button>
                  </div>
              </template>
            </a-table>
          </a-form>
					...
					if (target) {
						...
						if (target.id && params) {
							...
							if (this.getConstructions() && params.is_defaults) {
								this.$toast.open({
									message: 'Đã có 3 đơn vị Xây dựng được chọn mặc định vui lòng đưa trường mặc định về không để lưu',
									type: 'error',
									position: 'top-right'
								})
