	...
	<div class="container-fluid">
		<div class="pannel">
			...
			>
				...
				<router-link
					:to="{ name: 'customer_group_third.create' }"
					class="btn btn-create btn-white text-nowrap index-screen-button mt-xl-0 mt-2 m-0 mb-0 px-3"
					tag="button"
					v-if="add"
					><img
...
export default {
	...
	methods: {
		async onFilterChange($event) {
			...
			this.filter = { ...$event };
			await this.listCustomerGroupThird(params);
		},
		...
		async listCustomerGroupThird(params = {}) {
			this.isLoading = true;
			const filter = {};
			...
			try {
				const resp = await CustomerGroupThird.paginate({
					query: {
						page: 1,
						limit: 20,
						...params,
