    ...
    <ValidationObserver tag="form"
                ...
                <div class="col-12 col-lg-3">
                  <div class="card-body__avatar">
                    ...
                    <div class="container__input">
                      <button type="button" class="btn btn-orange mr-0" @click="showAvatar()">Thay đổi avatar</button>
                    </div>
                  </div>
                </div>
                <div class="col-12 col-lg-9">
                  <div class="container-img">
                    <div class="container__input d-flex justify-content-end mb-1" style="margin-right: 0.75rem">
                      <button type="button" class="btn btn-orange mr-0">Tải ảnh lên</button>
                      <input type="file" id="image" accept="image/png, image/gif, image/jpeg, image/jpg" class="input__image" multiple @change="onImageChange($event)"/>
                    </div>
            ...
            <div class="container-fluid">
              <div class="row justify-content-between">
                <div class="col-12 col-md-6 col-lg-3 form-group-container">
                  ...
                  <div class="form-control form-control__id disabled" v-if="$route.name === 'customer.create'"><p class="mb-0" ></p></div>
                  <div class="form-control form-control__id disabled" v-if="$route.name === 'customer.edit'"><p class="mb-0" style="color: #FAA831" >{{'DT_' + form.id}}</p></div>
                </div>
                ...
                <div class="col-12 col-md-6 col-lg-3 form-group-container">
                  ...
                  <div class="form-control disabled"><p class="mb-0">{{form.created_date}}</p></div>
                </div>
                <div class="col-12 col-md-6 col-lg-3 form-group-container">
                  <label class="font-weight-bold color-black">Người tạo</label>
                  <div class="form-control disabled"><p class="mb-0">{{form.created_by}}</p></div>
                </div>
                <div class="col-12 col-md-6 col-lg-4 form-group-container">
                  <label class="font-weight-bold color-black">Trạng thái</label>
                  <div class="form-control disabled"><p class="mb-0">{{form.status === 'inactive' ? 'Đang vô hiệu hóa' : form.status === 'active' ? 'Đang hoạt động' : ''}}</p></div>
                </div>
              </div>
              <div class="card-info">
                <div class="row">
      ...
      <div class="btn-footer d-md-flex d-block justify-content-end align-items-center">
        <div class="d-lg-flex d-block button-contain">
          <button class="btn btn-white btn-orange text-nowrap" :class="{'btn-loading disabled': isSubmit}" type="submit" > <img src="../../assets/icons/ic_save.svg" :class="{'d-none': isSubmit}"  style="margin-right: 12px" alt="save"> Lưu</button>
          <button @click.prevent="handleOpenModalCancel" type="button" class="btn btn-white text-nowrap" :class="{'disabled': isSubmit}">
            <img src="../../assets/icons/ic_destroy.svg" style="margin-right: 12px" alt="save">
            Hủy
          </button>
          <button class="btn btn-white text-nowrap" :class="{'disabled': isSubmit}" type="button" v-if="$route.name === 'customer.edit' && form.status === 'active'" @click="changeStatusInactive">
            <img src="../../assets/icons/ic_lock.svg" style="margin-right: 12px" alt="save">
            Vô hiệu hóa
          </button>
          <button type="button" class="btn btn-white text-nowrap" :class="{'disabled': isSubmit}" v-if="$route.name === 'customer.edit' && form.status === 'inactive'" @click="changeStatusActive">
            <img src="../../assets/icons/ic_unlock.svg" style="margin-right: 12px" alt="save">
            Kích hoạt lại
          </button>
          <button class="btn btn-white text-nowrap" :class="{'disabled': isSubmit}" v-if="$route.name === 'customer.edit'" disabled>
            <img src="../../assets/icons/ic_destroy.svg" style="margin-right: 12px" alt="save">
            Xem doanh thu
          </button>
        </div>
...
export default {
	...
	methods: {
		...
		changeStatusInactive () {
			this.messageNotification = 'Bạn có chắc muốn chuyển trạng thái sang vô hiệu hóa không?'
			this.disable = true
			this.active = false
			this.modalNotificationCustomer = true
		},
		changeStatusActive () {
			this.messageNotification = 'Bạn có chắc muốn chuyển trạng thái sang hoạt động không?'
			this.disable = false
			this.active = true
			this.modalNotificationCustomer = true
		},
		...
		getCreatedAt () {
			...
			this.form.created_date = `${today.getDate() < 10 ? '0' + today.getDate() : today.getDate()}` + '/' + `${(today.getMonth() + 1) < 10 ? '0' + (today.getMonth() + 1) : (today.getMonth() + 1)}` + '/' + today.getFullYear()
		},
		getCenter (center) {
			this.center = center
		},
		...
		uploadImage () {
			...
			return File.upload({data: formData}).then(response => {
				if (response && response.data && response.data.data) {
					...
					if (this.form.customer_picture === '' || this.form.customer_picture === undefined || this.form.customer_picture === null) {
		...
		async handleSaveTangible (tangible, tangible_index) {
			...
			total.forEach(item => {
				total_tangible_area = total_tangible_area + parseFloat(item.total_construction_area).toFixed(0)
