	...
	<div id="customer" class="container-fluid position-relative">
		...
		<div class="pannel">
            ...
            <img src="../../assets/icons/ic_search.svg" style="margin-right: 8px" alt="search">Tìm kiếm
					...
					>
						<img
							src="../../assets/icons/ic_disable.svg"
							style="margin-right: 8px"
							alt="search"
						/>
						Vô hiệu hóa
					...
					>
						<img
							src="../../assets/icons/ic_active.svg"
							style="margin-right: 8px"
							alt="search"
						/>
						Kích hoạt lại
		...
		<div
			class="table-detail position-relative"
		...
		>
			...
			>
				...
				<template slot="id" slot-scope="id, property">
					...
					>
						<button @click.prevent="handleDetail(id)" class="link-detail">
							{{ "DT_" + id }}
						</button>
					</div>
				</template>
...
export default {
	...
	methods: {
		...
		async enableClick(index) {
			...
			this.isSubmit = false;
			this.message =
				"Hủy vô hiệu hóa  " + "<b>" + "TSS_" + index + "</b>" + " thành công.";
			this.modalNotification = true;
		},
	...
	beforeRouteEnter: async (to, from, next) => {
		...
		const category = await Customer.find(
			to.query[
				("search",
				"id",
				"name",
