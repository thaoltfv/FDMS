<template>
	<div v-if="!isMobile()" class="main-wrapper-new">
            ...
            <img src="@/assets/icons/ic_new.svg" style="margin-right: 8px" alt="search">Tạo mới
					...
					<!-- <b-dropdown class="dropdown-container" no-caret v-if="this.export">
						...
						<b-dropdown-item @click.prevent="exportMonthBefore"
							>Xuất dữ liệu tháng trước</b-dropdown-item
						>
		...
		<div class="container-fluid appraise-container mt-3">
			<TableAll
				:listCertificates="listPriceEstimateAll"
				:isLoading="isLoading"
...
import PriceEstimates from "@/models/PriceEstimates";
...
export default {
	...
	data() {
		return {
			...
			listPriceEstimateDraft: [],
			listPriceEstimateOpen: [],
			listPriceEstimateLock: [],
			listPriceEstimateClose: [],
			listPriceEstimateAll: [],
			list: [],
			paginationAll: {},
			paginationDraft: {},
			paginationOpen: {},
	...
	methods: {
		...
		async getDataDraft(params = {}) {
			...
			const filter = {};
			try {
				const resp = await PriceEstimates.paginate({
					query: { page: 1, limit: 20, ...params, ...filter, status: 1 }
				});
				this.listPriceEstimateDraft = [...resp.data.data];
		...
		async getDataOpen(params = {}) {
			...
			try {
				...
				this.listPriceEstimateOpen = [...resp.data.data];
		...
		async getDataLock(params = {}) {
			...
			try {
				...
				this.listPriceEstimateLock = [...resp.data.data];
		...
		async getDataClose(params = {}) {
			...
			try {
				...
				this.listPriceEstimateClose = [...resp.data.data];
		...
		async getDataAll(params = {}) {
			...
			try {
				...
				this.listPriceEstimateAll = [...resp.data.data];
	...
	async mounted() {
		...
		// 	const response = await Certificate.getPriceEstimateOthers()
		// 	store.commit(types.SET_APPRAISE_OTHER, {...response})
		// }
		// if (this.$store.getters.apartments && this.$store.getters.apartments.length === 0) {
		// 	const apartment = await WareHouse.getApartment()
