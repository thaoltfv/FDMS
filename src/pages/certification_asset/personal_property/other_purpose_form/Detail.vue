...
export default {
	...
	beforeRouteEnter: async (to, from, next) => {
		const getDataStep = await CertificateAsset.getDataAllStepOther(to.query['id'], to.query['asset_type_id'])
	...
	async created () {
		...
		if ('id' in this.$route.query && this.$route.name === 'certification_asset.other_purpose.detail') {
	...
	methods: {
		...
		async getHistoryTimeline (id) {
			const res = await CertificateAsset.getActivityOther(id)
			if (res.data) {
		...
		async handleSubmitStep_1 (id, data) {
			this.isSubmit = true
			const res = await CertificateAsset.submitOtherStep1(id || '', data)
			if (res.data) {
				await this.$toast.open({
		...
		async handleSubmitStep_2 (id, data) {
			this.isSubmit = true
			// call api
			const res = await CertificateAsset.submitOtherStep2(id, data)
			if (res.data) {
				await this.$toast.open({
					message: 'Lưu thông tin chung',
					type: 'success',
		...
		async handleSubmitStep_3 (id, data) {
			this.isSubmit = true
			// call api
			const res = await CertificateAsset.submitOtherStep3(id, data)
			if (res.data) {
				await this.$toast.open({
					message: 'Lưu thông tin chung',
					type: 'success',
		...
		async getDictionary () {
			...
			let propertyTypesOther = propertyTypesTem.filter(item => item.dictionary_acronym === 'KHAC')
		...
		async getAppraiseOthers () {
			...
			let purposeAppraisal = await appraisalPurposeTem.filter(item => item.dictionary_acronym.includes('KHAC'))
			let facilityAppraisal = await appraisalFacilityTem.filter(item => item.dictionary_acronym.includes('KHAC'))
			let principleAppraisal = await appraisalPrinciplesTem.filter(item => item.dictionary_acronym.includes('KHAC'))
			let approachMethod = await approachTem.filter(item => item.dictionary_acronym.includes('KHAC'))
			let usedMethod = await methodsUsedTem.filter(item => item.dictionary_acronym.includes('KHAC'))
