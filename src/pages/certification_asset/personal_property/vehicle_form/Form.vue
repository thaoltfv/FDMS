...
export default {
	...
	async created () {
		...
		if ('id' in this.$route.query && this.$route.name === 'certification_asset.vehicle.edit') {
			if (this.$route.meta['step']) {
				...
				this.form.step_1.vehicle_id = bindDataStep.vehicle_id
				this.form.step_1.transport_id = bindDataStep.transport_id
	...
	methods: {
		...
		async validateSubmitStep1 () {
			...
			if (isValid) {
				...
				} else if (this.$route.name === 'certification_asset.vehicle.create') {
					if (this.isDuplicate) {
		...
		async handleSubmitStep_1 (id, data) {
			this.isSubmit = true
			const res = await CertificateAsset.submitVehicleStep1(id || '', data)
			if (res.data) {
				await this.$toast.open({
		...
		async handleSubmitStep_2 (id, data) {
			this.isSubmit = true
			// call api
			const res = await CertificateAsset.submitVehicleStep2(id, data)
			if (res.data) {
				await this.$toast.open({
		...
		async handleSubmitStep_3 (id, data) {
			this.isSubmit = true
			// call api
			const res = await CertificateAsset.submitVehicleStep3(id, data)
			if (res.data) {
				await this.$toast.open({
		...
		async getDictionary () {
			...
			this.vehicleTypes = await [...resp.data.loai_phuong_tien]
			this.transportTypes = await [...resp.data.loai_phuong_tien_chi_tiet]
			...
			let manufacturerOther = manufacturerTem.filter(item => item.dictionary_acronym === 'PTVT')
			let propertyTypesTem = await [...resp.data.loai_tai_san]
			let propertyTypesOther = propertyTypesTem.filter(item => item.acronym === 'PTVT')
			if (this.$route.name === 'certification_asset.vehicle.create') {
				this.form.step_1.asset_type_id = propertyTypesOther[0].id
				this.form.step_1.vehicle_id = this.vehicleTypes.find(item => item.acronym === 'DUONG_BO').id
			}
			...
			this.transportTypes.forEach(item => {
				item.description = this.formatSentenceCase(item.description)
			})
