...
export default {
	...
	beforeRouteEnter: async (to, from, next) => {
		const getDataStep = await CertificateAsset.getDataAllStepVehicle(to.query['id'], to.query['asset_type_id'])
	...
	async created () {
		...
		if ('id' in this.$route.query && this.$route.name === 'certification_asset.vehicle.detail') {
	...
	methods: {
		...
		async getHistoryTimeline (id) {
			const res = await CertificateAsset.getActivityVerhicle(id)
			if (res.data) {
