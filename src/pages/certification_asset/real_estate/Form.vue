					...
					>
						<div class="d-lg-flex d-block button-contain">
							...
							<button
								...
								class="btn btn-white btn-orange text-nowrap"
								:class="{ 'btn_loading disabled': isSubmit }"
								@click.prevent="validateSubmitStep4"
								type="submit"
							>
								<img
									src="@/assets/icons/ic_save.svg"
					...
					>
						<div class="d-lg-flex d-block button-contain">
							...
							<button
								...
								class="btn btn-white btn-orange text-nowrap"
								:class="{ 'btn_loading disabled': isSubmit }"
								@click.prevent="validateSubmitStep5"
								type="submit"
							>
								<img
									src="@/assets/icons/ic_save.svg"
					...
					>
						<div class="d-lg-flex d-block button-contain">
							...
							<button
								...
								class="btn btn-white btn-orange text-nowrap"
								:class="{ 'btn_loading disabled': isSubmit }"
								@click.prevent="validateSubmitStep6"
								type="submit"
							>
								<img
									src="@/assets/icons/ic_save.svg"
...
export default {
		...
		if (
			...
			if (this.$route.meta["step"]) {
				...
				if (bindDataStep.filter_year) {
					this.filter_year = bindDataStep.filter_year;
				}
	...
	methods: {
		...
		// ------------------------------------------ Ation of STEP 2------------------------------------------------------------//
		addMainArea() {
			let checkIsCheckFacility = this.form.step_2.total_area.filter(
				...
				`${
					streetName.length > 0
						? (streetName.substring(0, 5) === "đường" ? "" : "Đường ") +
						  this.formatCapitalize(streetName) +
						  ", "
						: ""
				}` + this.full_address;
		...
		async getDictionary() {
			await WareHouse.getDictionaries()
				.then(resp => {
					...
					this.type_purposes = resp.data.loai_dat_chi_tiet.filter(
						i => i.status === 1
					);
		...
		checkDuplicateLandType(errorCustom) {
			if (this.step2DuplicateLandType === "") {
				this.step2DuplicateLandType = errorCustom;
			}
		}
	},
	async beforeMount() {
