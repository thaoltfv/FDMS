      ...
      <div class="card-body card-info" v-show="showDetailLand">
              ...
              <div v-for="(main_land, index) in data.total_area" :key="index" class=" mb-2 row content_form">
                ...
                <div class="col-3">
                  <InputCategoryBoolean
                    ...
                    :disabled="data.total_area.length === 1"
                  />
                </div>
        ...
        <div class="container-fluid mb-2">
          <div class="row">
            <div class="col-12 px-0">
              <div class="d-flex align-items-center sub_header_title">
                <label for="vio_land" class="label">
                  Phần diện tích vi phạm quy hoạch
                </label>
                <!-- <input id="vio_land" type="checkbox" :key="updateCheckBox" :value="checkShowPlanning" v-model="checkShowPlanning"
                       @change="handleChangeStatusPlanning"/> -->
                <label class="input-checkbox">
                  <input
                    id="vio_land" type="checkbox"
                    :key="updateCheckBox"
              ...
              <div v-if="data.planning_area.length == 0"  class="infor-box pl-2 w-100">
                <span class="mr-1">
              ...
              <div v-for="(planning_area, index) in data.planning_area" :key="index" class="mb-2 row content_form">
                ...
                <div class="col-3">
                  <InputAreaCustom
                    ...
                    :errorCustom="validateArea(planning_area.land_type_purpose_id)"
                  />
                </div>
                <div class="col-3">
                  <div class="col-12">
                ...
                <div v-if="data.planning_area.length > 1" class="col-1 px-3 d-flex align-items-end">
                  <div @click="handleDeletePlanningArea(index)" class="btn-delete">
                    <img src="@/assets/icons/ic_delete_2.svg" alt="delete">
                  </div>
                </div>
              </div>
      ...
      <div class="card-title">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <img class="img-dropdown" :class="!showDetailCommitee ? 'img-dropdown__hide' : ''"
               src="@/assets/images/icon-btn-down.svg" alt="dropdown" @click="showDetailCommitee = !showDetailCommitee">
        </div>
      </div>
	...
	methods: {
		validateArea (landTypePurposeId) {
			...
			if (landTypePurposeId) {
				let planningAreaData = this.data.planning_area.filter(item => item.land_type_purpose_id === landTypePurposeId)
				let totalAreaData = this.data.total_area.filter(item => item.land_type_purpose_id === landTypePurposeId)
				if (planningAreaData && totalAreaData && planningAreaData.length > 0 && totalAreaData.length > 0) {
					let planningArea = planningAreaData[0].planning_area
					let totalArea = totalAreaData[0].total_area
					if (planningArea > totalArea) error = 'Diện tích vi phạm không được lớn hơn diện tích sử dụng'
					else error = ''
				} else error = ''
			} else error = ''
			this.$emit('validateArea', error)
			return error
		},
		checkDuplicateLandType (landTypePurposeId, data) {
			...
			this.$emit('checkDuplicateLandType', error)
			return error
		},
		checkFacility (facility) {
			if (facility) {
				let checkFacility = this.data.total_area.filter(item => item.is_transfer_facility === true)
		...
		changeLandPlanningPurpose (index, land_type_purpose_id) {
			...
			if (this.isDuplicateLandTypePlanning) {
