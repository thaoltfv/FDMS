        ...
        <div class="card">
          ...
          <div class="container-title container-title__footer">
            <div class="d-lg-flex d-block justify-content-end shadow-bottom">
              ...
              <button class="btn btn-white btn-orange text-nowrap" type="button" @click.prevent="validateBeforeSubmit"><img src="@/assets/icons/ic_save.svg" style="margin-right: 5px" alt="save"> Lưu</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
	...
	methods: {
		...
		changeBuildingType (event) {
			...
			if (event) {
				this.form.tangible_name = this.housingTypes.filter(item => item.id === event)[0].description
			}
		},
