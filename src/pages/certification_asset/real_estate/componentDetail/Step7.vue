          ...
          <img class="img-dropdown" :class="!showDetailContruction ? 'img-dropdown__hide' : ''"
			...
			<div class="card-body card-info" v-show="showDetailContruction">
					...
					>
						<TabItem name="Bảng tổng hợp thông tin">
							<div class="content_detail_asset">
								<ValidationObserver tag="form" ref="step_7_tab1">
									<table
										class="table_detail_property color_content"
										v-if="
											appraises.asset_general &&
												appraises.asset_general.length > 0
										"
									>
										<thead class="ant-table-thead">
											<tr>
												...
												<th>TSTĐ</th>
												<th
													v-for="(asset, index) in appraises.asset_general"
													:key="'header' + index"
												>
													<a
														:href="`/property/detail?id=${asset.id}`"
														target="_blank"
														>{{ "TSSS " + asset.id }}</a
													>
												</th>
										...
										<tbody class="ant-table-tbody">
											<tr>
												...
												<td>
													{{
														appraises.asset_type
															? formatSentenceCase(
																	appraises.asset_type.description
															  )
															: "-"
													}}
												</td>
												<td
													...
													:key="'assetType' + index"
												>
													{{ formatSentenceCase(asset.asset_type.description) }}
												</td>
											</tr>
											<tr>
												<td rowspan="2">2</td>
											...
											<tr>
												...
												<td>
													<div>
														<div>
															{{
																appraises.coordinates
																	? `${appraises.coordinates.split(",")[0]},`
																	: "-"
															}}
														</div>
														<div>
											...
											<tr>
												...
												<td
													...
													:key="'assetType' + index"
												>
													{{ asset.full_address ? asset.full_address : "-" }}
												</td>
											</tr>
											<tr>
												...
												<td
													v-for="(asset, index) in appraises.asset_general"
													:key="'distance-land' + index"
												>
													<div
														v-for="(asset_com,
														index_com) in appraises.comparison_factor"
														:key="'distance-com' + index_com"
													>
														<div
															v-for="(asset_com_1,
															index_com_1) in asset_com.comparison_factor"
															:key="'distance-com1' + index_com_1"
														>
															<InputLengthArea1
																v-if="
																	asset.id == asset_com_1.asset_general_id &&
																		asset_com_1.type == 'khoang_cach'
																"
											...
											<tr>
												...
												<td>
													{{
														appraises.properties &&
														appraises.properties.length > 0
															? formatSentenceCase(
																	appraises.properties[0].legal.description
															  )
															: "-"
													}}
												</td>
												...
												>
													{{
														...
														asset.properties[0].legal
															? formatSentenceCase(
																	asset.properties[0].legal.description
															  )
															: "-"
													}}
												</td>
											...
											<tr>
												<td
													...
													:rowspan="
														3 +
															appropriate_zoning_land.length +
															violation_zoning_land.length
													"
												>
													6
												</td>
												...
												<td>
													{{
														...
														appraises.properties.length > 0
															? formatNumber(
																	appraises.properties[0].appraise_land_sum_area
															  )
															: "-"
													}}
												</td>
												...
												>
													<div
														v-for="(property,
														indexProperty) in appraises.propertyChoosing.filter(
															i => i.asset_general_id === asset.id
														)"
														:key="'propertyArea' + indexProperty"
													>
														{{
											...
											<tr>
												...
												<td
													v-for="(asset,
													indexAppropriateArea) in appraises.asset_general"
													:key="'viola' + indexAppropriateArea"
												>
													<span
														:style="
															arrayTotalAppropriateArea[asset.id] <= 0
																? { color: 'red' }
																: {}
														"
														>{{
															formatNumber(arrayTotalAppropriateArea[asset.id])
														}}</span
													>
												</td>
											</tr>
											<tr
												v-for="(area, index) in appropriate_zoning_land"
												:key="'area-price' + index"
											>
												<td>{{ area.name_purpose_land }}</td>
												<td>
													{{ formatNumber(area.appropriate_appraise_land) }}
												</td>
												<td
													v-for="(asset, index_area) in appraises.asset_general"
													:key="'area-land' + index_area"
												>
													<div
														v-if="
															!checkDataPriceUBND(
																asset,
																area.asset_general_land
															) && checkRemainValue()
														"
														:key="key_render_10"
													>
														<div
															v-for="(area_data,
															index_asset_land) in area.asset_general_land"
															:key="'areatype' + index_asset_land"
														>
															<!-- {{area_data}} -->
															<div
																:style="
															...
															>
																...
																<div
																	v-for="(asset_com,
																	index_com) in appraises.comparison_factor"
																	:key="'mucdichchinh-com' + index_com"
																>
																	<div
																		v-for="(asset_com_1,
																		index_com_1) in asset_com.comparison_factor"
																		:key="'mucdichchinh-com1' + index_com_1"
																	>
																		<!-- <InputLengthArea1
									...
									/>	 -->
										...
										{{area_data.name_purpose_land_asset}} -->
											...
											<tr>
												...
												<td
													v-for="(asset, indexVio) in appraises.asset_general"
													:key="'viola' + indexVio"
												>
											...
											<tr
												v-for="(violation_area,
												indexVioArea) in violation_zoning_land"
												:key="'area-violation-price' + indexVioArea"
											>
												<td>{{ violation_area.name_purpose_land }}</td>
												<td>
													{{
														formatNumber(violation_area.violation_appraise_land)
													}}
												</td>
												<td
													v-for="(asset, index_area) in appraises.asset_general"
													:key="'area-violation-land' + index_area"
												>
													<div
														v-if="
															!checkDataPriceUBND(
																asset,
																violation_area.asset_general_land
															)
														"
													>
														<div
															v-for="(area_data,
															index_violation_land) in violation_area.asset_general_land"
															:key="'areaViotype' + index_violation_land"
														>
															<InputArea
																v-if="area_data.asset_general_id === asset.id"
																class="label-none input_center"
											...
											<!-- Price UBND -->
											<tr
												v-for="(data, index) in data_land_price_demo"
												:key="'body-price' + index"
											>
												<td
													v-if="index === 0"
													style="padding: unset"
													:rowspan="data_land_price_demo.length"
												>
													7
												</td>
												...
												<td
													v-for="(asset,
													index_price) in appraises.asset_general"
													:key="'assetType' + index_price"
												>
													<div
														v-if="!checkDataPriceUBND(asset, data.price_asset)"
													>
														<div
															v-for="(price_data,
															index) in data.price_asset.filter(
																i => i.asset_general_id === asset.id
															)"
															:key="'pricetype' + index"
														>
															<InputCurrency
																:disabled="!isEditStatus"
																:class="{
																	'label-none input_center ': true,
																	input_number_error:
																		price_data.update_value < 0
																}"
											...
											<tr>
												...
												<td>
													...
													<div :key="'widthStreet'">
														{{
															formatNumber(
																`${getComparisonTitleByType(
																	appraises.comparison_factor[0]
																		.comparison_factor,
																	"chieu_rong_mat_tien",
																	false
																)}`
												...
												<td
													v-for="(asset, index) in appraises.comparison_factor"
													:key="'assetWidthStreet' + index"
												>
													<!-- <span :key="'chieu_rong_mat_tien' + asset.id">{{`${asset.comparison_factor.find(i => i.type === 'chieu_rong_mat_tien').asset_title}m`}}</span> -->
													<span :key="'chieu_rong_mat_tien' + asset.id"
														>{{
															formatNumber(
																`${getComparisonTitleByType(
																	asset.comparison_factor,
																	"chieu_rong_mat_tien",
																	true
																)}`
															)
											...
											<tr>
												...
												>
													...
													<span :key="'chieu_sau_khu_dat' + asset.id"
														>{{
															formatNumber(
																`${getComparisonTitleByType(
																	asset.comparison_factor,
												...
												<td>
													<div
														v-if="
															appraises.properties &&
																appraises.properties.length > 0 &&
																appraises.properties[0].land_shape
														"
													>
														{{
															formatSentenceCase(
																appraises.properties[0].land_shape.description
															)
														}}
													</div>
													<div v-else>-</div>
												...
												>
													<div
														v-if="
															asset.properties &&
																asset.properties.length > 0 &&
																asset.properties[0].land_shape
														"
													>
														{{
															formatSentenceCase(
																asset.properties[0].land_shape.description
															)
														}}
													</div>
													<div v-else>-</div>
											...
											<tr>
												...
												<td>
													<div
														v-if="
															appraises.tangible_assets &&
																appraises.tangible_assets.length > 0
														"
													>
														{{
															formatSentenceCase(
																appraises.tangible_assets[0].building_type
																	.description
															)
														}}
													</div>
												...
												>
													<div
														v-if="
															asset.tangible_assets &&
																asset.tangible_assets.length > 0
														"
													>
														{{
															formatSentenceCase(
																asset.tangible_assets[0].building_type
																	.description
															)
														}}
													</div>
											...
											<tr>
												...
												>
													...
													>
														{{
															formatNumber(
																asset.tangible_assets[0].total_construction_base
															)
														}}m<sup>2</sup>
													</div>
													<div v-else>-</div>
											...
											<tr>
												...
												<td>
													...
													>
														{{
															formatNumber(
																appraises.tangible_assets[0].remaining_quality
															)
														}}%
													</div>
													<div v-else>-</div>
												...
												>
													...
													>
														{{
															formatNumber(
																asset.tangible_assets[0].remaining_quality
															)
														}}%
													</div>
													<div v-else>-</div>
											...
											<tr>
												...
												<td
													v-for="(asset, index) in appraises.asset_general"
													:key="'assetConstructionAmount' + index"
												>
													{{
														asset.tangible_assets &&
														asset.tangible_assets.length > 0
															? (asset.tangible_assets[0].unit_price_m2
																	? formatNumber(
																			asset.tangible_assets[0].unit_price_m2
																	  )
																	: 0) + " đ"
												...
												>
													<div
														v-if="
															asset.tangible_assets &&
																asset.tangible_assets.length > 0 &&
																asset.tangible_assets[0]
														"
													>
														{{
															remainingBuildingPrice(asset.tangible_assets[0])
														}}
													</div>
													<div v-else>-</div>
												</td>
											...
											<tr>
												...
												<td
													v-for="(asset, index) in appraises.asset_general"
													:key="'othersAmount' + index"
												>
													<div v-if="asset.total_order_amount">
														{{ formatNumber(asset.total_order_amount) }}
													</div>
													<div v-else>-</div>
												</td>
											</tr>
											<tr>
												...
												<td>
													{{
														typeof appraises.properties !== "undefined" &&
														appraises.properties.length > 0
															? appraises.properties[0].description
															: ""
													}}
												</td>
												<td
													v-for="(asset, indexAsset) in appraises.asset_general"
													:key="'assetProperty' + indexAsset"
												>
													<span
														v-for="(property, index) in asset.properties.filter(
															i =>
																i.id ===
																appraises.appraise_has_assets[indexAsset]
											...
											<tr>
												...
												>
													...
													<span :key="'ket_cau_duong' + asset.id">{{
														formatSentenceCase(
															getComparisonTitleByType(
																asset.comparison_factor,
																"ket_cau_duong",
											...
											<tr>
												...
												>
													...
													<span :key="'do_rong_duong' + asset.id">{{
														formatSentenceCase(
															getComparisonTitleByType(
																asset.comparison_factor,
																"do_rong_duong",
											...
											<tr>
												...
												>
													...
													<span :key="'dieu_kien_ha_tang' + asset.id">{{
														formatSentenceCase(
															getComparisonTitleByType(
																asset.comparison_factor,
																"dieu_kien_ha_tang",
											...
											<tr>
												...
												<td
													v-for="(asset, index) in appraises.comparison_factor"
													:key="'assetSecurity' + index"
												>
													<span :key="'an_ninh_moi_truong_song' + asset.id">{{
														formatSentenceCase(
															getComparisonTitleByType(
																asset.comparison_factor,
																"an_ninh_moi_truong_song",
											...
											<tr>
												...
												<td
													v-for="(asset, index) in appraises.comparison_factor"
													:key="'appraisalFengshui' + index"
												>
													<span :key="'phong_thuy' + asset.id">{{
														formatSentenceCase(
															getComparisonTitleByType(
																asset.comparison_factor,
																"phong_thuy",
											...
											<tr>
												...
												<td
													v-for="(asset, index) in appraises.comparison_factor"
													:key="'appraisalPay' + index"
												>
													<span :key="'dieu_kien_thanh_toan' + asset.id">{{
														formatSentenceCase(
															getComparisonTitleByType(
																asset.comparison_factor,
																"dieu_kien_thanh_toan",
											...
											<tr>
												...
												<td>
													<p>Tỷ lệ rao bán</p>
													<p>Số tiền thương lượng</p>
												</td>
												<td>-</td>
												<td
													v-for="(asset, index) in form.appraise_adapter"
													:key="'adjustPercent' + index"
												>
													<InputPercent
											...
											<tr>
												...
												<td
													v-for="(asset,
													indexVioPrice) in form.appraise_adapter"
													:key="'changeViolationPrice' + indexVioPrice"
												>
													<InputCurrency
														:disabled="!isEditStatus"
														...
														defaultValue="0"
														@change="
															changeViolationPrice($event, indexVioPrice)
														"
						...
						<td>{{totalPriceViolationArea3 ? formatNumber(totalPriceViolationArea3) : 0}} đ</td> -->
											...
											<tr
												v-if="
													this.form.composite_land_remaning_slug !==
														'theo-ty-le-gia-dat-co-so-chinh' || checkOld()
												"
											>
												<td>26</td>
												<td id="land_price_change">
													Chi phí chuyển mục đích sử dụng
												</td>
												<b-tooltip
													placement="rightbottom"
													target="land_price_change"
												>
											...
											>
												...
												<td id="land_price_estimate">
													Giá trị QSDĐ
													{{
														appraises.properties[0].property_detail.find(
															property_detail =>
																property_detail.is_transfer_facility === true
											...
											>
												...
												<td id="ti_le_dat">
													...
													{{
														appraises.properties[0].property_detail.find(
															property_detail =>
																property_detail.is_transfer_facility === false
														)
															? appraises.properties[0].property_detail.find(
																	property_detail =>
																		property_detail.is_transfer_facility ===
																		false
															  ).land_type_purpose.acronym
															: "mục đích phụ"
													}}/đất
											...
											>
												<td>28</td>
												<td id="land_unit_price_estimate">
													Đơn giá
													{{
														appraises.properties[0].property_detail.find(
															property_detail =>
												...
												<b-tooltip
													placement="rightbottom"
													target="land_unit_price_estimate"
												>
												...
												<td>-</td>
												<td>
													{{ formatNumber(parseFloat(dgd1).toFixed(0)) }} đ
												</td>
												<td>
													{{ formatNumber(parseFloat(dgd2).toFixed(0)) }} đ
												</td>
												<td>
													{{ formatNumber(parseFloat(dgd3).toFixed(0)) }} đ
												</td>
											</tr>
											<tr
												v-if="
											...
											>
												...
												<td>{{ formatNumber(quydoivemucdichchinh3) }}</td>
											</tr>
											<tr
												v-if="
													this.form.composite_land_remaning_slug ===
						...
						<TabItem class="item_2" name="Bảng điều chỉnh QSDĐ">
							<div class="content_detail_asset">
								<table
									class="table_comparision color_content"
									v-if="
										appraises.comparison_factor &&
											appraises.comparison_factor.length > 0
									"
								>
									<thead class="ant-table-thead">
										<tr>
									...
									<tbody class="ant-table-tbody">
										...
										<tr
											v-for="(comparison_factor_appraise,
											index) in appraises.comparison_factor[0].comparison_factor.filter(
												comparison_factor_appraise =>
													comparison_factor_appraise.type === 'phap_ly' &&
													comparison_factor_appraise.status === 1
											)"
											:key="'appraisalLegalRow' + index"
										>
											<td rowspan="5">A</td>
											...
											<td
												...
												:key="'appraisalLegal' + index"
											>
												{{
													formatSentenceCase(
														comparison_factor_appraise.appraise_title
													)
												}}
											</td>
											<td
											...
											>
												<span
													v-for="(comparison_factor,
													indexItem) in appraises.comparison_factor[
														index
													].comparison_factor.filter(
														comparison_factor_appraise =>
															comparison_factor_appraise.type === 'phap_ly' &&
															comparison_factor_appraise.status === 1
													)"
													:key="'phap_ly' + indexItem"
													>{{
										...
										<tr
											...
											:key="'changeLegalRowC' + index"
										>
											<td>Hệ số tương quan</td>
											<td>100%</td>
											<td
												v-for="(asset, indexAsset) in appraises.asset_general"
												:key="'inputLegal' + indexAsset"
											>
												<div
													v-for="(comparison_factor,
													index) in appraises.comparison_factor[
														indexAsset
													].comparison_factor.filter(
														comparison_factor_appraise =>
															comparison_factor_appraise.type === 'phap_ly' &&
												...
												>
													<InputPercent
														...
														:text_center="true"
														@change="
															changeLegalCoefficient(
																$event,
																indexAsset,
																comparison_factor.type
															)
														"
													/>
												</div>
										...
										>
											...
											<td></td>
											<td>
												{{ formatNumber(parseFloat(pricePl1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(pricePl2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(pricePl3).toFixed(0)) }}
											</td>
										</tr>
										...
										>
											...
											<td>
												<strong>{{
													formatNumber(parseFloat(totalPricePL1).toFixed(0))
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(parseFloat(totalPricePL2).toFixed(0))
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(parseFloat(totalPricePL3).toFixed(0))
												}}</strong>
											</td>
										</tr>
										...
										<tr
											...
											index) in appraises.comparison_factor[0].comparison_factor.filter(
												comparison_factor_appraise =>
													comparison_factor_appraise.type === 'quy_mo' &&
													comparison_factor_appraise.status === 1
											)"
											:key="'quymo' + index"
										>
											<td rowspan="5">
												<div
													...
													v-if="isEditStatus"
													type="button"
													@click="dialogDeleteComparisionDefault('quy_mo')"
												>
													<img
										...
										>
											...
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceQm1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceQm2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceQm3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
										...
										>
											...
											<td>
												<strong>{{
													formatNumber(adjustPriceData["quy_mo"][0])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["quy_mo"][1])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["quy_mo"][2])
												}}</strong>
											</td>
										</tr>
										<!-- Adjust Comparison END -->
										...
										<tr
											...
											index) in appraises.comparison_factor[0].comparison_factor.filter(
												comparison_factor_appraise =>
													comparison_factor_appraise.type ===
														'chieu_rong_mat_tien' &&
													comparison_factor_appraise.status === 1
											)"
											:key="'appraisalWidthRow' + index"
										>
											<td rowspan="5">
												<div
													class="btn-delete"
													...
													type="button"
													@click="
														dialogDeleteComparisionDefault(
															'chieu_rong_mat_tien'
														)
													"
												>
											...
											>
												<span
													...
													:key="'chieu_rong_mat_tien' + indexItem"
													>{{
														formatSentenceCase(comparison_factor.asset_title)
													}}m</span
												>
										...
										>
											...
											>
												<div
													...
													:key="'chieu_rong_mat_tienC' + index"
												>
													<InputPercent
										...
										<tr
											...
											:key="'WidthChangeRow' + index"
										>
											<td>Mức điều chỉnh</td>
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceCrmt1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceCrmt2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceCrmt3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
										...
										>
											...
											<td>
												<strong>{{
													formatNumber(
														adjustPriceData["chieu_rong_mat_tien"][0]
													)
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(
														adjustPriceData["chieu_rong_mat_tien"][1]
													)
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(
														adjustPriceData["chieu_rong_mat_tien"][2]
													)
												}}</strong>
											</td>
										</tr>
										...
										>
											<td rowspan="5">
												<div
													...
													type="button"
													@click="
														dialogDeleteComparisionDefault('chieu_sau_khu_dat')
													"
												>
													<img
														src="@/assets/icons/ic_delete_2.svg"
											...
											>
												<span
													...
													:key="'chieu_sau_khu_dat' + indexItem"
													>{{
														formatSentenceCase(comparison_factor.asset_title)
													}}m</span
												>
										...
										<tr
											...
											:key="'DepthChangeRow' + index"
										>
											<td>Mức điều chỉnh</td>
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceCskd1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceCskd2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceCskd3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
											...
											:key="'DepthChangeRow2' + index"
										>
											<td><strong>Giá sau điều chỉnh</strong></td>
											<td></td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["chieu_sau_khu_dat"][0])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["chieu_sau_khu_dat"][1])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["chieu_sau_khu_dat"][2])
												}}</strong>
											</td>
										</tr>
										<!-- Adjust Comparison END -->
										...
										<tr
											...
											index) in appraises.comparison_factor[0].comparison_factor.filter(
												comparison_factor_appraise =>
													comparison_factor_appraise.type === 'hinh_dang_dat' &&
													comparison_factor_appraise.status === 1
											)"
											:key="'appraisalLandRow' + index"
										>
											<td rowspan="5">
												<div
													class="btn-delete"
													...
													type="button"
													@click="
														dialogDeleteComparisionDefault('hinh_dang_dat')
													"
												>
													<img
														src="@/assets/icons/ic_delete_2.svg"
										...
										<tr
											...
											:key="'LandChangeRow' + index"
										>
											<td>Mức điều chỉnh</td>
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceHdd1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceHdd2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceHdd3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
											...
											:key="'LandChangeRow2' + index"
										>
											<td><strong>Giá sau điều chỉnh</strong></td>
											<td></td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["hinh_dang_dat"][0])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["hinh_dang_dat"][1])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["hinh_dang_dat"][2])
												}}</strong>
											</td>
										</tr>
										<!-- Adjust Comparison END -->
										...
										<tr
											...
											index) in appraises.comparison_factor[0].comparison_factor.filter(
												comparison_factor_appraise =>
													comparison_factor_appraise.type === 'ket_cau_duong' &&
													comparison_factor_appraise.status === 1
											)"
											:key="'ketcauduong' + index"
										>
											<td rowspan="5">
												<div
													class="btn-delete"
													...
													type="button"
													@click="
														dialogDeleteComparisionDefault('ket_cau_duong')
													"
												>
													<img
														src="@/assets/icons/ic_delete_2.svg"
										...
										<tr
											...
											:key="'ketcauduong3C' + index"
										>
											<td>Hệ số tương quan</td>
											<td>100%</td>
											<td
												v-for="(asset, indexAsset) in appraises.asset_general"
												:key="'inputKetcauduong' + indexAsset"
											>
												<div
													v-for="(comparison_factor,
													index) in appraises.comparison_factor[
										...
										<tr
											...
											:key="'ketcauduong5' + index"
										>
											<td>Mức điều chỉnh</td>
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceKcd1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceKcd2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceKcd3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
											...
											:key="'ketcauduong6' + index"
										>
											<td><strong>Giá sau điều chỉnh</strong></td>
											<td></td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["ket_cau_duong"][0])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["ket_cau_duong"][1])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["ket_cau_duong"][2])
												}}</strong>
											</td>
										</tr>
										<!-- Adjust Comparison END -->
										...
										<tr
											...
											index) in appraises.comparison_factor[0].comparison_factor.filter(
												comparison_factor_appraise =>
													comparison_factor_appraise.type === 'do_rong_duong' &&
													comparison_factor_appraise.status === 1
											)"
											:key="'appraisalStreetRow' + index"
										>
											<td rowspan="5">
												<div
													class="btn-delete"
													...
													type="button"
													@click="
														dialogDeleteComparisionDefault('do_rong_duong')
													"
												>
													<img
														src="@/assets/icons/ic_delete_2.svg"
										...
										<tr
											...
											:key="'StreetRowC' + index"
										>
											<td>Hệ số tương quan</td>
											<td>100%</td>
											<td
												v-for="(asset, indexAsset) in appraises.asset_general"
												:key="'inputStreet' + indexAsset"
											>
												<div
													v-for="(comparison_factor,
													index) in appraises.comparison_factor[
										...
										<tr
											...
											:key="'StreetChangeRow' + index"
										>
											<td>Mức điều chỉnh</td>
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceDrd1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceDrd2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceDrd3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
											...
											:key="'StreetChangeRow2' + index"
										>
											<td><strong>Giá sau điều chỉnh</strong></td>
											<td></td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["do_rong_duong"][0])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["do_rong_duong"][1])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["do_rong_duong"][2])
												}}</strong>
											</td>
										</tr>
										<!-- Adjust Comparison END -->
										...
										<tr
											...
											:key="'appraisalConditionsRow' + index"
										>
											<td rowspan="5">
												<div
													class="btn-delete"
													...
													type="button"
													@click="
														dialogDeleteComparisionDefault('dieu_kien_ha_tang')
													"
												>
													<img
														src="@/assets/icons/ic_delete_2.svg"
											...
											>
												<span
													...
													:key="'dieu_kien_ha_tang' + indexItem"
													>{{
														formatSentenceCase(comparison_factor.asset_title)
													}}</span
												>
										...
										<tr
											...
											:key="'ConditionsRowC' + index"
										>
											<td>Hệ số tương quan</td>
											<td>100%</td>
											<td
												v-for="(asset, indexAsset) in appraises.asset_general"
												:key="'inputConditions' + indexAsset"
											>
												<div
													v-for="(comparison_factor,
													index) in appraises.comparison_factor[
										...
										<tr
											...
											:key="'ConditionsChangeRow' + index"
										>
											<td>Mức điều chỉnh</td>
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceDkht1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceDkht1).toFixed(0)) }}
											</td>
										...
										<tr
											...
											:key="'ConditionsChangeRow2' + index"
										>
											<td><strong>Giá sau điều chỉnh</strong></td>
											<td></td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["dieu_kien_ha_tang"][0])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["dieu_kien_ha_tang"][1])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["dieu_kien_ha_tang"][2])
												}}</strong>
											</td>
										</tr>
										<!-- Adjust Comparison END -->
										...
										<tr
											...
											index) in appraises.comparison_factor[0].comparison_factor.filter(
												comparison_factor_appraise =>
													comparison_factor_appraise.type === 'kinh_doanh' &&
													comparison_factor_appraise.status === 1
											)"
											:key="'appraisalBusinessRow' + index"
										>
											<td rowspan="5">
												<div
													class="btn-delete"
													v-if="isEditStatus"
													type="button"
													@click="dialogDeleteComparisionDefault('kinh_doanh')"
												>
													<img
														src="@/assets/icons/ic_delete_2.svg"
														alt="save"
										...
										<tr
											...
											:key="'BusinessRowC' + index"
										>
											<td>Hệ số tương quan</td>
											<td>100%</td>
											<td
												v-for="(asset, indexAsset) in appraises.asset_general"
												:key="'inputBusiness' + indexAsset"
											>
												<div
													v-for="(comparison_factor,
													index) in appraises.comparison_factor[
										...
										<tr
											...
											:key="'BusinessChangeRow' + index"
										>
											<td>Mức điều chỉnh</td>
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceKd1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceKd2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceKd3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
											...
											:key="'kinh_doanh2' + index"
										>
											<td><strong>Giá sau điều chỉnh</strong></td>
											<td></td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["kinh_doanh"][0])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["kinh_doanh"][1])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["kinh_doanh"][2])
												}}</strong>
											</td>
										</tr>
										<!-- Adjust Comparison END -->
										...
										<tr
											...
											:key="'appraisalSecurityRow' + index"
										>
											<td rowspan="5">
												<div
													class="btn-delete"
											...
											>
												<span
													...
													:key="'an_ninh_moi_truong_song' + indexItem"
													>{{
														formatSentenceCase(comparison_factor.asset_title)
													}}</span
												>
										...
										<tr
											...
											:key="'SecurityRowC' + index"
										>
											<td>Hệ số tương quan</td>
											<td>100%</td>
											<td
												v-for="(asset, indexAsset) in appraises.asset_general"
												:key="'inputSecurity' + indexAsset"
											>
												<div
													v-for="(comparison_factor,
													index) in appraises.comparison_factor[
													...
													:key="'an_ninh_moi_truong_songC' + index"
												>
													<InputPercent
										...
										>
											...
											>
												<div
													...
													:key="'an_ninh_moi_truong_song' + index"
												>
													<InputPercentNegative
										...
										<tr
											...
											:key="'SecurityChangeRow' + index"
										>
											<td>Mức điều chỉnh</td>
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceAnmts1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceAnmts2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceAnmts3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
										...
										>
											...
											<td>
												<strong>{{
													formatNumber(
														adjustPriceData["an_ninh_moi_truong_song"][0]
													)
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(
														adjustPriceData["an_ninh_moi_truong_song"][1]
													)
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(
														adjustPriceData["an_ninh_moi_truong_song"][2]
													)
												}}</strong>
											</td>
										</tr>
										...
										<tr
											...
											index) in appraises.comparison_factor[0].comparison_factor.filter(
												comparison_factor_appraise =>
													comparison_factor_appraise.type === 'phong_thuy' &&
													comparison_factor_appraise.status === 1
											)"
											:key="'appraisalFengShuiRow' + index"
										>
											<td rowspan="5">
												<div
													class="btn-delete"
													v-if="isEditStatus"
													type="button"
													@click="dialogDeleteComparisionDefault('phong_thuy')"
												>
													<img
														src="@/assets/icons/ic_delete_2.svg"
														alt="save"
										...
										<tr
											...
											:key="'FengShuiRowC' + index"
										>
											<td>Hệ số tương quan</td>
											<td>100%</td>
											<td
												v-for="(asset, indexAsset) in appraises.asset_general"
												:key="'inputFengShui' + indexAsset"
											>
												<div
													v-for="(comparison_factor,
													index) in appraises.comparison_factor[
										...
										<tr
											...
											:key="'TrafficChangeRow' + index"
										>
											<td>Mức điều chỉnh</td>
											<td></td>
											<td>
												{{ formatNumber(parseFloat(pricePt1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(pricePt2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(pricePt3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
											...
											:key="'TrafficChangeRow2' + index"
										>
											<td><strong>Giá sau điều chỉnh</strong></td>
											<td></td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["phong_thuy"][0])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["phong_thuy"][1])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["phong_thuy"][2])
												}}</strong>
											</td>
										</tr>
										<!-- Adjust Comparison END -->
										...
										<tr
											...
											index) in appraises.comparison_factor[0].comparison_factor.filter(
												comparison_factor_appraise =>
													comparison_factor_appraise.type === 'giao_thong' &&
													comparison_factor_appraise.status === 1
											)"
											:key="'appraisalTrafficRow' + index"
										>
											<td rowspan="5">
												<div
													class="btn-delete"
													v-if="isEditStatus"
													type="button"
													@click="dialogDeleteComparisionDefault('giao_thong')"
												>
													<img
														src="@/assets/icons/ic_delete_2.svg"
														alt="save"
										...
										<tr
											...
											:key="'TrafficRowC' + index"
										>
											<td>Hệ số tương quan</td>
											<td>100%</td>
											<td
												v-for="(asset, indexAsset) in appraises.asset_general"
												:key="'inputTraffic' + indexAsset"
											>
												<div
													v-for="(comparison_factor,
													index) in appraises.comparison_factor[
										...
										>
											...
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceGt1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceGt2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceGt3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
										...
										>
											...
											<td>
												<strong>{{
													formatNumber(adjustPriceData["giao_thong"][0])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["giao_thong"][1])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["giao_thong"][2])
												}}</strong>
											</td>
										</tr>
										<!-- Adjust Comparison END -->
										...
										<tr
											...
											index) in appraises.comparison_factor[0].comparison_factor.filter(
												comparison_factor_appraise =>
													comparison_factor_appraise.type === 'quy_hoach' &&
													comparison_factor_appraise.status === 1
											)"
											:key="'quyhoach' + index"
										>
											<td rowspan="3">
												<div
													...
													v-if="isEditStatus"
													type="button"
													@click="dialogDeleteComparisionDefault('quy_hoach')"
												>
													<img
														src="@/assets/icons/ic_delete_2.svg"
														alt="save"
										...
										<tr
											...
											:key="'quyhoach3C' + index"
										>
											<td>Hệ số tương quan</td>
											<td>100%</td>
											<td
												v-for="(asset, indexAsset) in appraises.asset_general"
												:key="'inputQuyhoach' + indexAsset"
											>
												<div
													v-for="(comparison_factor,
													index) in appraises.comparison_factor[
										...
										>
											...
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceQh1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceQh2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceQh3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
										...
										>
											...
											<td>
												<strong>{{
													formatNumber(adjustPriceData["quy_hoach"][0])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["quy_hoach"][1])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["quy_hoach"][2])
												}}</strong>
											</td>
										</tr>
										<!-- Adjust Comparison END -->
										...
										>
											...
											>
												<span
													...
													:key="'dieu_kien_thanh_toan' + indexItem"
													>{{
														formatSentenceCase(comparison_factor.asset_title)
													}}</span
												>
										...
										>
											...
											>
												<div
													...
													:key="'dieu_kien_thanh_toanC' + index"
												>
													<InputPercent
										...
										>
											...
											>
												<div
													...
													:key="'dieu_kien_thanh_toan' + index"
												>
													<InputPercentNegative
										...
										<tr
											...
											:key="'PayChangeRow' + index"
										>
											<td>Mức điều chỉnh</td>
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceDktt1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceDktt2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceDktt3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
										...
										>
											...
											<td>
												<strong>{{
													formatNumber(
														adjustPriceData["dieu_kien_thanh_toan"][0]
													)
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(
														adjustPriceData["dieu_kien_thanh_toan"][1]
													)
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(
														adjustPriceData["dieu_kien_thanh_toan"][2]
													)
												}}</strong>
											</td>
										</tr>
										...
										<tr
											...
											index) in appraises.comparison_factor[0].comparison_factor.filter(
												comparison_factor_appraise =>
													comparison_factor_appraise.type === 'vi_tri' &&
													comparison_factor_appraise.status === 1
											)"
											:key="'appraisalLocateRow' + index"
										>
											<td rowspan="5">
												<div
													class="btn-delete"
													v-if="isEditStatus"
													type="button"
													@click="dialogDeleteComparisionDefault('vi_tri')"
												>
													<img
														src="@/assets/icons/ic_delete_2.svg"
														alt="save"
										...
										<tr
											...
											:key="'LocateRowC' + index"
										>
											<td>Hệ số tương quan</td>
											<td>100%</td>
											<td
												v-for="(asset, indexAsset) in appraises.asset_general"
												:key="'inputLocate' + indexAsset"
											>
												<div
													v-for="(comparison_factor,
													index) in appraises.comparison_factor[
										...
										<tr
											...
											:key="'LocateChangeRow' + index"
										>
											<td>Mức điều chỉnh</td>
											<td></td>
											<td>
												{{ formatNumber(parseFloat(priceVitri1).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceVitri2).toFixed(0)) }}
											</td>
											<td>
												{{ formatNumber(parseFloat(priceVitri3).toFixed(0)) }}
											</td>
										</tr>
										<tr
											v-for="(comparison_factor_appraise,
											...
											:key="'LocateChangeRow2' + index"
										>
											<td><strong>Giá sau điều chỉnh</strong></td>
											<td></td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["vi_tri"][0])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["vi_tri"][1])
												}}</strong>
											</td>
											<td>
												<strong>{{
													formatNumber(adjustPriceData["vi_tri"][2])
												}}</strong>
											</td>
										</tr>
										<!-- Adjust Comparison END -->
										...
										<tr
											v-if="
												showOtherFactor &&
													...
													data_other_comparison.length > 0
											"
												...
												<table class="table_of_other_comparison color_content">
													<tr style="width: 100% !important">
														<td rowspan="5">
															<div
																...
																type="button"
																@click="
																	handleDialogDeleteComparision(
																		index,
																		data_appraise.other_factor_asset
																	)
																"
															>
																<img
														...
														<td
															class="td_other_asset_title"
															v-for="(data_asset,
															indexItem) in data_appraise.other_factor_asset"
															:key="'yeutokhac2' + indexItem"
														>
															<div>
																<InputText
																	...
																	styleInput="text-align:center"
																	@change="
																		handleChangeTitleAsset($event, data_asset)
																	"
																/>
															</div>
														</td>
													...
													<tr v-if="showOtherFactor">
														...
														<td
															class="td_other_asset_title"
															v-for="(rate_asset,
															index) in data_appraise.other_factor_asset"
															:key="'yeutokhac4C' + index"
														>
															<div>
																<InputPercent
																	:key="key_render_111"
																	...
																	:text_center="true"
																	@change="
																		changeOtherCoefficient($event, rate_asset)
																	"
																/>
															</div>
														</td>
													...
													<tr v-if="showOtherFactor">
														...
														<td
															class="td_other_asset_title"
															v-for="(price_other,
															index) in price_other_comparison
																? price_other_comparison[index]
																: []"
															:key="'yeutokhacprice' + index"
														>
															{{
																formatNumber(
																	parseFloat(
																		price_other.indication_price_asset
																	).toFixed(0)
																) || 0
															}}
														</td>
													...
													<tr v-if="showOtherFactor">
														...
														<td
															...
															index) in price_other_comparison
																? price_other_comparison[index]
																: []"
															:key="'yeutokhacprice2' + index"
														>
															{{
																formatNumber(
																	parseFloat(
																		price_other.adjust_price_asset
																	).toFixed(0)
																) || 0
															}}
														</td>
										...
										<tr v-if="isEditStatus" class="other_button_container">
											...
											<td class="td_none" colspan="5">
												<div class="container_btn_add">
													<button
														class="btn text-warning btn-ghost other_button"
														type="button"
														@click="handleAddOtherFactor"
													>
														+ Thêm yếu tố khác
													</button>
												</div>
										...
										<tr>
											<td>3</td>
											<td colspan="2">
												Mức giá trung bình của các mức giá chỉ dẫn
												(đồng/m<sup>2</sup>)
											</td>
										...
										<tr>
											...
											<td
												...
												"
											>
												Mức độ chênh lệch với mức giá trung bình của các mức giá
												chỉ dẫn (%)
											</td>
											<td
												:class="mgcl1 > 15 || mgcl1 < -15 ? 'text-danger' : ''"
											>
												{{ mgcl1 + "%" }}
											</td>
											<td
												:class="mgcl2 > 15 || mgcl2 < -15 ? 'text-danger' : ''"
											>
												{{ mgcl2 + "%" }}
											</td>
											<td
												:class="mgcl3 > 15 || mgcl3 < -15 ? 'text-danger' : ''"
											>
												{{ mgcl3 + "%" }}
											</td>
										</tr>
										...
										<tr>
											...
											<td
												v-for="(asset, index_asset) in appraises.asset_general"
												:key="'asset_area_adjusted' + index_asset"
											>
												<div
													v-for="(percent, index) in area_adjusted"
													:key="'percent_adjust' + index"
													v-if="percent.id === asset.id"
												>
													{{
														`${checkMin(area_adjusted[asset.id])} - ${checkMax(
															area_adjusted[asset.id]
														)}`
													}}
												</div>
											</td>
										...
										<tr>
											<td>8</td>
											<td colspan="2">
												Tổng giá trị điều chỉnh thuần (đ/m<sup>2</sup>)
											</td>
										...
										<tr>
											...
											<td colspan="2">
												<div class="d-flex col-12 col-lg-12">
													...
													<div class="d-flex col-8 col-lg-8">
														<strong style="margin-top: 2px; margin-right: 30px"
															>Thống nhất mức giá chỉ dẫn</strong
														>
											...
											<td colspan="3" align="center">
												<strong>{{ formatNumber(getMainPrice) }}</strong>
											</td>
										</tr>
										...
										<tr>
											...
											<td colspan="2">
												<div
													class="d-flex col-12 col-lg-12 align-items-center container_round"
												>
											...
											<td colspan="3" align="center">
												{{ formatNumber(calMainRoundPrice) }} đ
											</td>
										</tr>

										<tr
											v-for="(purpose, index) in purposePrice"
											:key="'purposePrice' + index"
										>
											<td>
												{{
													purposePrice.length > 1 ? 11 + "." + (index + 1) : 11
												}}
											</td>
											<td colspan="2">
												<div>Đơn giá đất {{ purpose.acronym }} thị trường</div>
												<div>({{ type_method }})</div>
											</td>
											<td>
												<div>
											...
											<td>
												<div
													class="d-flex col-12 col-lg-12 align-items-center justify-content-center"
												>
													<a class="pr-3">Làm tròn</a>
													<div style="width: 70px">
														<InputNumberFormat
											...
											<td>
												{{ `${formatNumber(calPurposePriceByIndex(index))} đ` }}
											</td>
										</tr>
										<tr
											v-for="(violate, index) in violatePrice"
											:key="'violatePrice' + index"
										>
											<td>
												{{
													purposePrice.length > 0
														? violatePrice.length > 1
															? 12 + "." + (index + 1)
															: 12
														: violatePrice.length > 1
														? 11 + "." + (index + 1)
											...
											<td>
												{{
													`${formatNumber(
														roundPrice(violate.price, violate.round)
													)} đ`
												}}
											</td>
										</tr>
											...
											<table class="table_result_adjust">
												...
												<tbody>
													...
													<tr v-if="mainPrice">
														<td
															style="width:28%"
															:rowspan="purposePrice.length + 1"
														>
															Phần diện tích đất phù hợp quy hoạch
														</td>
														<td>{{ mainPrice.acronym }}</td>
														<td class="text-right">
															{{ formatNumber(mainArea) }}
														</td>
														<td class="text-right">
															{{ formatNumber(calMainRoundPrice) }}
														</td>
														<td class="text-right">
															{{ formatNumber(calMainTotalPrice) }}
														</td>
													</tr>
													<tr
														v-for="(purpose, index) in purposePrice"
													...
													>
														...
														<td class="text-right">
															{{ formatNumber(purpose.area.toFixed(2)) }}
														</td>
														<td class="text-right">
															{{
																formatNumber(
																	roundPrice(purpose.price, purpose.round)
																)
															}}
														</td>
														<td class="text-right">
													...
													>
														<td
															style="width:28%"
															v-if="index === 0"
															:rowspan="violatePrice.length"
														>
															Phần diện tích đất vi phạm quy hoạch
														</td>
														<td>{{ violate.acronym }}</td>
														<td class="text-right">
															{{ formatNumber(violate.area.toFixed(2)) }}
														</td>
														<td class="text-right">
															{{
																formatNumber(
								...
								>
									<div class="d-md-flex d-block">
										...
										<button
											...
											:class="{ 'btn_loading disabled': isSubmit }"
											class="btn btn-white btn-orange text-nowrap"
											@click.prevent="handleSaveTab2Ver2"
										>
											<img
												src="@/assets/icons/ic_save.svg"
												style="margin-right: 12px"
				...
				<ModalNotificationAppraisal
					...
					v-bind:notification="
						'Độ chênh lệch của mức giá trung bình với các mức giá chỉ dẫn đang lớn hơn 15% không phù hợp tiêu chuẩn thẩm định giá.\n Bạn có muốn lưu?'
					"
...
export default {
	...
	computed: {
		...
		getTotalArea() {
			let area = 0;
			if (
				this.appraises.properties &&
				this.appraises.properties.length > 0 &&
				this.appraises.properties[0].appraise_land_sum_area
			) {
				area = parseFloat(
					this.appraises.properties[0].appraise_land_sum_area
				).toFixed(2);
		...
		calMainRoundPrice() {
			let mainPrice = this.mainPrice;
			let price = this.roundPrice(
				mainPrice.islayerCuttingPirce
					? mainPrice.layerCuttingPirce
					: mainPrice.price,
				mainPrice.round
			);
			if (this.purposePrice.length > 0) {
				this.calPurposePrice(price, mainPrice.ubndPrice);
			}
			if (this.violatePrice.length > 0) {
				this.calViolatePrice(
					this.violatePrice.find(i => i.isMain === true),
					price
				);
			}
		...
		calMainTotalPrice() {
			let price = 0;
			price =
				price + this.roundPrice(this.calMainRoundPrice * this.mainPrice.area);
		...
		calTangibleAssetPrice() {
			return this.form.price_tangible_asset
				? this.form.price_tangible_asset
				: 0;
		},
		calOtherAssetPrice() {
		...
		calTotalPrice() {
			...
			let price =
				totalPrice.land_asset_price +
				totalPrice.tangible_asset_price +
				totalPrice.other_asset_price;
			return price;
		}
	},
	watch: {
	...
	methods: {
		...
		setDefaultConstructionData() {
			...
			let constructionInAppraisal = appraisal_methods.find(
				i => toLower(i.slug) === "xac_dinh_chat_luong_con_lai"
			);
			if (constructionInAppraisal) {
				constructionRemainQuality.forEach(item => {
					if (
						toLower(item.slug) === toLower(constructionInAppraisal.slug_value)
					) {
			...
			let priceTypeInAppraisal = appraisal_methods.find(
				i => toLower(i.slug) === "xac_dinh_don_gia_xay_dung"
			);
		...
		calPurposePriceByIndex(index) {
			...
			let violate = this.violatePrice.find(
				item => item.acronym === purpose.acronym
			);
			if (violate) {
				this.calViolatePrice(violate, price);
			}
			return price;
		},
		calPurposePrice(price, basePrice) {
			let slug = this.appraisal_methods.find(
				i => i.slug === "tinh_gia_dat_hon_hop_con_lai"
			);
			...
			if (slug && slugValue !== "theo-phuong-phap-doc-lap") {
				...
				this.purposePrice.forEach(i => {
					...
					let violate = this.violatePrice.find(
						item => item.acronym === i.acronym
					);
					if (violate) {
						this.calViolatePrice(violate, calPrice);
					}
					i.price = calPrice;
				});
			}
		...
		calViolatePrice(violate, price) {
			if (violate) {
				let slug = this.appraisal_methods.find(
					i => i.slug === "tinh_gia_dat_vi_pham_quy_hoach"
				);
				if (slug) {
					let value = slug.value;
					let slugValue = slug.slug_value;
		...
		getMainLandType() {
			let item = this.form.properties[0].property_detail.find(
				i => i.is_transfer_facility === true
			);
			let data = {};
			if (item) {
		...
		getPurposeLandType() {
			let detail = this.form.properties[0].property_detail.filter(
				i => i.is_transfer_facility === false && i.main_area > 0
			);
			let data = [];
			if (detail.length > 0) {
				detail.forEach(item => {
					...
					let slug_price = slug + "_price";
					let slug_round = slug + "_round";
					let landPriceData = this.form.asset_price.find(
						i => i.slug === slug_price
					);
					let roundData = this.form.asset_price.find(
						i => i.slug === slug_round
					);
					let price = landPriceData ? landPriceData.value : 0;
					let round = roundData && roundData.value ? roundData.value : 0;
		...
		getComparisonTitleByType(data, type, isAsset = true) {
			...
			if (factor) {
				if (isAsset) {
					return factor.asset_title;
				} else {
					return factor.appraise_title;
				}
			} else {
				return "-";
			}
		...
		remainingBuildingPrice(tangible_assets) {
			if (
				tangible_assets &&
				tangible_assets.unit_price_m2 &&
				tangible_assets.remaining_quality &&
				tangible_assets.total_construction_base
			) {
				return (
					this.formatNumber(
						floor(
		...
		changePercentSaleRating(event, index) {
			...
			if (event) {
				...
				this.form.appraise_adapter[index].change_negotiated_price =
					(this.form.asset_general[index].total_amount * event) / 100 -
					this.form.asset_general[index].total_amount;
				console.log(
					"this.form.appraise_adapter[index]",
					this.form.appraise_adapter[index]
				);
				this.key_render_1++;
			}
		...
		changeNegotiatedPrice(event, index) {
			...
			if (event) {
				...
				this.form.appraise_adapter[index].percent = (
					((this.form.asset_general[index].total_amount + event) /
						this.form.asset_general[index].total_amount) *
					100
				).toFixed(2);
				console.log(
		...
		checkRemainValue() {
			if (
				this.form.composite_land_remaning_slug ===
				"theo-ty-le-gia-dat-co-so-chinh"
			) {
		...
		getData() {
			...
			if (
				+this.form.round_violation_facility ||
				+this.form.round_violation_facility === 0
			) {
			...
			if (
				+this.form.round_violation_composite ||
				+this.form.round_violation_composite === 0
			) {
			...
			this.key_render_around += 1;
			if (
				this.form.properties[0].property_detail &&
				this.form.properties[0].property_detail.length > 1
			) {
				this.type_is_not_facility = this.form.properties[0].property_detail.find(
					property_detail => property_detail.is_transfer_facility === false
				).land_type_purpose.acronym;
			}
			...
			if (typeof appraise.asset_general !== "undefined") {
				appraise.comparison_factor.forEach((data, index) => {
					...
					if (
						data.adjust_coefficient === "100" &&
						data.adjust_percent !== "0"
					) {
						// console.log('lấy data cũ', index, data.adjust_percent)
						appraise.comparison_factor[index].adjust_coefficient = (
							(100 /
								(parseFloat(appraise.comparison_factor[index].adjust_percent) +
									100)) *
							100
						).toFixed(0);
				...
				appraise.asset_general.forEach(asset_general => {
					...
					const comparison_factor_TSSS = appraise.comparison_factor.filter(
						comparison => comparison.asset_general_id === asset_general.id
					);
					asset_generals.push({
						id: asset_general.id,
					...
					asset_general.properties.forEach(property => {
						let appraiseHasAsset = appraise.appraise_has_assets.filter(
							item =>
								item.asset_general_id === asset_general.id &&
								item.asset_property_detail_id === property.id
						);
		...
		async getOtherComparison() {
			...
			await this.other_comparison.forEach(item => {
				if (!map.has(item.position)) {
					...
					const comparison_other = this.other_comparison.filter(
						data => data.position === indexTem["id_check"]
					);
					...
					comparison_other.forEach(yeu_to => {
						...
						// console.log('lấy data yếu tố', yeu_to)
						if (
							yeu_to.adjust_coefficient === "100" &&
							yeu_to.adjust_percent !== "0"
						) {
							// console.log('lấy data cũ', yeu_to.adjust_percent)
							yeu_to.adjust_coefficient = (
								(100 / (parseFloat(yeu_to.adjust_percent) + 100)) *
								100
							).toFixed(0);
							// console.log('tính data mới', yeu_to.adjust_coefficient)
						}
			...
			// set otherIndex
			let positionTem = [];
			await this.other_comparison.map(data => {
				positionTem.push(data.position);
			});
			let max = Math.max(...positionTem);
			if (
				positionTem.length === 0 ||
				max === -Infinity ||
				max === Infinity ||
				!max
			) {
		...
		getDataLand() {
			this.appropriate_zoning_land = [];
			this.violation_zoning_land = [];
			this.asset_appropriate_area_arr = [];
			this.total_appraise_area = 0;
			this.total_vio_appraise_area = 0;
			...
			formDataClone.asset_general.forEach(asset => {
				asset.properties.forEach(property_asset => {
					let appraiseHasAsset = formDataClone.appraise_has_assets.filter(
						item => item.asset_property_detail_id === property_asset.id
					);
					if (appraiseHasAsset && appraiseHasAsset.length > 0) {
						property_asset.property_detail.forEach(property_asset_detail => {
							// lay gia tri dat vi pham quy hoach tu asset_unit_area
							let getDataUnitPrice = this.form.asset_unit_area.filter(
								itemVioLand =>
									itemVioLand.asset_general_id ===
										property_asset.asset_general_id &&
									itemVioLand.land_type_id ===
										property_asset_detail.land_type_purpose
							);
							...
							if (getDataUnitPrice && getDataUnitPrice.length > 0) {
								this.asset_appropriate_area_arr.push({
									...
									total_area: parseFloat(
										(
											property_asset_detail.total_area -
											+getDataUnitPrice[0].violation_asset_area
										).toFixed(2)
									),
									name_purpose_land_asset:
										property_asset_detail.land_type_purpose_data.acronym
								});
							}
						});
					}
			...
			if (
				formDataClone.properties &&
				formDataClone.properties[0] &&
				formDataClone.properties[0].property_detail &&
				formDataClone.properties[0].property_detail.length > 0
			) {
				formDataClone.properties[0].property_detail.forEach(
					property_appraise => {
						...
						this.appropriate_zoning_land.push({
							...
							appropriate_appraise_land: Number(
								(
									+property_appraise.total_area -
									+property_appraise.planning_area
								).toFixed(2)
							),
							asset_general_land: []
						});
						...
						this.total_appraise_area += Number(
							(
								+property_appraise.total_area - +property_appraise.planning_area
							).toFixed(2)
						);
			...
			this.asset_appropriate_area_arr.forEach(itemLandAsset => {
				if (!map.has(itemLandAsset.name_purpose_land_asset)) {
					...
					let dataLand = this.asset_appropriate_area_arr.filter(
						item =>
							item.name_purpose_land_asset ===
							itemLandAsset.name_purpose_land_asset
					);
					// check ton tai trong dat phu hop quy hoach
					let checkExist = this.appropriate_zoning_land.filter(
						checkItem =>
							checkItem.name_purpose_land ===
							itemLandAsset.name_purpose_land_asset
					);
					...
					if (checkExist && checkExist.length > 0) {
						...
						this.appropriate_zoning_land.forEach((appropriateLand, index) => {
							if (
								appropriateLand.name_purpose_land ===
								itemLandAsset.name_purpose_land_asset
							) {
								this.appropriate_zoning_land[index][
									"asset_general_land"
								] = dataLand;
							}
						});
			...
			this.form.asset_unit_area.forEach(asset_data => {
				if (!map1.has(asset_data.land_type_id)) {
					...
					const dataViolationLand = this.form.asset_unit_area.filter(
						item => item.land_type_id === asset_data.land_type_id
					);
					// check ton tai trong dat vi pham quy hoach
					let checkExist = this.violation_zoning_land.filter(
						checkItem =>
							checkItem.name_purpose_land === asset_data.land_type_data.acronym
					);
					if (checkExist && checkExist.length > 0) {
						this.violation_zoning_land.forEach((violationLand, index) => {
							if (
								violationLand.name_purpose_land ===
								asset_data.land_type_data.acronym
							) {
								this.violation_zoning_land[index][
									"asset_general_land"
								] = dataViolationLand;
							}
						});
		...
		getDataPrice() {
			...
			let data1_id = asset1.properties[0].asset_general_id;
			let data1_acro = asset1.properties[0].property_detail;
			let data2_id = asset2.properties[0].asset_general_id;
			let data2_acro = asset2.properties[0].property_detail;
			let data3_id = asset3.properties[0].asset_general_id;
			let data3_acro = asset3.properties[0].property_detail;
			let arr_compare = [];
			...
			this.form.asset_unit_price.forEach(itemUnitPrice => {
				if (!map1.has(itemUnitPrice.land_type_id)) {
					...
					this.data_land_price_demo.forEach((itemLandPrice, index) => {
						...
						if (
							itemUnitPrice.land_type_data.acronym == itemLandPrice.purpose_land
						) {
							// // console.log('compare', itemUnitPrice.land_type_data.acronym, itemLandPrice.purpose_land)
							let get_data_filter = this.form.asset_unit_price.filter(
								item_filter =>
									item_filter.land_type_data.acronym ===
									itemLandPrice.purpose_land
							);
							...
							for (let i = 0; i < get_data_filter.length; i++) {
								...
								for (let t = 0; t < arr_compare.length; t++) {
									...
									if (
										check1.asset_general_id === check2.asset_general_id &&
										check1.land_type_data.acronym === check2.acronym
									) {
										count = 1;
									}
								}
							...
							this.data_land_price_demo[index].price_asset.push(
								...get_data_filter
							);
			...
			this.form.asset_unit_price.forEach(itemUnitPrice => {
				if (!map2.has(itemUnitPrice.land_type_id)) {
					...
					let checkDataUnitExist = this.data_land_price_demo.filter(
						item => item.purpose_land === itemUnitPrice.land_type_data.acronym
					);
					if (checkDataUnitExist && checkDataUnitExist.length === 0) {
						let get_data_filter = this.form.asset_unit_price.filter(
							item_filter =>
								item_filter.land_type_data.acronym ===
								itemUnitPrice.land_type_data.acronym
						);
						this.data_land_price_demo.push({
		...
		async handleSaveSummarizationVer1(payloadData) {
			...
			// 	this.isSubmit = true
			// }
			const res = await CertificateAsset.submitStep7ver1(
				payloadData,
				this.idData
			);
			if (res.data) {
		...
		showPriceApproriateFacility(property) {
			if (property) {
				let facilityProperty = property.property_detail.find(
					property_detail => property_detail.is_transfer_facility === true
				);
		...
		showPriceApproriateRemaining(property) {
			if (property) {
				let remaingProperty = property.property_detail.find(
					property_detail => property_detail.is_transfer_facility === false
				);
				let remaining_price = this.formatRoundComposite(
					this.remaining_commerce_price
				);
				let area = +remaingProperty.total_area - +remaingProperty.planning_area;
				this.result_price_appropriate_remaining = (
					remaining_price * area
				).toFixed(0);
		...
		showPriceViolationFacility(property) {
			if (property) {
				...
				let vio_facility_price = this.formatRoundViolationFacility(
					this.violation_facility_price
				);
				let area = +facilityProperty.planning_area;
				this.result_price_violation_facility = (
					vio_facility_price * area
				).toFixed(0);
		...
		showPriceViolationRemaining(property) {
			if (property) {
				...
				let vio_remaining_price = this.formatRoundViolationComposite(
					this.violation_composite_price
				);
				let area = +remaingProperty.planning_area;
				this.result_price_violation_remaining = (
					vio_remaining_price * area
				).toFixed(0);
		...
		showRemainingArea(property) {
			if (property) {
				let violationProperty = property.property_detail.find(
					property_detail => property_detail.is_transfer_facility === false
				);
		...
		showTotal() {
			let result_price_appropriate_facility = +this
				.result_price_appropriate_facility;
			let result_price_appropriate_remaining = +this
				.result_price_appropriate_remaining;
			let result_price_violation_facility = +this
				.result_price_violation_facility;
			let result_price_violation_remaining = +this
				.result_price_violation_remaining;
			let total =
				result_price_appropriate_facility +
				result_price_appropriate_remaining +
				result_price_violation_facility +
		...
		formatRoundComposite(value) {
			if (
				this.round_composite &&
				this.round_composite > 0 &&
				this.round_composite <= 7
			) {
		...
		formatRoundViolationFacility(value) {
			if (
				this.round_violation_facility &&
				this.round_violation_facility > 0 &&
				this.round_violation_facility <= 7
			) {
		...
		formatRoundViolationComposite(value) {
			if (
				this.round_violation_composite &&
				this.round_violation_composite > 0 &&
				this.round_violation_composite <= 7
			) {
		...
		getArrayMin(arr) {
			let arrTemp = [];
			arr.forEach(item => {
				item.adjust_percent = Math.abs(item.adjust_percent);
			});
		...
		async changePriceLand(event, dataItem, indexPrice) {
			await this.form.asset_unit_price.forEach(item => {
				if (
					item.asset_general_id === dataItem.asset_general_id &&
					item.land_type_id === dataItem.land_type_id
				) {
					item.update_value = event;
				}
				item.update = 2;
			});
		...
		async changeMucdichchinh(
			...
			if (event.target.checked == true) {
				...
				const res = await CertificateAsset.updateMucdichchinh(
					data,
					asset_com_1.id
				);
				location.reload();
		...
		async changeViolationLand(event, dataItemLand, indexArea) {
			this.form.asset_unit_area.forEach(item => {
				if (
					item.asset_general_id === dataItemLand.asset_general_id &&
					item.land_type_id === dataItemLand.land_type_id
				) {
					if (+event) {
			...
			await this.getDataLand();
			await this.calculationUpdateMucDichChinh(this.form);
			await this.calculation(this.form);
			this.key_render_1 += 1;
		},
		...
		changeLegalCoefficient(e, indexAsset, type) {
			...
			let factor = this.appraises.comparison_factor[
				indexAsset
			].comparison_factor.find(i => i.type === type);
			if (factor) {
		...
		handleDeleteOtherFactor() {
			...
			if (data && data.length > 0) {
				// data-delete
				const delete_comparison = this.other_comparison.filter(
					item => item.position === data[0].position
				);
				this.delete_other_comparison.push(...delete_comparison);
				// data-remain
				const other_comparison = this.other_comparison.filter(
					item => item.position !== data[0].position
				);
		...
		changeLayerCuttingProcedure(event) {
			...
			if (event === true) {
				this.mainPrice.layerCuttingPirce = parseInt(
					this.mainPrice.price.toFixed(0)
				);
			} else {
			...
			if (data && data.length > 0) {
				this.other_comparison.forEach(item => {
					if (
						...
						item.asset_general_id === data.asset_general_id
					) {
		...
		handleSaveTab2() {
			...
			if (
				this.form.properties[0].property_detail &&
				this.form.properties[0].property_detail.length < 2
			) {
				remaining_price = {};
			}
			if (typeof this.appraises !== "undefined") {
			...
			if (layer_cutting_procedure && !layer_cutting_procedure_price) {
				...
				Math.abs(this.mgcl2) > 15 ||
				Math.abs(this.mgcl3) > 15
			) {
		...
		handleSaveTab2Ver2() {
			...
			const roundMessage =
				"Số làm tròn thuộc khoảng -7 tới 7, vui lòng kiểm tra lại";
		...
		async handleSaveAdjustPlan(payloadData) {
			...
			if (res.data) {
				...
				await this.form.asset_general.forEach(asset_general => {
					const comparison_factor_TSSS = res.data.comparison_factor.filter(
						comparison => comparison.asset_general_id === asset_general.id
					);
		...
		calculationViolationPrice(asset, indexAdapter) {
			...
			this.detail1.property_detail.forEach((item, index) => {
				let filterArea1 = this.form.asset_unit_area.filter(
					item_unit_area =>
						item_unit_area.asset_general_id === this.detail1.asset_general_id
				);
				let filterPrice1 = this.form.asset_unit_price.filter(
					item_unit_price =>
						item_unit_price.asset_general_id === this.detail1.asset_general_id
				);
				if (filterArea1 && filterArea1.length > 0) {
					filterArea1.forEach(item_area_filter1 => {
						...
						filterPrice1.forEach(item_price_filter1 => {
							if (
								item_price_filter1.land_type_id ===
								item_area_filter1.land_type_id
							) {
								price_violation1.push(
									+item_price_filter1.update_value *
										+item_area_filter1.violation_asset_area
								);
							}
						});
					});
				...
				if (price_violation1.length > 0 && price_violation1.length > 1) {
					totalPriceViolationArea1 = price_violation1.reduce(
						(a, b) => a + b,
						0
					);
					price_violation1 = [];
				} else if (
					price_violation1.length > 0 &&
					price_violation1.length === 1
				) {
			...
			this.detail2.property_detail.forEach((item, index) => {
				let filterArea2 = this.form.asset_unit_area.filter(
					item_unit_area =>
						item_unit_area.asset_general_id === this.detail2.asset_general_id
				);
				let filterPrice2 = this.form.asset_unit_price.filter(
					item_unit_price =>
						item_unit_price.asset_general_id === this.detail2.asset_general_id
				);
				if (filterArea2 && filterArea2.length > 0) {
					filterArea2.forEach(item_area_filter2 => {
						...
						filterPrice2.forEach(item_price_filter2 => {
							if (
								item_price_filter2.land_type_id ===
								item_area_filter2.land_type_id
							) {
								price_violation2.push(
									+item_price_filter2.update_value *
										+item_area_filter2.violation_asset_area
								);
							}
						});
					});
				...
				if (price_violation2.length > 0 && price_violation2.length > 1) {
					totalPriceViolationArea2 = price_violation2.reduce(
						(a, b) => a + b,
						0
					);
					price_violation2 = [];
				} else if (
					price_violation2.length > 0 &&
					price_violation2.length === 1
				) {
			...
			this.detail3.property_detail.forEach((item, index) => {
				let filterArea3 = this.form.asset_unit_area.filter(
					item_unit_area =>
						item_unit_area.asset_general_id === this.detail3.asset_general_id
				);
				let filterPrice3 = this.form.asset_unit_price.filter(
					item_unit_price =>
						item_unit_price.asset_general_id === this.detail3.asset_general_id
				);
				if (filterArea3 && filterArea3.length > 0) {
					filterArea3.forEach(item_area_filter3 => {
						...
						filterPrice3.forEach(item_price_filter3 => {
							if (
								item_price_filter3.land_type_id ===
								item_area_filter3.land_type_id
							) {
								price_violation3.push(
									+item_price_filter3.update_value *
										+item_area_filter3.violation_asset_area
								);
							}
						});
					});
				...
				if (price_violation3.length > 0 && price_violation3.length > 1) {
					totalPriceViolationArea3 = price_violation3.reduce(
						(a, b) => a + b,
						0
					);
					price_violation3 = [];
				} else if (
					price_violation3.length > 0 &&
					price_violation3.length === 1
				) {
		...
		calculationChangePrice(asset, indexAdapter) {
			...
			this.baseAcronym = "";
			if (
				typeof asset.properties[0] !== "undefined" &&
				typeof asset.properties[0].property_detail !== "undefined" &&
				asset.properties[0].property_detail.length === 1
			) {
				this.baseUnitPrice =
					asset.properties[0].property_detail[0].circular_unit_price;
				this.baseAcronym =
					asset.properties[0].property_detail[0].land_type_purpose.acronym;
			} else {
			...
			this.detail1.property_detail.forEach((item, index) => {
				this.form.asset_unit_price.forEach((itemPrice1, index2) => {
					if (
						this.baseAcronym === itemPrice1.land_type_data.acronym &&
						itemPrice1.asset_general_id === this.detail1.asset_general_id
					) {
				...
				if (item.land_type_purpose_data.acronym !== this.baseAcronym) {
					...
					if (typeof this.form.asset_unit_price !== "undefined") {
						this.form.asset_unit_price.forEach((itemPrice1, index2) => {
							if (
								itemPrice1.asset_general_id === this.detail1.asset_general_id &&
								itemPrice1.land_type_id === item.land_type_purpose &&
								itemPrice1.position_type_id === item.position_type_id
							) {
								if (itemPrice1.update === 2) {
					...
					if (typeof this.form.asset_unit_area !== "undefined") {
						this.form.asset_unit_area.forEach(itemArea1 => {
							if (
								itemArea1.asset_general_id === this.detail1.asset_general_id &&
								itemArea1.land_type_id === item.land_type_purpose &&
								itemArea1.position_type_id === item.position_type_id
							) {
								appropriate_area =
									item.total_area - itemArea1.violation_asset_area;
							}
						});
					}
					let unitPrice =
						price_facility_asset1 - parseFloat(circular_unit_price);
			...
			this.detail2.property_detail.forEach((item, index) => {
				this.form.asset_unit_price.forEach((itemPrice2, index2) => {
					if (
						this.baseAcronym === itemPrice2.land_type_data.acronym &&
						itemPrice2.asset_general_id === this.detail2.asset_general_id
					) {
				...
				if (item.land_type_purpose_data.acronym !== this.baseAcronym) {
					...
					if (typeof this.form.asset_unit_price !== "undefined") {
						this.form.asset_unit_price.forEach((itemPrice2, index2) => {
							if (
								itemPrice2.asset_general_id === this.detail2.asset_general_id &&
								itemPrice2.land_type_id === item.land_type_purpose &&
								itemPrice2.position_type_id === item.position_type_id
							) {
								if (itemPrice2.update === 2) {
					...
					if (typeof this.form.asset_unit_area !== "undefined") {
						this.form.asset_unit_area.forEach(itemArea2 => {
							if (
								itemArea2.asset_general_id === this.detail2.asset_general_id &&
								itemArea2.land_type_id === item.land_type_purpose &&
								itemArea2.position_type_id === item.position_type_id
							) {
								appropriate_area =
									item.total_area - itemArea2.violation_asset_area;
							}
						});
					}
					let unitPrice =
						price_facility_asset2 - parseFloat(circular_unit_price);
			...
			this.detail3.property_detail.forEach((item, index) => {
				this.form.asset_unit_price.forEach((itemPrice3, index3) => {
					if (
						this.baseAcronym === itemPrice3.land_type_data.acronym &&
						itemPrice3.asset_general_id === this.detail3.asset_general_id
					) {
				...
				if (item.land_type_purpose_data.acronym !== this.baseAcronym) {
					...
					if (typeof this.form.asset_unit_price !== "undefined") {
						this.form.asset_unit_price.forEach((itemPrice3, index3) => {
							if (
								itemPrice3.asset_general_id === this.detail3.asset_general_id &&
								itemPrice3.land_type_id === item.land_type_purpose &&
								itemPrice3.position_type_id === item.position_type_id
							) {
								if (itemPrice3.update === 2) {
					...
					if (typeof this.form.asset_unit_area !== "undefined") {
						this.form.asset_unit_area.forEach(itemArea3 => {
							if (
								itemArea3.asset_general_id === this.detail3.asset_general_id &&
								itemArea3.land_type_id === item.land_type_purpose &&
								itemArea3.position_type_id === item.position_type_id
							) {
								appropriate_area =
									item.total_area - itemArea3.violation_asset_area;
							}
						});
					}
					let unitPrice =
						price_facility_asset3 - parseFloat(circular_unit_price);
		...
		async calculation(asset, indexPrice = null, isChangeMDSD = null) {
			...
			let asset_percent1 =
				typeof asset.appraise_adapter[0] !== "undefined"
					? asset.appraise_adapter[0].percent
					: null;
			let asset_percent2 =
				typeof asset.appraise_adapter[1] !== "undefined"
					? asset.appraise_adapter[1].percent
					: null;
			let asset_percent3 =
				typeof asset.appraise_adapter[2] !== "undefined"
					? asset.appraise_adapter[2].percent
					: null;

			let asset_neo_price1 =
				typeof asset.appraise_adapter[0] !== "undefined"
					? asset.appraise_adapter[0].change_negotiated_price
					: null;
			let asset_neo_price2 =
				typeof asset.appraise_adapter[1] !== "undefined"
					? asset.appraise_adapter[1].change_negotiated_price
					: null;
			let asset_neo_price3 =
				typeof asset.appraise_adapter[2] !== "undefined"
			...
			if (asset_percent1 !== 100 && asset_neo_price1 === null) {
				console.log("tính 1");
				asset_neo_price1 =
					(asset1.total_amount * asset_percent1) / 100 - asset1.total_amount;
				asset.appraise_adapter[0].change_negotiated_price =
					(asset1.total_amount * asset_percent1) / 100 - asset1.total_amount;
			}
			if (asset_percent2 !== 100 && asset_neo_price2 === null) {
				console.log("tính 2");
				asset_neo_price2 =
					(asset2.total_amount * asset_percent2) / 100 - asset2.total_amount;
				asset.appraise_adapter[1].change_negotiated_price =
					(asset2.total_amount * asset_percent2) / 100 - asset2.total_amount;
			}
			...
			if (asset_percent3 !== 100 && asset_neo_price3 === null) {
				console.log("tính 3");
				asset_neo_price3 =
					(asset3.total_amount * asset_percent3) / 100 - asset3.total_amount;
				asset.appraise_adapter[2].change_negotiated_price =
					(asset3.total_amount * asset_percent3) / 100 - asset3.total_amount;
			}
			...
			// detail contruction
			this.dtsxd1 =
				typeof asset1.tangible_assets[0] !== "undefined"
					? asset1.tangible_assets[0].total_construction_base
					: 0;
			this.dtsxd2 =
				typeof asset2.tangible_assets[0] !== "undefined"
					? asset2.tangible_assets[0].total_construction_base
					: 0;
			this.dtsxd3 =
				typeof asset3.tangible_assets[0] !== "undefined"
					? asset3.tangible_assets[0].total_construction_base
					: 0;
			...
			this.form.asset_general.forEach(assetItem => {
				let violationArray = this.form.asset_unit_area.filter(
					item => item.asset_general_id === assetItem.id
				);
			...
			this.form.asset_general.forEach(asset => {
				asset.properties.forEach(property_asset => {
					let appraiseHasAsset = this.form.appraise_has_assets.filter(
						item => item.asset_property_detail_id === property_asset.id
					);
					if (appraiseHasAsset && appraiseHasAsset.length > 0) {
						this.arrayTotalAppropriateArea[asset.id] = parseFloat(
							(
								property_asset.asset_general_land_sum_area -
								this.arrayTotalViolationArea[asset.id]
							).toFixed(2)
						);
						property_asset.property_detail.forEach(property_asset_detail => {
							let getDataUnitPrice = this.form.asset_unit_area.filter(
			...
			this.dgxd1 =
				typeof asset1.tangible_assets[0] !== "undefined"
					? asset1.tangible_assets[0].unit_price_m2
					: 0;
			this.dgxd2 =
				typeof asset2.tangible_assets[0] !== "undefined"
					? asset2.tangible_assets[0].unit_price_m2
					: 0;
			this.dgxd3 =
				typeof asset3.tangible_assets[0] !== "undefined"
					? asset3.tangible_assets[0].unit_price_m2
					: 0;

			// remaining percent
			this.clcl1 =
				typeof asset1.tangible_assets[0] !== "undefined"
					? asset1.tangible_assets[0].remaining_quality
					: 0;
			this.clcl2 =
				typeof asset2.tangible_assets[0] !== "undefined"
					? asset2.tangible_assets[0].remaining_quality
					: 0;
			this.clcl3 =
				typeof asset3.tangible_assets[0] !== "undefined"
					? asset3.tangible_assets[0].remaining_quality
					: 0;
			...
			let totalPriceViolationArea1 =
				typeof asset.appraise_adapter[0] !== "undefined"
					? +asset.appraise_adapter[0].change_violate_price
					: null;
			let totalPriceViolationArea2 =
				typeof asset.appraise_adapter[0] !== "undefined"
					? +asset.appraise_adapter[1].change_violate_price
					: null;
			let totalPriceViolationArea3 =
				typeof asset.appraise_adapter[0] !== "undefined"
					? +asset.appraise_adapter[2].change_violate_price
					: null;

			// declare purpose converting price
			let change_purpose_price1 =
				typeof asset.appraise_adapter[0] !== "undefined"
					? +asset.appraise_adapter[0].change_purpose_price
					: null;
			let change_purpose_price2 =
				typeof asset.appraise_adapter[1] !== "undefined"
					? +asset.appraise_adapter[1].change_purpose_price
					: null;
			let change_purpose_price3 =
				typeof asset.appraise_adapter[2] !== "undefined"
					? +asset.appraise_adapter[2].change_purpose_price
					: null;
			...
			// }
			if (asset_neo_price1) {
				this.totalPriceEstimate1 =
					typeof asset1.total_amount !== "undefined"
						? asset1.total_amount + asset_neo_price1
						: 0;
				this.totalPrice1 =
					(typeof asset1.total_estimate_amount !== "undefined"
						? asset1.total_amount + asset_neo_price1
						: 0) -
					totalPriceViolationArea1 +
					+change_purpose_price1 -
					floor((this.dgxd1 * this.dtsxd1 * this.clcl1) / 100) -
					+otherAmount1;
			} else {
				// tính Tổng giá trị tài sản ước tính
				this.totalPriceEstimate1 =
					typeof asset1.total_amount !== "undefined"
						? (asset_percent1 * asset1.total_amount) / 100
						: 0;
			...
			if (asset_neo_price2) {
				this.totalPriceEstimate2 =
					typeof asset1.total_amount !== "undefined"
						? asset2.total_amount + asset_neo_price2
						: 0;
				this.totalPrice2 =
					(typeof asset2.total_estimate_amount !== "undefined"
						? asset2.total_amount + asset_neo_price2
						: 0) -
					totalPriceViolationArea2 +
					+change_purpose_price2 -
					floor((this.dgxd2 * this.dtsxd2 * this.clcl2) / 100) -
					+otherAmount2;
			} else {
				this.totalPriceEstimate2 =
					typeof asset1.total_amount !== "undefined"
						? (asset_percent2 * asset2.total_amount) / 100
						: 0;
				this.totalPrice2 =
					(typeof asset2.total_estimate_amount !== "undefined"
						? (asset_percent2 * asset2.total_amount) / 100
			...
			if (asset_neo_price3) {
				this.totalPriceEstimate3 =
					typeof asset1.total_amount !== "undefined"
						? asset3.total_amount + asset_neo_price3
						: 0;
				this.totalPrice3 =
					(typeof asset3.total_estimate_amount !== "undefined"
						? asset3.total_amount + asset_neo_price3
						: 0) -
					totalPriceViolationArea3 +
					+change_purpose_price3 -
					floor((this.dgxd3 * this.dtsxd3 * this.clcl3) / 100) -
					+otherAmount3;
			} else {
				this.totalPriceEstimate3 =
					typeof asset1.total_amount !== "undefined"
						? (asset_percent3 * asset3.total_amount) / 100
						: 0;
				this.totalPrice3 =
					(typeof asset3.total_estimate_amount !== "undefined"
						? (asset_percent3 * asset3.total_amount) / 100
			...
			this.dgd1 =
				typeof this.detail1.asset_general_land_sum_area !== "undefined"
					? this.totalPrice1 /
					  this.arrayTotalAppropriateArea[this.detail1.asset_general_id]
					: 0;
			this.dgd2 =
				typeof this.detail2.asset_general_land_sum_area !== "undefined"
					? this.totalPrice2 /
					  this.arrayTotalAppropriateArea[this.detail2.asset_general_id]
					: 0;
			this.dgd3 =
				typeof this.detail3.asset_general_land_sum_area !== "undefined"
					? this.totalPrice3 /
					  this.arrayTotalAppropriateArea[this.detail3.asset_general_id]
					: 0;
			...
			if (
				...
				this.remaining_commerce_price = parseFloat(
					(this.formatCurrent(this.mgtnTemp) *
						this.form.composite_land_remaning_value) /
						100
				).toFixed(0);
				// console.log('ffsdấdsd', this.asset_appropriate_area_arr)
				...
				if (
					...
					let arena1 =
						this.asset_appropriate_area_arr.filter(
							item =>
								item.name_purpose_land_asset !==
									comparisonFactor1["muc_dich_chinh"].asset_title &&
								item.asset_general_id == asset1.id
					...
					// if (arena1){
					this.quydoivemucdichchinh1 = parseFloat(
						(arena1 * this.form.composite_land_remaning_value) / 100
					).toFixed(2);
					this.sauquydoivemucdichchinh1 = parseFloat(
						(arena1 * this.form.composite_land_remaning_value) / 100 + Tarena1
					).toFixed(2);
					this.dgd1 =
						typeof this.detail1.asset_general_land_sum_area !== "undefined"
							? this.totalPrice1 /
					...
					// }
					// if (arena2){
					this.quydoivemucdichchinh2 = parseFloat(
						(arena2 * this.form.composite_land_remaning_value) / 100
					).toFixed(2);
					this.sauquydoivemucdichchinh2 = parseFloat(
						(arena2 * this.form.composite_land_remaning_value) / 100 + Tarena2
					).toFixed(2);
					this.dgd2 =
						typeof this.detail2.asset_general_land_sum_area !== "undefined"
							? this.totalPrice2 /
					...
					// }
					// if (arena3){
					this.quydoivemucdichchinh3 = parseFloat(
						(arena3 * this.form.composite_land_remaning_value) / 100
					).toFixed(2);
					this.sauquydoivemucdichchinh3 = parseFloat(
						(arena3 * this.form.composite_land_remaning_value) / 100 + Tarena3
					).toFixed(2);
					this.dgd3 =
						typeof this.detail3.asset_general_land_sum_area !== "undefined"
							? this.totalPrice3 /
			...
			this.priceQm3 = 0;
			if (
				typeof comparisonFactor1["quy_mo"] !== "undefined" &&
				comparisonFactor1["quy_mo"].status === 1
			) {
				...
				let price1 = this.roundPrice(
					(percentQm1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentQm2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentQm3 * this.totalPricePL3) / 100,
					0
				);
			...
			this.priceCrmt3 = 0;
			if (
				typeof comparisonFactor1["chieu_rong_mat_tien"] !== "undefined" &&
				comparisonFactor1["chieu_rong_mat_tien"].status === 1
			) {
				let percentCrmt1 =
					+comparisonFactor1["chieu_rong_mat_tien"].adjust_percent || 0;
				let percentCrmt2 =
					+comparisonFactor2["chieu_rong_mat_tien"].adjust_percent || 0;
				let percentCrmt3 =
					+comparisonFactor3["chieu_rong_mat_tien"].adjust_percent || 0;

				// mức điều chỉnh của yếu tố CHIỀU RỘNG MẶT TIỀN
				let price1 = this.roundPrice(
					(percentCrmt1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentCrmt2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentCrmt3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["chieu_rong_mat_tien"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.priceCskd3 = 0;
			if (
				typeof comparisonFactor1["chieu_sau_khu_dat"] !== "undefined" &&
				comparisonFactor1["chieu_sau_khu_dat"].status === 1
			) {
				let percentCskd1 =
					+comparisonFactor1["chieu_sau_khu_dat"].adjust_percent || 0;
				let percentCskd2 =
					+comparisonFactor2["chieu_sau_khu_dat"].adjust_percent || 0;
				let percentCskd3 =
					+comparisonFactor3["chieu_sau_khu_dat"].adjust_percent || 0;

				// mức điều chỉnh của yếu tố CHIỀU SÂU KHU ĐẤT
				let price1 = this.roundPrice(
					(percentCskd1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentCskd2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentCskd3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["chieu_sau_khu_dat"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.priceHdd3 = 0;
			if (
				typeof comparisonFactor1["hinh_dang_dat"] !== "undefined" &&
				comparisonFactor1["hinh_dang_dat"].status === 1
			) {
				let percentHdd1 =
					+comparisonFactor1["hinh_dang_dat"].adjust_percent || 0;
				let percentHdd2 =
					+comparisonFactor2["hinh_dang_dat"].adjust_percent || 0;
				let percentHdd3 =
					+comparisonFactor3["hinh_dang_dat"].adjust_percent || 0;
				// mức điều chỉnh của yếu tố HÌNH DÁNG DẤT
				let price1 = this.roundPrice(
					(percentHdd1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentHdd2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentHdd3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["hinh_dang_dat"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.priceKcd3 = 0;
			if (
				typeof comparisonFactor1["ket_cau_duong"] !== "undefined" &&
				comparisonFactor1["ket_cau_duong"].status === 1
			) {
				let percentKcd1 =
					+comparisonFactor1["ket_cau_duong"].adjust_percent || 0;
				let percentKcd2 =
					+comparisonFactor2["ket_cau_duong"].adjust_percent || 0;
				let percentKcd3 =
					+comparisonFactor3["ket_cau_duong"].adjust_percent || 0;
				// mức điều chỉnh của yếu tố KẾT CẤU ĐƯỜNG
				let price1 = this.roundPrice(
					(percentKcd1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentKcd2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentKcd3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["ket_cau_duong"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.priceDrd3 = 0;
			if (
				typeof comparisonFactor1["do_rong_duong"] !== "undefined" &&
				comparisonFactor1["do_rong_duong"].status === 1
			) {
				let percentDrd1 =
					+comparisonFactor1["do_rong_duong"].adjust_percent || 0;
				let percentDrd2 =
					+comparisonFactor2["do_rong_duong"].adjust_percent || 0;
				let percentDrd3 =
					+comparisonFactor3["do_rong_duong"].adjust_percent || 0;

				// mức điều chỉnh của yếu tố ĐỘ RỘNG ĐƯỜNG
				let price1 = this.roundPrice(
					(percentDrd1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentDrd2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentDrd3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["do_rong_duong"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.priceDkht3 = 0;
			if (
				typeof comparisonFactor1["dieu_kien_ha_tang"] !== "undefined" &&
				comparisonFactor1["dieu_kien_ha_tang"].status === 1
			) {
				let percentDkht1 =
					+comparisonFactor1["dieu_kien_ha_tang"].adjust_percent || 0;
				let percentDkht2 =
					+comparisonFactor2["dieu_kien_ha_tang"].adjust_percent || 0;
				let percentDkht3 =
					+comparisonFactor3["dieu_kien_ha_tang"].adjust_percent || 0;
				// mức điều chỉnh của yếu tố ĐIỀU KIỆN HẠ TẦNG
				let price1 = this.roundPrice(
					(percentDkht1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentDkht2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentDkht3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["dieu_kien_ha_tang"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.priceKd3 = 0;
			if (
				typeof comparisonFactor1["kinh_doanh"] !== "undefined" &&
				comparisonFactor1["kinh_doanh"].status === 1
			) {
				...
				let price1 = this.roundPrice(
					(percentKd1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentKd2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentKd3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["kinh_doanh"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.priceAnmts3 = 0;
			if (
				typeof comparisonFactor1["an_ninh_moi_truong_song"] !== "undefined" &&
				comparisonFactor1["an_ninh_moi_truong_song"].status === 1
			) {
				let percentAnmts1 =
					+comparisonFactor1["an_ninh_moi_truong_song"].adjust_percent || 0;
				let percentAnmts2 =
					+comparisonFactor2["an_ninh_moi_truong_song"].adjust_percent || 0;
				let percentAnmts3 =
					+comparisonFactor3["an_ninh_moi_truong_song"].adjust_percent || 0;
				// mức điều chỉnh của yếu tố AN NINH, MÔI TRƯỜNG SỐNG
				let price1 = this.roundPrice(
					(percentAnmts1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentAnmts2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentAnmts3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["an_ninh_moi_truong_song"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.pricePt3 = 0;
			if (
				typeof comparisonFactor1["phong_thuy"] !== "undefined" &&
				comparisonFactor1["phong_thuy"].status === 1
			) {
				...
				let price1 = this.roundPrice(
					(percentPt1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentPt2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentPt3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["phong_thuy"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.priceQh3 = 0;
			if (
				typeof comparisonFactor1["quy_hoach"] !== "undefined" &&
				comparisonFactor1["quy_hoach"].status === 1
			) {
				...
				// mức điều chỉnh của yếu tố AN NINH, MÔI TRƯỜNG SỐNG
				let price1 = this.roundPrice(
					(percentQh1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentQh2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentQh3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["quy_hoach"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.priceDktt3 = 0;
			if (
				typeof comparisonFactor1["dieu_kien_thanh_toan"] !== "undefined" &&
				comparisonFactor1["dieu_kien_thanh_toan"].status === 1
			) {
				let percentDktt1 =
					+comparisonFactor1["dieu_kien_thanh_toan"].adjust_percent || 0;
				let percentDktt2 =
					+comparisonFactor2["dieu_kien_thanh_toan"].adjust_percent || 0;
				let percentDktt3 =
					+comparisonFactor3["dieu_kien_thanh_toan"].adjust_percent || 0;
				// mức điều chỉnh của yếu tố ĐIỀU KIỆN THANH TOÁN
				let price1 = this.roundPrice(
					(percentDktt1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentDktt2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentDktt3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["dieu_kien_thanh_toan"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.priceVitri3 = 0;
			if (
				typeof comparisonFactor1["vi_tri"] !== "undefined" &&
				comparisonFactor1["vi_tri"].status === 1
			) {
				...
				// mức điều chỉnh của yếu tố ĐIỀU KIỆN THANH TOÁN
				let price1 = this.roundPrice(
					(percentVitri1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentVitri2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentVitri3 * this.totalPricePL3) / 100,
					0
				);
			...
			this.priceGt3 = 0;
			if (
				typeof comparisonFactor1["giao_thong"] !== "undefined" &&
				comparisonFactor1["giao_thong"].status === 1
			) {
				...
				let price1 = this.roundPrice(
					(percentGt1 * this.totalPricePL1) / 100,
					0
				);
				let price2 = this.roundPrice(
					(percentGt2 * this.totalPricePL2) / 100,
					0
				);
				let price3 = this.roundPrice(
					(percentGt3 * this.totalPricePL3) / 100,
					0
				);
				...
				adjustPrice2 += price2;
				adjustPrice3 += price3;
				this.adjustPriceData["giao_thong"] = [
					adjustPrice1,
					adjustPrice2,
					adjustPrice3
				];
			...
			this.data_other_comparison.forEach(data => {
				data.other_factor_asset.forEach((item, index) => {
					if (!map.has(item.position)) {
						...
						const comparison_other = this.other_comparison.filter(
							data => data.position === item.position
						);
						comparison_other.forEach((dataOther, indexAsset) => {
							...
							let price = this.roundPrice(
								(dataOther.adjust_percent * arrayPriceTem[indexAsset]) / 100,
								0
							);
			...
			asset.asset_general.forEach((asset, index) => {
				const asset_comparison_data = this.other_comparison.filter(
					data => data.asset_general_id === asset.id
				);
				...
				asset_comparison_data.forEach(dataOther => {
					...
					mgcd_price_other_abs[index] += this.roundPrice(
						Math.abs((dataOther.adjust_percent * arrayPriceTem[index]) / 100)
					);
				});
			});
			...
			this.tldcg1 =
				Math.abs(this.pricePl1) +
				Math.abs(this.priceHdd1) +
				Math.abs(this.priceKcd1) +
				Math.abs(this.priceKd1) +
				Math.abs(this.priceDkht1) +
				Math.abs(this.pricePt1) +
				Math.abs(this.priceDktt1) +
				Math.abs(this.priceQm1) +
				Math.abs(this.priceCrmt1) +
				Math.abs(this.priceCskd1) +
				Math.abs(this.priceDrd1) +
				Math.abs(mgcd_price_other_abs[0]) +
				Math.abs(this.priceVitri1);
			this.tldcg2 =
				Math.abs(this.pricePl2) +
				Math.abs(this.priceHdd2) +
				Math.abs(this.priceKcd2) +
				Math.abs(this.priceKd2) +
				Math.abs(this.priceDkht2) +
				Math.abs(this.pricePt2) +
				Math.abs(this.priceDktt2) +
				Math.abs(this.priceQm2) +
				Math.abs(this.priceCrmt2) +
				Math.abs(this.priceCskd2) +
				Math.abs(this.priceDrd2) +
				Math.abs(mgcd_price_other_abs[1]) +
				Math.abs(this.priceVitri2);
			this.tldcg3 =
				Math.abs(this.pricePl3) +
				Math.abs(this.priceHdd3) +
				Math.abs(this.priceKcd3) +
				Math.abs(this.priceKd3) +
				Math.abs(this.priceDkht3) +
				Math.abs(this.pricePt3) +
				Math.abs(this.priceDktt3) +
				Math.abs(this.priceQm3) +
				Math.abs(this.priceCrmt3) +
				Math.abs(this.priceCskd3) +
				Math.abs(this.priceDrd3) +
				Math.abs(mgcd_price_other_abs[2]) +
				Math.abs(this.priceVitri3);
			...
			this.mgtb =
				asset.asset_general.length > 0
					? (this.mgcd1 + this.mgcd2 + this.mgcd3) / asset.asset_general.length
					: 0;
			...
			this.mgcl1 = this.mgtb
				? Math.round(((this.mgcd1 - this.mgtb) / this.mgtb) * 100, 0)
				: 0;
			this.mgcl2 = this.mgtb
				? Math.round(((this.mgcd2 - this.mgtb) / this.mgtb) * 100, 0)
				: 0;
			this.mgcl3 = this.mgtb
				? Math.round(((this.mgcd3 - this.mgtb) / this.mgtb) * 100, 0)
				: 0;

			// biên độ điều chỉnh
			asset.asset_general.forEach(assetData => {
				// check all comparison
				let temp_comparison_factor = asset.comparison_factor.filter(
					item => item.type !== "yeu_to_khac"
				);
				...
				const assetFilter = temp_comparison_factor.filter(
					item => item.asset_general_id === assetData.id && item.status === 1
				);
			...
			if (
				...
				if (this.form.properties && this.form.properties.length > 0) {
					this.form.properties.forEach(property => {
						property.property_detail.forEach(propertyDetail => {
							let array_property_facility = property.property_detail.filter(
								item => item.is_transfer_facility
							);
							if (!propertyDetail.is_transfer_facility) {
								array_difference_price.push(
									array_property_facility[0].circular_unit_price -
										propertyDetail.circular_unit_price
								);
							}
						});
					});
				...
				this.remaining_commerce_price = parseFloat(
					this.formatCurrent(this.mgtnTemp) - difference_price
				).toFixed(0);
				...
				this.form.composite_land_remaning_slug === "theo-phuong-phap-doc-lap"
			) {
			...
			if (
				this.form.properties[0].property_detail.find(
					property_detail => property_detail.is_zoning === true
				)
			) {
			...
			if (
				this.form.planning_violation_price_slug === "theo-gia-dat-qd-ubnd" &&
				...
				if (
					...
					facility_price = this.form.properties[0].property_detail.find(
						property_detail => property_detail.is_transfer_facility === true
					).circular_unit_price;
				}
				if (
					...
					composite_price = this.form.properties[0].property_detail.find(
						property_detail => property_detail.is_transfer_facility === false
					).circular_unit_price;
				}
				...
				this.form.planning_violation_price_slug ===
					"theo-ty-le-gia-dat-thi-truong" &&
				this.form.properties &&
				this.form.properties.length > 0
			) {
				this.type_violation_method = `Phương pháp: tỷ lệ ${this.form.planning_violation_price_value}% giá đất thị trường`;
				this.violation_facility_price = this.layer_cutting_procedure
					? (this.formatCurrent(this.round_layer_cutting_produre) *
							+this.form.planning_violation_price_value) /
					  100
					: (this.formatCurrent(this.mgtn) *
							this.form.planning_violation_price_value) /
					  100;
				this.violation_composite_price =
					(this.formatRoundComposite(this.remaining_commerce_price) *
						this.form.planning_violation_price_value) /
					100;
			}
			this.key_render_around += 1;
		...
		// Fix doi muc dich chinh
		calculationUpdateMucDichChinh(asset) {
			let asset1 =
				typeof asset.asset_general[0] !== "undefined"
					? asset.asset_general[0]
					: null;
			...
			// 	this.asset_appropriate_area_arr
			// );
			// console.log("arr_arena1", arr_arena1);
			let arr_arena2 = this.asset_appropriate_area_arr.filter(
				item =>
					item.name_purpose_land_asset !==
						comparisonFactor2["muc_dich_chinh"].asset_title &&
			...
			if (arr_arena1.length > 0 && this.form.asset_unit_price.length > 0) {
				...
				for (let i = 0; i < arr_arena1.length; i++) {
					...
					chiphichuyendoi +=
						parseFloat(a1.total_area) * (giatienchinh - giatienphu);
					// console.log("gia tien chinh", giatienchinh);
					// console.log("gia tien phu", giatienphu);
					// console.log("a1", a1);
					// console.log("cpcd", chiphichuyendoi);
				}
				// console.log("chiphichuyendoi1", chiphichuyendoi);
		...
		calculatePrintData(data) {
			...
			if (
				data.properties[0] &&
				data.properties[0].property_detail &&
				data.properties[0].property_detail.length > 0
			) {
				...
				propertyDetail.forEach(item => {
					acronym = item.land_type_purpose.acronym;
					purposePrice = assetPrice.filter(
						i =>
							i.slug === "land_asset_purpose_" + acronym + "_price" &&
							i.value > 0
					);
					violatePrice = assetPrice.filter(
						i =>
							i.slug === "land_asset_purpose_" + acronym + "_violation_price" &&
							i.value > 0
					);
					if (purposePrice && purposePrice.length > 0) {
						if (item.main_area > 0) {
							if (item.is_transfer_facility) {
								...
								roundData = assetPrice.filter(
									i => i.slug === "round_composite"
								);
							}
		...
		getAssetPrice(data) {
			...
			let assetPrice = data.asset_price.filter(
				item => item.slug.substr(0, strSlug.length) === strSlug
			);
			let priceLand = [];
			// let price = []
			if (landDetails.length > 0) {
				landDetails.forEach(item => {
					priceLand = assetPrice.filter(
						f =>
							f.slug.substr(strSlug.length, 3) ===
							item.land_type_purpose.acronym
					);
