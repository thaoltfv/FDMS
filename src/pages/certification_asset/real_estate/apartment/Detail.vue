						...
						<div class="btn-footer d-md-flex d-block justify-content-end align-items-center">
							<div class="d-lg-flex d-block button-contain">
								...
								<button v-if="edit || add" class="btn btn-white btn-orange text-nowrap" @click.prevent="duplicateCertificateAsset" type="submit">
											<img src="@/assets/icons/ic_save.svg" style="margin-right: 12px" alt="save"/>Nhân bản
								</button>
...
export default {
			...
			await CertificateAsset.getAllStepApartment(to.query['id'])
				.then((resp) => {
					if (resp.data) {
						...
						if (resp.error && resp.error.statusCode) { return next('/'.resp.error.statusCode) }
					}
				})
				.catch(() => {
					return next('/403')
	...
	async created () {
		...
		if ('id' in this.$route.query && this.$route.name === 'certification_asset.apartment.detail') {
			this.isEdit = true
			if (this.$route.meta['step']) {
				...
				// step 5
				if (bindDataStep.assets_general && bindDataStep.assets_general.length > 0) {
			...
			if (this.$route.meta['step']) {
				...
				if (bindDataStep.general_infomation) { this.form.step_1 = bindDataStep.general_infomation }
				if (bindDataStep.traffic_infomation) { this.form.step_1.traffic_infomation = bindDataStep.traffic_infomation }
				if (bindDataStep.pic && bindDataStep.pic.length > 0) { this.form.step_1.pic = bindDataStep.pic }
				// step 2
				if (bindDataStep.law && bindDataStep.law.length > 0) { this.form.step_2.law = bindDataStep.law }
				// step 3
				if (bindDataStep.appraisal_methods) { this.form.step_3.appraisal_methods = bindDataStep.appraisal_methods }
	...
	methods: {
		...
		duplicateCertificateAsset () {
			this.$router.push({ name: 'certification_asset.apartment.create', params: { id: this.idData } }).catch(_ => {})
		},
		async handleChange (prevIndex, nextIndex) {
			...
			if (nextIndex + 1 === 1) { this.key_step_1 += 1 }
			if (nextIndex + 1 === 2) { this.key_step_2 += 1 }
			if (nextIndex + 1 === 4) { this.key_step_4 += 1 }
		...
		async handleChangeProject (projectId) {
			...
			if (projectId) {
				...
				this.form.step_1.appraise_asset = `${wardName}, ` + `${districtName}, ` + provinceName
