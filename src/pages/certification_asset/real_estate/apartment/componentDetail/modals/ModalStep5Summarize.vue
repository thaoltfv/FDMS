						...
						<tbody>
								...
								<td>
									{{
										apartment_total_price
											? `${formatNumber(apartment_total_price.toFixed(0))} đ`
											: "0 đ"
									}}
								</td>
							</tr>
							<tr>
								...
								<td>
									{{
										other_asset_price
											? `${formatNumber(other_asset_price.toFixed(0))} đ`
											: "0 đ"
									}}
								</td>
							</tr>
							<tr>
								...
								<td>
									<strong>{{
										total_price
											? `${formatNumber(total_price.toFixed(0))} đ`
											: "0 đ"
									}}</strong>
								</td>
							</tr>
							<tr>
								...
								<td>
									<strong>{{
										total_price
											? `${formatNumber(
													formatCurrent(total_price.toFixed(0))
											  )} đ`
											: `0 đ`
									}}</strong>
								</td>
...
export default {
	...
	props: [
		...
		"apartment_total_price",
		"other_asset_price",
		"apartment_asset_price",
		"apartment_round_total",
		"total_price",
		"isEditStatus"
	],
	components: {
	...
	methods: {
		async printTSSS() {
			let arrayAsset = [];
			if (
				this.data.apartment_has_asset &&
				this.data.apartment_has_asset.length > 0
			) {
		...
		saveData() {
			...
			if (
				apartments_round_total < -7 ||
				apartments_round_total > 7 ||
				(!apartments_round_total && apartments_round_total !== 0)
			) {
				this.$toast.open({
		...
		async postDataSummarizationReport(apartments_round_total) {
			this.isSubmit = true;
			const res = await CertificateAsset.postDataSummarizeApartment(
				this.idData,
				apartments_round_total
			);
			if (res.data) {
				this.$toast.open({
