  ...
  <div class="row content_detail_asset">
      ...
      <div class="btn-footer d-md-flex d-block justify-content-end align-items-center w-100">
        <div class="d-md-flex d-block">
          ...
          <button v-if="isEditStatus" :class="{ 'btn_loading disabled': isSubmit }" class="btn btn-white btn-orange text-nowrap" @click.prevent="saveData">
...
export default {
	...
	props: ['data', 'idData', 'edit', 'status', 'checkRole', 'apartment_total_price', 'other_asset_price', 'apartment_asset_price', 'apartment_round_total', 'total_price', 'isEditStatus'],
	components: {
		InputText,
		InputCurrency,
		InputPercent,
	...
	data () {
		return {
			...
			key_render: 6152,
			apartments_round_total: 0,
			isSubmit: false
		}
	},
	mounted () {
		this.getSummarizedPrice()
		this.key_render += 1
	},
	beforeUpdate () {
	},
	methods: {
		async printTSSS () {
			let arrayAsset = []
			if (this.data.apartment_has_asset && this.data.apartment_has_asset.length > 0) {
				await this.data.apartment_has_asset.forEach(item => {
		...
		getSummarizedPrice () {
			if (this.form.price && this.form.price.length > 0) {
				this.form.price.forEach(item => {
					if (item.slug === 'apartment_round_total') {
						this.apartments_round_total = item.value
					}
				})
			}
		},
		...
		changeRoundTotal (event) {
			if (event || event === 0) {
				this.apartments_round_total = parseFloat(event).toFixed(0)
			} else {
				this.apartments_round_total = ''
			}
		},
		formatCurrent (value) {
		...
		saveData () {
			const apartments_round_total = this.apartments_round_total
			if (apartments_round_total < -7 || apartments_round_total > 7 || (!apartments_round_total && apartments_round_total !== 0)) {
				this.$toast.open({
			...
			} else this.postDataSummarizationReport(apartments_round_total)
		},
		async postDataSummarizationReport (apartments_round_total) {
			this.isSubmit = true
			const res = await CertificateAsset.postDataSummarizeApartment(this.idData, apartments_round_total)
			if (res.data) {
				this.$toast.open({
