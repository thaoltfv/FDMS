		...
		<div v-if="!isMobile()" class="certification-asset">
			...
			>
				...
				<div class="wizard-custom-info" v-if="createdBy">
					<div class="col-13">
						...
						<div class="row d-flex">
							...
							<a
								class="mb-1"
								:href="`/certification_brief/detail?id=${form.certificate.id}`"
								v-if="form.certificate"
								target="_blank"
								>{{ form.certificate ? form.certificate.id : "" }}</a
							>
							<p class="mb-1" v-else>
								{{ form.certificate ? form.certificate.id : "" }}
							</p>
						</div>
						<div class="">
				...
				<tab-content title="Quyền sử dụng đất" icon="">
					<ValidationObserver tag="form" ref="step_2">
						<Step2
							:key="key_step_2"
							:data="form.step_2"
				...
				<tab-content title="Công trình xây dựng" icon="">
					<ValidationObserver tag="div" ref="step_3" class="height_form_wizard">
						<Step3
							:data="form.step_3"
							:key="key_step_3"
				...
				<tab-content title="Pháp lý tài sản" icon="">
					<ValidationObserver tag="div" ref="step_4" class="height_form_wizard">
						<Step4
							:data="form.step_4"
							:key="key_step_4"
				...
				<tab-content title="Cơ sở thẩm định" icon="">
					<ValidationObserver tag="div" ref="step_5">
						<Step5
							:data="form.step_5"
							:key="key_step_5"
				...
				<tab-content title="Tài sản so sánh" icon="">
					<ValidationObserver tag="div" ref="step_6">
						<Step6
							:data="form.step_6"
							:key="key_step_6"
...
export default {
	...
	beforeRouteEnter: async (to, from, next) => {
		await CertificateAsset.getDataAllStep(to.query["id"])
			.then(async resp => {
				if (resp.data) {
					to.meta["step"] = resp.data;
					if (resp.data.step >= 6) {
						const getDataStep7 = await CertificateAsset.getDataStep7(
							to.query["id"]
						);
		...
		if (
			...
			if (this.$route.meta["step"]) {
				...
				if (bindDataStep.general_infomation) {
					this.form.step_1.general_infomation = bindDataStep.general_infomation;
				}
				if (bindDataStep.traffic_infomation) {
					this.form.step_1.traffic_infomation = bindDataStep.traffic_infomation;
				}
				if (bindDataStep.geographical_location) {
					this.form.step_1.geographical_location =
						bindDataStep.geographical_location;
				}
				if (
					bindDataStep.picture_infomation &&
					bindDataStep.picture_infomation.length > 0
				) {
					this.form.step_1.picture_infomation = bindDataStep.picture_infomation;
				}
				// step 2
				if (bindDataStep.land_details) {
					this.form.step_2.land_details = bindDataStep.land_details;
				}
				if (bindDataStep.total_area && bindDataStep.total_area.length > 0) {
					this.form.step_2.total_area = bindDataStep.total_area;
				}
				if (
					bindDataStep.planning_area &&
					bindDataStep.planning_area.length > 0
				) {
					this.form.step_2.planning_area = bindDataStep.planning_area;
				}
				if (bindDataStep.UBND_price && bindDataStep.UBND_price.length > 0) {
					this.form.step_2.UBND_price = bindDataStep.UBND_price;
				}
				if (bindDataStep.real_estate) {
					this.form.step_2.real_estate = bindDataStep.real_estate;
				}
				// step 3
				if (bindDataStep.construction && bindDataStep.construction.length > 0) {
					this.form.step_3.construction = bindDataStep.construction;
				}
				// step 4
				if (bindDataStep.law && bindDataStep.law.length > 0) {
					this.form.step_4.law = bindDataStep.law;
				}
				// step 5
				if (bindDataStep.appraisal_methods) {
					this.form.step_5.appraisal_methods = bindDataStep.appraisal_methods;
				}
				if (bindDataStep.value_base_and_approach) {
					this.form.step_5.value_base_and_approach =
						bindDataStep.value_base_and_approach;
				}
				if (
					bindDataStep.value_base_and_approach.document_description == "" ||
					bindDataStep.value_base_and_approach.document_description == null
				) {
					this.form.step_5.value_base_and_approach.document_description =
						"- Giả thiết:\n- Giả thiết đặc biệt:";
				}
				// step 6
				if (
					...
					this.form.step_6.comparison_factor = bindDataStep.comparison_factor;
				}
				if (
					bindDataStep.assets_general &&
					bindDataStep.assets_general.length > 0
				) {
					this.form.step_6.assets_general = bindDataStep.assets_general;
				}
				if (bindDataStep.map_img) {
					this.form.step_6.map_img = bindDataStep.map_img;
				}
			...
			if (this.$route.meta["step7"]) {
				this.form.step_7 = Object.assign(this.form.step_7, {
					...this.$route.meta["step7"]
				});
			}
			if (
				this.form.step_7.construction_company &&
				this.form.step_7.construction_company.length > 0 &&
				this.form.step_7.tangible_assets &&
				this.form.step_7.tangible_assets.length > 0
			) {
				await this.form.step_7.tangible_assets[0].construction_company.forEach(
					item => {
						this.form.construction_company_ids.push(
							item.construction_company_id
						);
					}
				);
			}
	...
	methods: {
		...
		async handleChangeTab(oldId, newId) {
			...
			if (isValid) {
				...
				if (step_1.traffic_infomation.front_side === "") {
					...
					step_1.traffic_infomation.front_side === 0 &&
					step_1.traffic_infomation.property_turning_time.length === 0
				) {
					this.isSubmit = false;
					this.$toast.open({
		...
		async handleSubmitStep_1(dataStep1) {
			const res = await CertificateAsset.submitStep1(
				dataStep1,
				dataStep1.general_infomation.id
			);
			if (res.data) {
				this.isSubmit = false;
				this.idData = res.data.general_infomation.id;
				this.form.step_2.land_details.coordinates =
		...
		async changeLandTypePurpose(index, land_type_purpose_id) {
			if (land_type_purpose_id) {
				...
				let getDataTypePrupose = this.type_purposes.filter(
		...
		handleChangeUBNDPrice() {
			...
			allPurposeArray.forEach(itemPurpose => {
				let checkUBNDIsExist = this.form.step_2.UBND_price.filter(
					itemUBND =>
		...
		handleGetTotalArea() {
			...
			this.form.step_2.land_details.appraise_land_sum_area =
		...
		changeUsingYear(event) {
			if (event) {
				if (this.form.step_3.construction.created_at) {
					if (
						+moment(this.form.step_3.construction.created_at).format("YYYY") -
							this.form.step_3.construction.start_using_year >
						this.form.step_3.construction.duration
					) {
						this.form.step_3.construction.remaining_quality = 0;
					} else
						this.form.step_3.construction.remaining_quality = parseFloat(
							(1 -
								(+moment(this.form.step_3.construction.created_at).format(
									"YYYY"
								) -
									this.form.step_3.construction.start_using_year) /
									this.form.step_3.construction.duration) *
		...
		findProvince(isBindData) {
			const province = this.provinces.find(
				province =>
					province.id === this.form.step_1.general_infomation.province_id
			);
			if (province) {
		...
		findDistrict(isBindData) {
			const district = this.districts.find(
				district =>
					district.id === this.form.step_1.general_infomation.district_id
			);
			if (district) {
		...
		findWard(isBindData) {
			const ward = this.wards.find(
				ward => ward.id === this.form.step_1.general_infomation.ward_id
			);
			if (ward) {
		...
		findStreet() {
			const street = this.streets.find(
				street => street.id === this.form.step_1.general_infomation.street_id
			);
			if (street) {
		...
		findDistance() {
			const distance = this.distances.find(
				distances =>
					distances.id === this.form.step_1.general_infomation.distance_id
			);
			if (distance) {
		...
		changeAssetType(id) {
			const assetType = this.propertyTypes.find(
				assetType =>
					assetType.id === this.form.step_1.general_infomation.asset_type_id
			);
			if (assetType) {
		...
		async getAppraiseOthers() {
			await Certificate.getAppraiseOthers()
				.then(resp => {
					this.appraisalPurposes = resp.data.muc_dich_tham_dinh_gia.filter(
					...
					this.appraisalPrinciples = resp.data.nguyen_tac_tham_dinh.filter(
						item => item.dictionary_acronym.includes("BDS")
					);
		...
		findUnifyIndicativePrice() {
			this.form.step_5.appraisal_methods.thong_nhat_muc_gia_chi_dan.slug_value = this.unifyIndicativePrice.find(
				item => item.is_defaults === true
			).slug;
		},
		findCompositeLandRemaning() {
			this.form.step_5.appraisal_methods.tinh_gia_dat_hon_hop_con_lai.slug_value = this.compositeLandRemaning.find(
				item => item.is_defaults === true
			).slug;
			if (
				this.form.step_5.appraisal_methods.tinh_gia_dat_hon_hop_con_lai
					.slug_value === "theo-ty-le-gia-dat-co-so-chinh"
			) {
				this.form.step_5.appraisal_methods.tinh_gia_dat_hon_hop_con_lai.value = 80;
			}
		},
		findPlanningViolationPrice() {
			this.form.step_5.appraisal_methods.tinh_gia_dat_vi_pham_quy_hoach.slug_value = this.planningViolationPrice.find(
				item => item.is_defaults === true
			).slug;
			if (
				this.form.step_5.appraisal_methods.tinh_gia_dat_vi_pham_quy_hoach
					.slug_value === "theo-ty-le-gia-dat-thi-truong"
			) {
				this.form.step_5.appraisal_methods.tinh_gia_dat_vi_pham_quy_hoach.value = 50;
			}
		},
		findAppraisalFacility() {
			this.form.step_5.value_base_and_approach.appraise_basis_property_id = this.appraisalFacility.find(
				appraisalFacility => appraisalFacility.is_defaults === true
			).id;
		},
		findAppraisalPrinciples() {
			this.form.step_5.value_base_and_approach.appraise_principle_id = this.appraisalPrinciples.find(
				appraisalPrinciple => appraisalPrinciple.is_defaults === true
			).id;
		},
		findApproach() {
			this.form.step_5.value_base_and_approach.appraise_approach_id = this.approach.find(
				approach => approach.is_defaults === true
			).id;
		},
		findMethodsUsed() {
			this.form.step_5.value_base_and_approach.appraise_method_used_id = this.methodsUsed.find(
			...
			let body = {
				...
				front_side:
					this.form.properties.length > 0
						? this.form.properties[0].front_side
						: "",
				main_road_length:
					this.form.properties.length > 0 &&
					this.form.properties[0].property_detail.length > 0
		...
		getPropertyId(id, version) {
			this.form.assets_general.push({
				...
				asset_property_detail_id: this.property.properties[0]
					? this.property.properties[0].id
					: 0,
				version: version
			});
		},
		getDataID(id, indexProperty, asset_id, property_data) {
			...
			this.form.assets_general.forEach((item, index) => {
				if (item.assets_general_id === asset_id) {
					...
					this.form.assets_general[index].version =
						property_data.version.length > 0 ? maxVersion : 1;
				}
			});
		},
