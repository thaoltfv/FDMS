...
export default {
	...
	methods: {
		async getDataProcessProgress(fromDate, toDate) {
			...
			const res = await Report.getReportProcessProgressPreCertificate(
				this.form
			);
			...
			if (res.data && res.data.status && res.data.status.length > 0) {
				...
				res.data.status.forEach(item => {
					if (item === 1) {
						...
						this.chartDoughnut.labels.push("Tiếp nhận yêu cầu");
					} else if (item === 2) {
						this.chartDoughnut.datasets[0].backgroundColor.push("#FAB005");
						this.chartDoughnut.labels.push("Đang thực hiện");

						// } else if (item === 8) {
						// 	this.chartDoughnut.datasets[0].backgroundColor.push("#45AAE0");
						// 	this.chartDoughnut.labels.push("Phân hồ sơ");
						// } else if (item === 2) {
						// 	this.chartDoughnut.datasets[0].backgroundColor.push("#0062AF");
						// 	this.chartDoughnut.labels.push("Định giá sơ bộ");
						// } else if (item === 3) {
						// 	this.chartDoughnut.datasets[0].backgroundColor.push("#EBC784");
						// 	this.chartDoughnut.labels.push("Duyệt giá sơ bộ");
						// } else if (item === 4) {
						// 	this.chartDoughnut.datasets[0].backgroundColor.push("#FAB005");
						// 	this.chartDoughnut.labels.push("Thương thảo");
						// } else if (item === 5) {
						// 	this.chartDoughnut.datasets[0].backgroundColor.push("#FAB005");
						// 	this.chartDoughnut.labels.push("Phát hành KQSB");
					} else if (item === 6) {
