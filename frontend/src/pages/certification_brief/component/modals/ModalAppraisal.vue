		...
		<div class="card">
			<div class="container-title">
				<div class="d-flex justify-content-between">
					<h2 class="title">
						{{ ModalEdit ? "Tổ thẩm định" : "Xác nhận lại Tổ thẩm định" }}
					</h2>
					<img
						height="35px"
						@click="handleCancel"
...
export default {
	...
	methods: {
		...
		async getTimeStamp() {
			...
			if (res.data) {
				...
				if (this.data.status === 1) {
					...
					statusObject = res.data.filter(
						item => item.acronym === "DANG-THAM-DINH"
					);
					this.minutes = statusObject[0].value;
			...
			this.form.status_expired_at = await moment(dateConverted).format(
				"DD-MM-YYYY HH:mm"
			);
		},
		async getAppraisers() {
			...
			if (this.form && this.form.appraiser_manager_id) {
				this.appraisers = await appraiser.filter(
					item => item.id !== this.form.appraiser_manager_id
				);
			}
			if (this.form && this.form.appraiser_id) {
				const filterData = await appraiser.filter(
					item =>
						item.id !== this.form.appraiser_id &&
						item.id !== this.form.appraiser_manager_id
				);
		...
		async handleAction(event) {
			let dateConverted = moment(this.form.status_expired_at).format(
				"YYYY-MM-DD HH:mm:ss"
			);
			const data = {
				status: this.status,
			...
			if (this.ModalEdit) {
				const res = await CertificationBrief.updateAppraisers(
					this.idData,
					data
				);
				if (res.data) {
	...
	computed: {
		...
		administrative_compute: {
			...
			set: function(newValue) {
				// // console.log('newwww', newValue)
				this.form.administrative_id = newValue;
			}
		},
