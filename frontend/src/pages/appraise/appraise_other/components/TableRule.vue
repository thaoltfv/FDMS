							...
							>
								<div class="row table-footer" v-if="isAddNewItem">
									<div class="input-table col-6">
										<label>Tên nguyên tắc thẩm định</label>
										<InputText
											class="label-none input-error"
											v-model="form.name"
											vid="addName"
											label="Tên nguyên tắc thẩm định"
											rules="required"
										/>
									</div>
...
export default {
	...
	computed: {
		columns() {
			return [
				{
					title: "Tên nguyên tắc thẩm định",
					align: "left",
					width: "55%",
	...
	methods: {
		...
		async handleDelete() {
			...
			this.$toast.open({
				message: "Xóa tên nguyên tắc thẩm định thành công",
				type: "success",
				position: "top-right"
			});
		},
		...
		async createAppraiser(data) {
			try {
				...
				if (resp) {
					if (resp.data) {
						this.isSubmit = false;
						this.message = "Tạo mới tên nguyên tắc thẩm định thành công.";
		...
		async handleUpdateTDV(data) {
			...
			if (resp.data) {
				this.message = "Cập nhật tên nguyên tắc thẩm định thành công.";
				this.openNotification = true;
				this.editingKey = "";
				this.isAddNewItem = false;
				this.onPageChange(1);
