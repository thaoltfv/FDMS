						...
						<template slot="footer" slot-scope="currentPageData" v-if="add">
							...
							>
								<div class="row table-footer" v-if="isAddNewItem">
									<div class="input-table col-6">
										<label>Mục đích thẩm định giá</label>
										<InputText
											class="label-none input-error"
											v-model="form.name"
											vid="addName"
											label="Mục đích thẩm định giá"
											rules="required"
										/>
									</div>
...
export default {
	...
	methods: {
		...
		async handleDelete() {
			...
			this.$toast.open({
				message: "Xóa mục đích thẩm định giá thành công",
				type: "success",
				position: "top-right"
			});
		},
		...
		async createAppraiser(data) {
			try {
				...
				if (resp) {
					if (resp.data) {
						this.isSubmit = false;
						this.message = "Tạo mới mục đích thẩm định giá thành công.";
		...
		async handleUpdateTDV(data) {
			...
			if (resp.data) {
				this.message = "Cập nhật mục đích thẩm định giá thành công.";
				this.openNotification = true;
				this.editingKey = "";
				this.isAddNewItem = false;
				this.onPageChange(1);
