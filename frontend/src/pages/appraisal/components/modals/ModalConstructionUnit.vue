        ...
        <div class="card">
          ...
          <div class="contain-detail">
            ...
            <vue-tabs @tab-change="handleChangeTab">
             <v-tab v-for="(dataChild,index) in constructions" :key="dataChild.id" :title="`TSTD_${dataChild.oldID ? dataChild.oldID : dataChild.id}`">
                <div class="property-detail">
                  <div class="container__table container__table--detail">
                  <a-table bordered
                            :columns="columnDocuments"
                            :data-source="constructions && constructions.length > 0 ?  constructions[index].construction_company : constructions"
                            :loading="isLoading"
...
export default {
	...
	methods: {
		// set onChange :row-selection="{ selectedRowKeys: selectedRowKeys && selectedRowKeys.length > 0 ? selectedRowKeys[index] : [], onChange: onSelectChange}"
		...
		sortDocument (index) {
			let arrayTemple = []
			if (this.selectedRowKeys && this.selectedRowKeys[index]) {
				this.documents.forEach(item => {
					const select = this.selectedRowKeys[index].find(selectedKey => selectedKey === item.id)
					if (select) {
						arrayTemple.push(item)
					}
				})
