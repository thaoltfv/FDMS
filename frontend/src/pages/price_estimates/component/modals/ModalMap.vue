	...
	>
		...
		>
			...
			>
				<img
					slot="legend-caption"
					src="@/assets/images/search_for_real_estate.jpeg"
...
;"
		...
		>
			<div class="card" style="padding:  10px;">
				<div
					...
					style="margin-bottom: 20px;width: 105%;
    margin-left: -10px;"
				>
					<div
						class="d-lg-flex d-block shadow-bottom"
				...
				<div
					class="d-md-flex d-block justify-content-center align-items-center"
    ...
    color: rgb(9, 44, 76);"
						...
						<div v-else class="row" style="margin: 0;">
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.so_to_ban_do
										? dataResult.attributes.so_to_ban_do
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.so_hieu_thua
										? dataResult.attributes.so_hieu_thua
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.quan ? dataResult.attributes.quan : "-"
								}}</span>
							</div>
							<div class="col-6">
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.dien_tich_thua
										? dataResult.attributes.dien_tich_thua
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.dt_k_phu_hop
										? dataResult.attributes.dt_k_phu_hop
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.loai_dat_quy_hoach
										? dataResult.attributes.loai_dat_quy_hoach
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.chieu_sau
										? dataResult.attributes.chieu_sau
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.mat_tien
										? dataResult.attributes.mat_tien
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.nhan_dien
										? dataResult.attributes.nhan_dien
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.tong_mat_tien
										? dataResult.attributes.tong_mat_tien
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.tiep_giap_1
										? dataResult.attributes.tiep_giap_1
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.rong_hem_nho_1
										? dataResult.attributes.rong_hem_nho_1
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.hem_truoc_nha_1
										? dataResult.attributes.hem_truoc_nha_1
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.khoang_cach_1
										? dataResult.attributes.khoang_cach_1
										: "-"
								}}</span>
							</div>
							...
							<div class="col-6">
								<span>{{
									dataResult.attributes.quyet_dinh
										? dataResult.attributes.quyet_dinh
										: "-"
								}}</span>
							</div>
				...
				<div class="d-block d-sm-flex justify-content-between my-3">
					...
					<button
						...
						title="Tìm kiếm theo tờ, thửa"
						alt="Tìm kiếm theo tờ, thửa"
						type="button"
						@click="handleOpenEM"
						style="background-color: #FFFFFF;padding: 0;pointer-events: auto"
					>
                  ...
                  <path
                    ...
                    fill="#007EC6"/>
						...
						<img
							src="https://firebasestorage.googleapis.com/v0/b/fast-value.appspot.com/o/assets%2Fland%20parcel.2.png?alt=media&token=3a1a0bd8-4c64-4dbe-9373-423e664f3669"
...
;"
				...
				<div class="my-3 row">
					...
					<div class="col-2" style="padding-left:0;">
						<button
							...
							type="button"
							@click="handleOpenEM"
							style="background-color: #FFFFFF;padding: 0;pointer-events: auto;    margin-top: 0;"
						>
							<img
								src="https://firebasestorage.googleapis.com/v0/b/fast-value.appspot.com/o/assets%2Fland%20parcel.2.png?alt=media&token=3a1a0bd8-4c64-4dbe-9373-423e664f3669"
								style="height: -webkit-fill-available;
    width: 75px;"
				...
				<div
					class="main-map"
					style="height: -webkit-fill-available;margin-bottom: 60px;"
				>
					<div id="mapid" class="layer-map">
						<l-map
							ref="lmap"
...
export default {
	...
	computed: {
		disabledButton() {
			// console.log('vô vô')
			if (
				(this.emSoToCode || this.emSoThuaCode) &&
				this.emCityCode && this.emDistrictCode && this.emWardCode
			) {
				return false;
			} else return true;
		},
	...
	async mounted() {
		...
		for (var i in cityJson) result.push(cityJson[i]);
	...
	methods: {
		changeProvince(code) {
			...
			this.listWard = [];
			this.emWardCode = "";
			const districtJson = require("@/assets/json/phuluc_dmhc/district/" +
				code +
				".json");
			for (var i in districtJson) this.listDistrict.push(districtJson[i]);
		...
		changeDistrict(code) {
			this.listWard = [];
			this.emWardCode = "";
			const wardJson = require("@/assets/json/phuluc_dmhc/ward/" +
				code +
				".json");
			for (var i in wardJson) this.listWard.push(wardJson[i]);
			...
			if (this.tinhthanh) {
				if (this.listCity.length > 0) {
					...
					for (let i = 0; i < this.listCity.length; i++) {
						...
						if (
							e.name_with_type.toLowerCase() == this.tinhthanh.toLowerCase() ||
							e.name.toLowerCase() == this.tinhthanh.toLowerCase()
						) {
								...
								if (districtJson) {
									...
									if (this.listDistrict.length > 0) {
										...
										for (let i = 0; i < this.listDistrict.length; i++) {
											...
											if (
												q.name_with_type.toLowerCase() ==
													this.quanhuyen.toLowerCase() ||
												q.name.toLowerCase() == this.quanhuyen.toLowerCase()
											) {
													...
													if (wardJson) {
														...
														if (this.listWard.length > 0) {
															...
															for (let i = 0; i < this.listWard.length; i++) {
																...
																if (
																	x.name_with_type.toLowerCase() ==
																		this.phuongxa.toLowerCase() ||
																	x.name.toLowerCase() ==
																		this.phuongxa.toLowerCase()
																) {
				...
				if (APItoken) {
					...
					await File.getInfoByLand({ data: formdata }).then(response => {
						...
						if (
							...
							this.isOpen = false;
							this.map.center = [
								this.geo_data.geometry.coordinates[0][0][0][1]
									? this.geo_data.geometry.coordinates[0][0][0][1]
									: this.geo_data.geometry.coordinates[0][0][1],
								this.geo_data.geometry.coordinates[0][0][0][0]
									? this.geo_data.geometry.coordinates[0][0][0][0]
				...
				await geocoder.geocode(keySearch, function(results, status) {
					if (status === "OK") {
						...
						if (
							...
							].long_name.toLowerCase() == "việt nam"
						) {
