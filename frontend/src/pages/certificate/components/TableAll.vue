  ...
  <div class="table-detail position-relative empty-data">
    <a-table bordered
      ...
      <template slot="status" slot-scope="status">
        <p v-if="status === 1" :style="{color:'#610bec'}" class="status text-none">Nháp</p>
        <p v-if="status === 2" :style="{color:'#CC6600'}" class="status text-none">Đã xác nhận</p>
        <p v-if="status === 3" :style="{color:'#009933'}" class="status text-none">Đã được chọn</p>
        <p v-if="status === 4" :style="{color:'#000033'}" class="status text-none">Hoàn thành</p>
        <p v-if="status === 5" :style="{color:'#DD0000'}" class="status text-none">Đã hủy</p>
        <!-- <p :style="{color:status === 0 ? '#DD0000' : '#610bec'}" class="status text-none">
          {{status === 0 ? 'Đã hủy' : ''}}
          {{status === 1 ? 'Nháp' : ''}}
          {{status === 2 ? 'Đã xác nhận' : ''}}
          {{status === 3 ? 'Đã được chọn' : ''}}
          {{status === 4 ? 'Hoàn thành' : ''}}
        </p> -->
      </template>
		...
		columns () {
			let dataColumn = [
				...
				},
				{
					...
					width: '30px',
					sorter: (a, b) => a.properties[0].appraise_land_sum_area - b.properties[0].appraise_land_sum_area,
					sortDirections: ['descend', 'ascend'],
					hiddenItem: false
				},
				{
