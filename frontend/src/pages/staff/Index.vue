<template>
	<div class="container-fluid">
		...
		<div class="pannel">
			...
			>
				...
				<div class="container__input" v-if="add">
					<button type="button" class="btn btn-create btn-white text-nowrap">
						<img
				...
				<button
					class="btn btn-create btn-white text-nowrap index-screen-button mt-md-0 mt-2 m-0 px-3"
					tag="button"
					v-if="add && can_add"
			...
			<div
				class="pannel__content d-md-flex d-block justify-content-start align-items-center"
			>
                ...
                border-radius: 5px;
				background: green"
						>Tổng cộng: {{ list_total.length }}</span
					>
				</div>
				<div style="margin-right: 20px;">
					<span
                ...
                border-radius: 5px;
				background: #0062AF"
						>Đang kích hoạt: {{ count_enable }}</span
					>
				</div>
				<div>
					<span
                ...
                border-radius: 5px;
				background: #6E7582"
						>Vô hiệu hóa: {{ count_disable }}</span
					>
				</div>
			</div>
		</div>
		...
		>
			<!--Custom type table-->
			<template slot="action" slot-scope="action_edit, action">
				<div class="d-flex justify-content-end">
					<a-tooltip
						placement="bottom"
						:title="$t('tooltip_is_legal_representative')"
						v-if="
							active &&
								action.is_legal_representative == 0 &&
								action.appraiser_number !== ''
						"
						class="mr-2"
					>
						<a
						...
						>
							<img
								class="icon-action"
								src="../../assets/images/account-tie.svg"
								alt="icon"
							/>
						</a>
					</a-tooltip>
					<a-tooltip
						placement="bottom"
						:title="$t('tooltip_isnt_legal_representative')"
						v-if="
							deactive &&
								action.is_legal_representative == 1 &&
								action.appraiser_number !== ''
						"
						class="mr-2"
					>
						...
						>
							<img
								class="icon-action"
								src="../../assets/images/account-convert.svg"
								alt="icon"
							/>
						</a>
					</a-tooltip>
            ...
            <a href="#"
              ...
              <img class="icon-action" src="../../assets/images/icon-delete.svg"
                   alt="icon">
					...
					<a-tooltip
						placement="bottom"
						:title="$t('tooltip_active')"
						v-if="active && action.status_user == 'deactive'"
						class="mr-2"
					>
						<a
							href="#"
						...
						>
							<img
								class="icon-action"
								src="../../assets/images/lock-check-outline.svg"
								alt="icon"
							/>
						</a>
					</a-tooltip>
					<a-tooltip
						placement="bottom"
						:title="$t('tooltip_deactive')"
						v-if="deactive && action.status_user == 'active'"
						class="mr-2"
					>
						<a
							href="#"
						...
						>
							<img
								class="icon-action"
								src="../../assets/images/lock-alert-outline.svg"
								alt="icon"
							/>
						</a>
					</a-tooltip>
			...
			<template slot="status_user" slot-scope="status_user">
				<div class="d-flex justify-content-center">
					<span
						v-if="status_user == 'active'"
			...
			<template
				slot="is_legal_representative"
				slot-scope="is_legal_representative"
			>
				<div class="d-flex justify-content-center">
					<span
						v-if="is_legal_representative == 1"
						style="font-size: 14px;
                font-weight: bold;
                color: white;
                padding: 5px;
			...
			<template slot="is_guest" slot-scope="is_guest">
				<div class="d-flex ">
					<span v-if="!is_guest"> Nội bộ</span>
...
import ModalSelectTypeAccount from "./ModalSelectTypeAccount.vue";
		...
		columns() {
			return [
				...
				},
				{
					...
					align: "left",
					dataIndex: "is_guest",
					scopedSlots: { customRender: "is_guest" }
				},
				{
					title: "Quyền hạn",
					align: "left",
