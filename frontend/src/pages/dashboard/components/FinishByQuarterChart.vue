...
export default {
	...
	props: {
		name: {
			type: String,
			default: 'Hồ sơ hoàn thành theo quý'
		}
	},
	data () {
		return {
	...
	async mounted () {
		await this.getDataCertificationFinish()
	},
	methods: {
		async getDataCertificationFinish () {
			let dataset = []
			this.chartBar.datasets = []
			this.chartBar.labels = []
			const res = await Report.getReportCertificationFinishByQuarters()
			if (res.data && res.data.data) {
				await res.data.data.forEach(item => {
					this.currentYear = new Date().getFullYear()
					if (item.label === `${this.currentYear}`) {
						dataset.push({
							data: item.count,
							label: item.label,
							borderColor: '#FAB005',
