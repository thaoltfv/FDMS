				...
				<div class="d-flex justify-content-between align-items-center">
					...
					<!-- <div class="d-flex justify-content-between align-items-center">
						<InputDatePickerRange
							...
							/>
							<button class="btn ml-2" @click="getDataAssetCompare()">
				...
				<div class="row">
					<div class="col-12">
						...
						<div v-if="this.dataRange && this.dataRange.length > 0" class='legend-scale '>
							<h3 class='legend-title'>Tài Sản So Sánh</h3>
...
export default {
	...
	async mounted () {
		await this.getDataAssetCompare()
	},
	methods: {
		formatNumber (num) {
		...
		async getDataAssetCompare () {
			const res = await Report.getReportAssetCompare()
			if (res.data && res.data.length > 0) {
				...
				await this.$toast.open({
					message: 'Hiện không có dữ liệu thống kê tài sản so sánh',
					type: 'error',
					position: 'top-right',
					duration: 4000
				})
