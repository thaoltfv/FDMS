<template>
	<div v-if="!isMobile()" class="certification-asset">
		...
		>
			...
			<tab-content title="Giá trị tài sản" icon=""> </tab-content>
		</form-wizard>
	...
	computed: {
		getApartmentName() {
			...
			const project_name = this.form.step_1.project.name
				...
				: "";
			let apartmentName =
				apartment_name + floor_name + block_name + project_name;
	...
	async created() {
		...
		if (
			...
			if (this.$route.meta["step"]) {
				...
				if (bindDataStep.project) {
					this.form.step_1.project = bindDataStep.project;
				}
			...
			if (
				this.step_active < 3 ||
				(this.step_active === 3 && this.form.step_4.assets_general.length === 0)
			) {
	...
	methods: {
		...
		async handleChange(prevIndex, nextIndex) {
			if (nextIndex === 3 && this.isAutomation) {
				const response = await CertificateAsset.getAutomationApartment(
					this.idData
				);
				if (response.data) {
					this.form.step_4.assets_general = await response.data.assets;
					this.form.step_4.comparison_factor = await response.data
						.comparison_factors;
					...
					this.$toast.open({
						message:
							"Hệ thống hiện không lấy được tài sản so sánh, vui lòng liên hệ hỗ trợ",
						type: "error",
						position: "top-right",
						duration: 6000
					});
		...
		async getProfiles() {
			...
			this.checkRole =
				profile.data.user.id === this.form.created_by ||
