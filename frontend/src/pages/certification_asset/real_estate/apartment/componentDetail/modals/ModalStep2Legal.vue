...
export default {
	...
	methods: {
		...
		async getContent () {
			...
			// 	this.setContentRows()
			// }
			this.form.content = await 'Chứng nhận ' + `${this.form.legal_name_holder ? this.form.legal_name_holder + ' ' : ''}` + 'được quyền sử dụng chung cư thuộc ' + `${land_description} ` + `${this.full_address ? this.full_address : ''}.`
			land_description = ''
		},
		getProvince () {
