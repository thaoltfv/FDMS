<template>
  <ValidationObserver tag="form" ref="tab_other_property">
    <div class="row mt-3 content_detail_asset">
          ...
          <div class="main-wrapper">
              <div class="responsive-table">
                <div class="row container_header">
                  ...
                  <div class="col-2 class_header class_content">Thao tác</div>
                </div>
                <div v-for="(other_property, index) in form.other_assets" :key="index"  class="row">
                  ...
                  <div class="col-2 class_content">
                    <InputNumberNoneFormat
                      v-model="other_property.quantity"
                  ...
                  <div class="d-flex justify-content-center class_content col-2">
                    <button class="btn-delete" type="button" @click="handleEditOtherProperty(index, other_property)"><img src="@/assets/icons/ic_edit_2.svg" alt="add"/></button>
                    <button class="btn-delete" type="button" @click="handleDeleteOtherProperty(index)"><img src="@/assets/icons/ic_delete_2.svg" alt="delete_land"></button>
                  </div>
                </div>
              </div>
              <div class="d-flex">
                <div class="w-100 d-flex justify-content-end">
                  <button v-if="isEditStatus" class="btn text-warning btn-ghost btn-add" type="button" @click="addOtherAsset">
        ...
        <!-- <div v-if="!checkRole && form.other_assets && form.other_assets.length === 0" class="col-12">
          <div class="infor-box">
        ...
        <div class="btn-footer d-md-flex d-block justify-content-end align-items-center w-100">
          <div class="d-md-flex d-block">
            ...
            <button v-if="isEditStatus" :class="{ 'btn_loading disabled': isSubmit }" class="btn btn-white btn-orange text-nowrap" @click.prevent="handleSaveOtherAsset">
              <img src="@/assets/icons/ic_save.svg" style="margin-right: 12px" alt="save"/>Lưu
            </button>
          </div>
        </div>
        <ModalEditStep5Description
...
import ModalEditStep5Description from './ModalEditStep5Description'

import Vue from 'vue'
import Icon from 'buefy'
Vue.use(Icon)
export default {
	name: 'ModalStep5TabOtherProperty',
	props: ['data', 'idData', 'edit', 'status', 'checkRole', 'isEditStatus'],
	components: {
		InputText,
		InputCurrency,
		InputPercent,
	...
	methods: {
		...
		changeUnitPrice (e, index) {
			if (e) {
				...
				this.form.other_assets[index].total_price = e * (this.form.other_assets[index].quantity ? this.form.other_assets[index].quantity : 0)
			} else {
		...
		changeQuantity (e, index) {
			if (e) {
				this.form.other_assets[index].quantity = e
				...
				this.form.other_assets[index].quantity = ''
			}
			this.key_render += 1
		},
		...
		async saveOtherAsset () {
			...
			const res = await CertificateAsset.submitOtherAssetStep5(data, this.idData)
			if (res.data) {
				this.$toast.open({
