<template>
  <div
    ...
    <div class="card">
      ...
      <div class="card-body">
        <p style="font-size: 18px" v-html="notification">
        </p>

        <div class="btn__group">
          <button
            class="btn btn-white font-weight-normal font-weight-bold"
            @click.prevent="handleCancel">Hủy</button>
          <button
            class="btn btn-white btn-orange font-weight-bold mt-md-0 mt-2"
            @click.prevent="handleAction">Cập nhập</button>
        </div>
      </div>
    </div>
  </div>
...
<script>
export default {
	name: 'ModalNotificationProperty',
	props: ['notification'],
	methods: {
		handleCancel (event) {
