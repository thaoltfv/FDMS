					...
					<div class="card-table">
						...
						>
							<div class="property-content row">
								...
								<button
									class="link-detail property-detail property-detail__id col"
									@click="handleOpenCertificateAsset(property.id)"
								>
									{{ "TSTD" + "_" + property.id }}
								</button>
							</div>
							<div class="property-content row">
								...
								<p class="property-detail col">
									{{ property.properties[0].front_side ? "Mặt tiền" : "Hẻm" }}
								</p>
							</div>
							<div
								class="property-content row"
								v-if="property.properties[0].front_side"
							>
								<p class="property-title col-4">Bề rộng đường:</p>
								<p class="property-detail col">
									{{
										formatFloat(property.properties[0].main_road_length) + " m"
									}}
								</p>
							</div>
							<div
								class="property-content row"
								v-if="property.properties[0].front_side === 0"
							>
								<p class="property-title col-4">Bề rộng hẻm:</p>
								<p class="property-detail col">
									{{
										formatFloat(
											property.properties[0].property_turning_time[
												property.properties[0].property_turning_time.length - 1
											].main_road_length
										) + " m"
									}}
								</p>
							...
							>
								...
								<p class="property-detail col">
									{{
										`${property.street.name}, ${property.ward.name}, ${property.district.name}, ${property.province.name}`
									}}
								</p>
							</div>
							<div
								class="property-content row"
								v-if="property.appraise_law && property.appraise_law.length > 0"
							>
								<p class="property-title col-4">Số tờ:</p>
								<p class="property-detail col">
									{{
										property.tothua && property.tothua.length > 0
											? property.tothua.length === 1
												? property.tothua[0].doc_no
												: "Nhiều tờ"
											: "Không có"
									}}
								</p>
							...
							>
								...
								>
									<div class="property-content row">
										...
										<p class="property-detail col">
											{{
												property_detail.land_type_purpose
													? property_detail.land_type_purpose.description
													: ""
											}}
										</p>
									</div>
								...
								<div class="property-content row">
									...
									<p class="property-detail col">
										{{ format(properties.appraise_land_sum_area) }} m
										<sup>2</sup>
									</p>
								</div>
								<div class="property-content row">
									...
									<div class="property-content row">
										...
										<p class="property-detail col">
											<b>{{
												format(
													detailLandPrice(
														property_detail.land_type_purpose.acronym
													)
												) +
													"đ " +
													"(" +
					...
					<div class="card-table">
						...
						>
							...
							<div class="property-content row">
								...
								<button
									class="link-detail property-detail col"
									role="link"
									@click.prevent="
										handleOpenCertificateBrief(property.certificate.id)
									"
								>
									HSTD_{{ property.certificate.id }}
								</button>
							</div>
							<div class="property-content row">
								<p class="property-title col-4">Số chứng thư:</p>
								<p class="property-detail col">
									{{ property.certificate.certificate_num }}
								</p>
							</div>
							<div class="property-content row">
								...
								<p class="property-detail col">
									{{
										property.certificate.certificate_date
											? formatDate(property.certificate.certificate_date)
											: ""
									}}
								</p>
							</div>
							<div class="property-content row">
								...
								<p class="property-detail col">
									{{
										property.certificate.appraise_date
											? formatDate(property.certificate.appraise_date)
											: ""
									}}
								</p>
							</div>
							<div class="property-content row">
								...
								<p class="property-detail col">
									{{
										property.certificate.appraiser_perform
											? property.certificate.appraiser_perform.name
											: ""
									}}
								</p>
							</div>
...
export default {
	...
	computed: {
		totalPrice() {
			let price = 0;
			let round = 0;
			let priceData = this.property.asset_price.find(
				i => i.slug === "total_asset_price"
			);
			let roundData = this.property.asset_price.find(
				i => i.slug === "round_appraise_total"
			);
			if (priceData) {
		...
		landPrice() {
			...
			let round = 0;
			let priceData = this.property.asset_price.find(
				i => i.slug === "land_asset_price"
			);
			let roundData = this.property.asset_price.find(
				i => i.slug === "round_total"
			);
			if (priceData) {
				price = priceData.value;
			}
	...
	methods: {
		...
		detailLandPrice(acronym) {
			...
			let round = 0;
			let priceData = this.property.asset_price.find(
				i => i.slug === "land_asset_purpose_" + acronym + "_price"
			);
			let roundData = this.property.asset_price.find(
				i => i.slug === "land_asset_purpose_" + acronym + "_round"
			);
			if (priceData) {
				price = priceData.value;
			}
