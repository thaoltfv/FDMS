  ...
  <div
    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="font-weight-bold">Hình ảnh tài sản so sánh</h3>
      ...
      <div class="card-body" v-if="picList.length > 0">
        ...
        <a :href="`/property/detail?id=${id[activeName]}`" class="image-caption my-3">
          Tin đăng: <span class="link-detail">TSS_{{ id[activeName] }}</span> tại địa chỉ
          <span class="link-detail">{{ address[activeName] }}</span>
        </a>
      </div>
      <div class="card-body" v-if="picList.length === 0">
...
export default {
	...
	data () {
		return {
			picList: this.pics.images,
			address: this.pics.address,
			id: this.pics.id,
			image: '',
			activeImage: 0,
			activeName: 0
