...
export default {
	...
	data () {
		return {
			valueNoFormat: this.value ? this.value : (this.value === 0 ? 0 : ''),
			errorMessage: ''
		}
	},
	model: {
		...
		async onChange (event) {
			if (event.target.value) {
				if (event.target.value.match(/^[0-9]*$/g)) {
					let valueChecked = event.target.value.match(/^[0-9]*$/g)[0]
					if (this.validateInput(valueChecked)) {
						let formatNumberDecimal = +parseFloat(valueChecked).toFixed(this.decimal)
						this.$emit('change', formatNumberDecimal)
						// change value number to dot format
						this.valueNoFormat = formatNumberDecimal
						this.errorMessage = ''
					}
				} else {
